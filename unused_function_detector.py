...
class UnusedFunctionDetector:
    ...
    def analyze_directory(self) -> None:
        ...
        print(f"📁 Excluding: {', '.join(self.default_exclude_patterns + self.exclude_patterns)}")
    ...
    def find_unused_functions(self) -> List[FunctionInfo]:
        ...
        for func_name, definitions in self.function_definitions.items():
            for func_info in definitions:
                ...
                is_called = (
                    func_name in self.function_calls or
                    func_name in self.method_calls or
                    self._check_dynamic_usage(func_name)
                )
    ...
    def _should_skip_function(self, func_info: FunctionInfo) -> bool:
        ...
        lifecycle_methods = {
            ...
            'run', 'main', 'execute', 'process', 'handle',
            'get', 'post', 'put', 'delete',  # HTTP methods
            'mousePressEvent', 'mouseReleaseEvent', 'paintEvent',  # Qt events
    ...
    def _check_dynamic_usage(self, func_name: str) -> bool:
        ...
        common_dynamic_names = {
            ...
            'validate', 'clean', 'save', 'delete', 'create',
            'encode', 'decode', 'parse', 'format'
        }
        
        return func_name in common_dynamic_names

    def generate_report(self, unused_functions: List[FunctionInfo], 
        ...
        # Summary by category
        categories = {
            'Public Functions': [f for f in unused_functions if not f.is_private and not f.is_method],
            'Private Functions': [f for f in unused_functions if f.is_private and not f.is_method],
        ...
        for file_path in sorted(by_file.keys()):
            ...
            for func in sorted(funcs, key=lambda x: x.line_number):
                ...
                report_lines.append(f"{prefix} {func.name}(){class_info} (line {func.line_number})")
    ...
    def export_json(self, unused_functions: List[FunctionInfo], 
...
def main():
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Quick analysis (excludes more patterns)"
    )
