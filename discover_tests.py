...
def discover_tests():
    ...
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30, cwd="F:/CODE/TKA")
        ...
        if result.returncode == 0:
            ...
            test_count = 0
            for line in lines:
                if "::test_" in line or line.strip().endswith(" PASSED") or line.strip().endswith(" FAILED"):
                    test_count += 1
            
            # Look for summary line
            summary_line = [line for line in lines if "collected" in line and "item" in line]
