...
class DirectThumbnailMeasurer:
        ...
        try:
            ...
            if info_label:
                ...
                if "Length:" in info_text:
                    try:
                        length_part = (
                            info_text.split("Length:")[1].split("\n")[0].strip()
                        )
    ...
    def create_report(self, analysis: Dict) -> str:
        ...
        report.append(f"\n📊 HEIGHT STATISTICS:")
        report.append(
            f"Container heights: {stats['container_height']['min']}-{stats['container_height']['max']}px (avg: {stats['container_height']['avg']:.1f})"
        )
        report.append(
            f"Image heights: {stats['image_height']['min']}-{stats['image_height']['max']}px (avg: {stats['image_height']['avg']:.1f})"
        )
        report.append(
            f"Content heights: {stats['content_height']['min']}-{stats['content_height']['max']}px (avg: {stats['content_height']['avg']:.1f})"
        )
        ...
        for m in analysis["measurements"]:
            ...
            report.append(
                f"  {m['index']:2d}: {m['sequence_word'][:15]:15} | "
    ...
    def run_measurement(self) -> bool:
        ...
        if not browse_panel:
            print(
                "❌ Could not find browse panel. Make sure TKA is running with browse tab open."
            )
            return False
