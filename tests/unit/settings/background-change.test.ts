...
describe('Background Change Functionality', () => {
  ...
  it('should have starfield as the initial default background', () => {
    ...
    if (storedSettings) {
      ...
      console.log('📦 No stored settings, using default: NIGHT_SKY (starfield)');
    }
  ...
  it('should demonstrate that selecting Aurora and clicking Apply does not change the background', async () => {
    ...
    await waitFor(() => {
      ...
      console.log('4️⃣ Current background after Apply:', currentBackground || 'none');
  ...
  it('should demonstrate that updateBodyBackground function is called but body is not updated', async () => {
    ...
    await waitFor(() => {
      ...
      try {
        ...
        console.log('❌ TEST FAILED: Body data-background attribute NOT updated');
        console.log('   Expected:', BackgroundType.AURORA);
  ...
  it('should trace the complete flow from settings change to background update', async () => {
    const { updateSettings, getSettings } = await import('$lib/shared/application/state/app-state.svelte');
    const { updateBodyBackground } = await import('$lib/shared/background');
    ...
    await waitFor(() => {
      ...
      try {
        ...
        console.log('   This is the root issue - updateBodyBackground is not being called or not working');
        throw error;
      }
    });
  });
  ...
  it('should show that the MainApplication effect is not triggering on settings change', async () => {
    ...
    console.log('4️⃣ New backgroundType in settings:', newSettings.backgroundType);
    ...
    console.log('\n❌ DIAGNOSIS:');
    console.log('   - Settings state changed:', newSettings.backgroundType === BackgroundType.AURORA);
    console.log('   - Body updated:', bodyBackground === BackgroundType.AURORA);
    console.log('   - Problem: The reactive effect or updateBodyBackground is not working');
  });
});
