...
class TestIntegratedApplicationInitialization:
    ...
    def test_complete_initialization_flow_with_error_recovery(
        self, mock_timer, mock_main_window
    ):
...
class TestErrorHandlingIntegration:
    ...
    def test_standardized_error_handling_across_components(self):
        ...
        # Create a mock logger to test error handling calls
        mock_logger = Mock()
        ...
        for call in error_calls:
            ...
            assert (
                "failed:" in message
            ), "All errors should use consistent failure format"
...
class TestServiceRegistrationIntegration:
    ...
    def test_service_registration_helper_integration(self):
        ...
        with (
            patch.object(
                ServiceRegistrationHelper, "register_common_data_services"
            ) as mock_data,
            patch.object(
                ServiceRegistrationHelper, "register_common_core_services"
            ) as mock_core,
            patch.object(
                ServiceRegistrationHelper, "register_common_session_services"
            ) as mock_session,
            patch.object(
                ServiceRegistrationHelper, "register_common_pictograph_services"
            ) as mock_pictograph,
            patch.object(
                ServiceRegistrationHelper, "register_visibility_services"
            ) as mock_visibility,
        ):
    ...
    def test_application_factory_uses_helper_consistently(self):
        ...
        with (
            patch.object(
                ServiceRegistrationHelper, "register_all_common_services"
            ) as mock_register,
            patch(
                "desktop.modern.core.dependency_injection.di_container.set_container"
            ) as mock_set,
        ):
...
class TestUIFallbackIntegration:
    ...
    def test_ui_fallback_with_error_handling_integration(
        self, mock_widget, mock_tab_widget, mock_main_window
    ):
        ...
        with (
            ...
            patch(
                "desktop.modern.application.services.ui.ui_setup_manager.logger"
            ) as mock_logger,
        ):
...
class TestEndToEndApplicationFlow:
    ...
    def test_end_to_end_application_startup(
        self, mock_get_container, mock_timer, mock_main_window
    ):
        ...
        with patch.object(ServiceRegistrationHelper, "register_all_common_services"):
            ...
            try:
                ...
                result = orchestrator.initialize_application(
                    mock_window, splash_screen=Mock(update_progress=progress_callback)
                )
