...
class ValidationSteps:
    ...
    def sequence_has_length(self, expected_length: int) -> bool:
        ...
        if actual_length == expected_length:
            ...
            logger.error(f"Sequence length validation failed: expected {expected_length}, got {actual_length}")
            return False
    ...
    def sequence_length_in_range(self, min_length: int, max_length: int) -> bool:
        ...
        if min_length <= actual_length <= max_length:
            logger.debug(f"Sequence length range validation passed: {actual_length} in [{min_length}, {max_length}]")
            return True
        else:
            logger.error(f"Sequence length range validation failed: {actual_length} not in [{min_length}, {max_length}]")
            return False
    
    def sequence_length_increased_by(self, expected_increase: int, initial_length: int) -> bool:
        """
        ...
        if actual_increase == expected_increase:
            logger.debug(f"Sequence length increase validation passed: increased by {actual_increase}")
            return True
        else:
            logger.error(f"Sequence length increase validation failed: "
    ...
    def workbench_is_functional(self) -> bool:
        ...
        try:
            length = self.workbench.get_sequence_length()
            logger.debug(f"Workbench functionality validation passed: can get length ({length})")
            return True
    ...
    def validate_sequence_state(self, expected_state: Dict[str, Any]) -> bool:
        ...
        if "valid" in expected_state:
            ...
            if actual_valid != expected_valid:
                logger.error(f"Sequence validity mismatch: expected {expected_valid}, got {actual_valid}")
                all_passed = False
        ...
        if "empty" in expected_state:
            ...
            if actual_empty != expected_empty:
                logger.error(f"Sequence empty state mismatch: expected {expected_empty}, got {actual_empty}")
                all_passed = False
        
        # Check length range if specified
        if "min_length" in expected_state and "max_length" in expected_state:
            if not self.sequence_length_in_range(expected_state["min_length"], expected_state["max_length"]):
                all_passed = False
    ...
    def assert_sequence_length(self, expected_length: int, message: Optional[str] = None):
        """
