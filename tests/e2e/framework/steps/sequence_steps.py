...
class SequenceSteps:
    ...
    def build_sequence(self, length: int) -> bool:
        ...
        for i in range(length):
            ...
            if current_length != expected_length:
                logger.warning(f"Unexpected sequence length at step {step_number}: "
        ...
        if final_length == target_length:
            ...
            logger.error(f"Sequence building failed: expected {target_length}, got {final_length}")
            return False
    ...
    def extend_sequence(self, additional_length: int) -> bool:
        ...
        if final_length == expected_length:
            logger.info(f"Successfully extended sequence: {initial_length} -> {final_length}")
            return True
        else:
            logger.error(f"Extension failed: expected {expected_length}, got {final_length}")
            return False
    
    def build_sequence_with_specific_options(self, option_identifiers: List[str]) -> bool:
        """
        ...
        for i, option_id in enumerate(option_identifiers):
            step_number = i + 1
            logger.debug(f"Adding specific option {step_number}/{len(option_identifiers)}: {option_id}")
            
            if not self.option_picker.select_option(option_id):
                logger.error(f"Failed to select option '{option_id}' at step {step_number}")
                return False
        ...
        if final_length == expected_length:
            logger.info(f"Successfully built sequence with specific options")
            return True
        else:
            logger.error(f"Specific sequence building failed: expected {expected_length}, got {final_length}")
            return False
    ...
    def get_sequence_info(self) -> Dict[str, Any]:
        ...
        logger.debug(f"Sequence info: length={info['length']}, valid={info['is_valid']}, "
