...
class BasePage(ABC):
    ...
    def wait_for_load(self, timeout: int = 5000) -> bool:
        ...
        """
        logger.debug(f"Waiting for {self.__class__.__name__} to load (timeout: {timeout}ms)")
        ...
        if self._loaded:
            ...
            logger.warning(f"{self.__class__.__name__} failed to load within {timeout}ms")
    ...
    def get_element(self, name: str, force_refresh: bool = False) -> Optional[QWidget]:
        ...
        if name not in self._elements or force_refresh:
            ...
            if self._elements[name]:
                logger.debug(f"Found element '{name}': {self._elements[name].__class__.__name__}")
            else:
    ...
    def _find_widgets_by_class_name(self, class_name_contains: str) -> List[QWidget]:
        ...
        logger.debug(f"Found {len(matches)} widgets containing class name: {class_name_contains}")
        return matches
    ...
    def _find_widget_by_object_name(self, object_name: str) -> Optional[QWidget]:
        ...
        if widget:
            logger.debug(f"Found widget by object name '{object_name}': {widget.__class__.__name__}")
        else:
