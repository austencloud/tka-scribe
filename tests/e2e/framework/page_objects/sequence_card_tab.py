...
class SequenceCardTabPageObject(BasePageObject):
    ...
    def select_card(self, card_id: str) -> bool:
        ...
        try:
            ...
            if card_element and hasattr(card_element, 'click'):
                ...
                logger.warning(f"⚠️ Could not find clickable element for card: {card_id}")
                return False
    ...
    def _find_card_elements(self) -> List[QWidget]:
        ...
        if hasattr(self.parent, "findChildren"):
            ...
            for child in children:
                ...
                if any(indicator in class_name for indicator in ["card", "sequence", "item"]):
    ...
    def _find_length_selector(self) -> Optional[QWidget]:
        ...
        if hasattr(self.parent, "findChildren"):
            ...
            for child in children:
                class_name = child.__class__.__name__.lower()
                if any(indicator in class_name for indicator in ["length", "filter", "combo"]):
                    return child
        return None
    ...
    def _find_column_selector(self) -> Optional[QWidget]:
        ...
        if hasattr(self.parent, "findChildren"):
            ...
            for child in children:
                class_name = child.__class__.__name__.lower()
                if any(indicator in class_name for indicator in ["column", "grid", "layout"]):
                    return child
        return None
        ...
        if hasattr(self.parent, "findChildren"):
            ...
            for child in children:
                if hasattr(child, 'text'):
                    ...
                    if any(keyword in text for keyword in ["refresh", "reload", "update"]):
                        return child
        return None
