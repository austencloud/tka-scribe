...
class StartPositionPickerPage(BasePage):
    ...
    def get_available_positions(self) -> List[str]:
        ...
        try:
            ...
            # Strategy 2: Try alternative method names
            for method_name in ['getAvailablePositions', 'available_positions', 'get_positions']:
                if hasattr(picker, method_name):
                    positions = getattr(picker, method_name)()
                    if positions:
                        logger.debug(f"Found {len(positions)} positions via {method_name}")
                        return positions
    ...
    def get_current_position(self) -> Optional[str]:
        ...
        try:
            ...
            for method_name in ['get_current_position', 'getCurrentPosition', 'current_position', 'selected_position']:
                if hasattr(picker, method_name):
    ...
    def _find_start_position_picker(self):
        ...
        for pattern in picker_patterns:
            ...
            if widget:
                logger.debug(f"Found start position picker: {widget.__class__.__name__}")
                return widget
        ...
        for child in children:
            ...
            if "start" in class_name and "position" in class_name:
                logger.debug(f"Found start position component: {child.__class__.__name__}")
                return child
    ...
    def _validate_picker_functionality(self, picker) -> bool:
        ...
        expected_attributes = [
            'select_position', 'selectPosition', 'setPosition',
            'get_available_positions', 'getAvailablePositions'
        ]
