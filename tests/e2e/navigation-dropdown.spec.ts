...
test.describe("Navigation Dropdown", () => {
  ...
  test("should show Build dropdown on hover/click", async ({ page }) => {
    ...
    await expect(
      dropdownMenu.locator(".dropdown-item").filter({ hasText: "Construct" })
    ).toBeVisible();
    await expect(
      dropdownMenu.locator(".dropdown-item").filter({ hasText: "Generate" })
    ).toBeVisible();
    await expect(
      dropdownMenu.locator(".dropdown-item").filter({ hasText: "Edit" })
    ).toBeVisible();
    await expect(
      dropdownMenu.locator(".dropdown-item").filter({ hasText: "share" })
    ).toBeVisible();
  });

  test("should switch build modes when dropdown item is clicked", async ({
    page,
  }) => {
