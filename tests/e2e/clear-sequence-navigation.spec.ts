...
test.describe("Clear Sequence Navigation from Animate Tab", () => {
  ...
  test("returns to Construct when clearing from Animate after Construct", async ({ page }) => {
  ...
  test("returns to Generate when clearing from Animate after Generate", async ({ page }) => {
    // 1. Navigate to Build module
    const buildTab = page.locator(".nav-tab-container").filter({ hasText: "Build" });
    await buildTab.click();
  ...
  test("handles complex navigation: Construct -> Generate -> Animate -> Clear", async ({ page }) => {
    // 1. Navigate to Build module
    const buildTab = page.locator(".nav-tab-container").filter({ hasText: "Build" });
    await buildTab.click();
