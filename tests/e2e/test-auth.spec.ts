...
test('Google Sign-In Flow Test', async ({ page }) => {
  ...
  page.on('console', msg => {
    const text = msg.text();
    logs.push(text);
    if (text.includes('[google]') || text.includes('[authStore]') || text.includes('Clearing')) {
  ...
  await page.screenshot({ path: 'test-screenshots/02-user-menu.png', fullPage: true });
  ...
  await page.screenshot({ path: 'test-screenshots/03-auth-sheet.png', fullPage: true });
  ...
  if (currentUrl.includes('accounts.google.com')) {
    ...
    await page.screenshot({ path: 'test-screenshots/03-google-auth.png', fullPage: true });
    ...
    await page.screenshot({ path: 'test-screenshots/03-redirect-failed.png', fullPage: true });
    console.log('📸 Screenshot: redirect failed');
  } else {
    console.log('⚠️ Unexpected URL:', currentUrl);
    await page.screenshot({ path: 'test-screenshots/03-unexpected-url.png', fullPage: true });
  }
