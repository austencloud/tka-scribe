...
describe("Letter Constraint Generation", () => {
  ...
  beforeAll(async () => {
    ...
    const { ensureContainerInitialized, loadFeatureModule } = await import(
  ...
  describe("Freeform Mode", () => {
    TEST_LETTERS.forEach(({ letter, name }) => {
      it(`should generate sequences containing ${name} within 5 seconds`, async () => {
        ...
        console.log(
          `  ✓ Generated "${word}" in ${duration}ms (contains '${letterStr}': ${word.includes(letterStr)})`
        );
  ...
  describe("Circular Mode", () => {
    TEST_LETTERS.forEach(({ letter, name }) => {
      it(`should generate circular sequences containing ${name} within 5 seconds`, async () => {
        ...
        console.log(
          `  ✓ Generated circular "${word}" in ${duration}ms (contains '${letterStr}': ${word.includes(letterStr)})`
        );
  ...
  describe("Stress Test - Multiple Iterations", () => {
    it("should reliably generate sequences with letter A (10 iterations)", async () => {
      ...
      console.log(
        `     Success rate: ${results.filter((r) => r.contains).length}/${iterations}`
      );
      console.log(`     Avg duration: ${avgDuration.toFixed(0)}ms`);
      console.log(`     Min duration: ${minDuration}ms`);
      console.log(`     Max duration: ${maxDuration}ms`);
      console.log(
        `     Words generated: ${results.map((r) => r.word).join(", ")}`
      );
