...
<script lang="ts">
  ...
  import type { Beat } from "$legacyLib/types/Beat";
  ...
  let exportResult: { dataUrl: string; width: number; height: number } | null =
    $state(null);
  ...
  async function handleExport() {
    if (!browser || !sequenceContainer) {
      exportError =
        "Cannot export: not in browser environment or no sequence container";
      return;
    }
...
<div class="enhanced-export-tester">
  ...
  <div class="content">
    <div class="sequence-container" bind:this={sequenceContainer}>
      ...
      <div
        class="beat-grid"
        style="--columns: {columns}; --spacing: {spacing}px;"
      >
          ...
          <div class="note-length-test">
            ...
            <div class="note-buttons">
              {#each noteOptions as option}
                <button
                  class="note-test-button"
                  onclick={() => (notes = option.text)}
                >
                  {option.label}
                </button>
              {/each}
