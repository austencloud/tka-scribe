<script lang="ts">
  ...
  async function runTest(
    size: (typeof testSizes)[0],
    sequence: (typeof testSequences)[0]
  ) {
    try {
      ...
      // Create dummy beats
      const beats: Beat[] = Array.from({ length: sequence.beats }, (_, i) =>
        createDummyBeat(i)
      );
...
<div class="text-rendering-test">
  ...
  <div class="test-results">
    <h2>
      Test Results ({testResults.length} of {testSizes.length *
          ...
          >
            ...
            {#if result.success}
              <div class="image-container">
                <img
                  src={result.imageUrl}
                  alt={`${result.sequence} at ${result.size}`}
                />
              </div>
            {:else}
