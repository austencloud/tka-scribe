...
export async function downloadImage(
  ...
  if (!browser) {
    return Promise.reject(
      new Error("Cannot download: not in browser environment")
    );
  }
  ...
  try {
    ...
    const mimeType =
      options.mimeType ||
      options.dataUrl.split(",")[0].match(/:(.*?);/)?.[1] ||
      "image/png";
    ...
    if (window.showSaveFilePicker) {
      try {
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                // @ts-ignore - This is a non-standard property that might be available in some browsers
                if (
                  (fileHandle as any).directoryHandle &&
                  (fileHandle as any).directoryHandle.name
                ) {
                  // @ts-ignore
                  folderPath = (fileHandle as any).directoryHandle.name;
                  console.log(
                ...
                // @ts-ignore - This is a non-standard property that might be available in some browsers
                  ...
                  if (lastSlashIndex > 0) {
                    ...
                    console.log(
                      "DownloadUtils: Got folder path from fileHandle.path:",
                      folderPath
                    );
                  }
                }
                ...
                // @ts-ignore - This is a non-standard property that might be available in some browsers
                  ...
                  try {
                    ...
                    console.warn(
                      "DownloadUtils: Error getting parent directory:",
                      parentDirError
                    );
                  }
                }
                ...
                if (!folderPath) {
                  ...
                  console.log(
                    "DownloadUtils: Using default Downloads folder path"
                  );
                }
              } catch (pathError) {
                console.warn(
                  "DownloadUtils: Could not get file path details:",
                  pathError
                );
              ...
              console.log(
                `DownloadUtils: File saved successfully to ${folderPath}/${fileName}`
              );
              return {
                success: true,
                fileName,
              ...
              console.error(
                "DownloadUtils: Error writing to file:",
                writeError
              );
              throw writeError;
            }
            ...
            console.error(
              "DownloadUtils: Error creating writable stream:",
              writableError
            );
            throw writableError;
          }
          ...
          console.error(
            "DownloadUtils: Error with save file picker:",
            pickerError
          );
          throw pickerError;
        }
      ...
      try {
        ...
        if (newWindow.document) {
          ...
          try {
            ...
            console.warn(
              "DownloadUtils: Modern document writing failed, using fallback",
              docError
            );
        ...
        console.error(
          "DownloadUtils: Alternative approach failed",
          windowError
        );
