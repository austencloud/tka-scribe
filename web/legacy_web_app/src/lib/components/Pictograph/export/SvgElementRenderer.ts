...
export async function renderSvgElements(
  ...
  const startPositionContainer =
    containerElement.querySelector(".start-position");
  ...
  if (startPositionContainer) {
    console.log(
      "EnhancedExporter: Found start position container in DOM, including it in export"
    );

    const startPositionSvg = startPositionContainer.querySelector("svg");
    if (startPositionSvg) {
      // Find the index of this SVG in our svgElements array
      startPositionIndex = svgElements.findIndex(
        (svg) => svg === startPositionSvg
      );
      console.log(
  ...
  if (startPositionIndex === -1) {
    for (let i = 0; i < svgElements.length; i++) {
      ...
      if (grandparent?.classList.contains("start-position")) {
        ...
        console.log(
          "EnhancedExporter: Found start position element at index",
          i
        );
        break;
      }
  ...
  for (let i = 0; i < svgElements.length; i++) {
    ...
    console.log(
      `EnhancedExporter: Processed SVG element ${i + 1}/${svgElements.length}`
    );

    // If this is a regular beat (not start position), check if it has reversal indicators
    if (
      !(hasStartPosition && i === startPositionIndex) &&
      options.addReversalSymbols
    ) {
