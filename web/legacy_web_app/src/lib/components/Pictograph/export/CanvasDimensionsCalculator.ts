import { autoAdjustLayout } from "$legacyLib/components/SequenceWorkbench/BeatFrame/beatFrameHelpers";
...
export function calculateDimensions(
  ...
  const bottomMarginPercentage = Math.max(
    0.05,
    0.1 - (rowsForBeats - 1) * 0.02
  );

  // Calculate margins with adjusted percentages
  const calculatedTopMargin = options.addWord
    ? Math.round(height * topMarginPercentage)
    : 0;
  ...
  const topMargin = options.addWord
    ? Math.max(calculatedTopMargin, MIN_TOP_MARGIN)
    : 0;
