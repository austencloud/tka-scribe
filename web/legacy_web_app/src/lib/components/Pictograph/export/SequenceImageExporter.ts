...
export async function exportSequenceImage(
  ...
  if (!browser) {
    return Promise.reject(
      new Error("Cannot export: not in browser environment")
    );
  }
  ...
  if (!containerElement) {
    return Promise.reject(
      new Error("Cannot export: no container element provided")
    );
  }
  ...
  try {
    ...
    const reversalIndicators = Array.from(
      containerElement.querySelectorAll(".reversal-indicator")
    );
    console.log(
      `EnhancedExporter: Found ${reversalIndicators.length} reversal indicators`
    );
    ...
    console.log("EnhancedExporter: Start position details", {
      ...
      hasStartPosition:
        mergedOptions.startPosition !== null &&
    ...
    if (mergedOptions.addDifficultyLevel) {
      ...
      adjustedSpacing =
        requiredSpace > topMargin
          ? Math.max(
              Math.floor((topMargin - 2 * radius) / 2),
              Math.floor(radius * 0.1)
            ) // Ensure at least 10% of radius as spacing
          : desiredSpacing;
