...
export async function exportSequenceAsImage(
  ...
  try {
    ...
    const defaultOptions: Required<
      Omit<SequenceExportOptions, "beats" | "startPosition">
    > = {
    ...
    const totalBeats =
      mergedOptions.beats.length + (mergedOptions.startPosition ? 1 : 0);
    ...
    const renderSvgElement = async (svgElement: SVGElement, index: number) => {
      try {
        ...
        console.error(
          `SequenceExporter: Error processing SVG element ${index}:`,
          error
        );
        throw error;
      }
