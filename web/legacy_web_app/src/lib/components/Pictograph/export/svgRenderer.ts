...
export async function renderSvgToImage(
  ...
  if (!browser) {
    return Promise.reject(
      new Error("Cannot render: not in browser environment")
    );
  }
  ...
  try {
    ...
    const imageLoadPromise = new Promise<SVGRenderResult>((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        console.log(
          "SVGRenderer: Generated data URL of length",
          dataUrl.length
        );
    ...
    try {
      ...
      console.warn(
        "SVGRenderer: Blob approach failed, falling back to data URL",
        blobError
      );
...
async function ensureTKAGlyphVisibility(svgElement: SVGElement): Promise<void> {
  try {
    ...
    if (tkaGlyphElements.length > 0) {
      console.log(
        `SVGRenderer: Found ${tkaGlyphElements.length} TKAGlyph elements`
      );
      ...
      for (const glyphElement of Array.from(tkaGlyphElements)) {
        ...
        const letterElements =
          glyphElement.querySelectorAll(".tka-letter image");
        ...
        for (const letterElement of Array.from(letterElements)) {
          ...
          const href =
            letterElement.getAttribute("href") ||
            letterElement.getAttribute("xlink:href");
          if (href && href.startsWith("/")) {
            ...
            try {
              ...
              console.log(
                "SVGRenderer: Updated TKAGlyph letter image with data URL"
              );
            } catch (fetchError) {
              console.error(
                "SVGRenderer: Failed to fetch TKAGlyph letter SVG:",
                fetchError
              );
            }
          }
