<script lang="ts">
  import { popIn } from "$legacyLib/transitions/popIn";
  ...
  onMount(() => {
    ...
    const serviceStore = {
      ...
      update: (
        updater: (value: PictographService | null) => PictographService | null
      ) => {
    ...
    const lastDataSnapshotStore = {
      ...
      update: (
        updater: (
          value: PictographDataSnapshot | null
        ) => PictographDataSnapshot | null
      ) => {
          ...
          {#each [{ color: "red", propData: redPropData, arrowData: redArrowData }, { color: "blue", propData: bluePropData, arrowData: blueArrowData }] as { color, propData, arrowData } (color)}
            {#if propData}
              <Prop
                ...
                on:error={(e) =>
                  handleComponentError(`${color}Prop`, e.detail.message)}
              />
            {/if}
            ...
            {#if arrowData}
              <Arrow
                ...
                on:error={(e) =>
                  handleComponentError(`${color}Arrow`, e.detail.message)}
              />
            {/if}
          {/each}
        </g>
