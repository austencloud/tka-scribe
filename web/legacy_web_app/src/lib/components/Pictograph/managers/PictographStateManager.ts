 ...
 */
export function createPictographDataStore(
  initialData?: PictographData
): Writable<PictographData> {
...
export function setupPictographDataSubscription(
  ...
  unsubscribe: () => void;
  getSnapshot: () => PictographDataSnapshot | null;
} {
  ...
  const unsubscribe = pictographDataStore.subscribe((data) => {
    if (data && service) {
      ...
      const { hasChanged, updatedSnapshot } = checkForChanges(
        data,
        currentSnapshot
      );
      ...
      if (hasChanged) {
        if (debug)
          console.debug("Pictograph data changed, updating components");
