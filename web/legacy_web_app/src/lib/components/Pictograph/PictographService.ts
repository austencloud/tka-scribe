...
import type { MotionData } from "$legacyLib/components/objects/Motion/MotionData";
import type { PropData } from "$legacyLib/components/objects/Prop/PropData";
import type { PictographData } from "$legacyLib/types/PictographData";
import { PropType, type Color, type Loc } from "$legacyLib/types/Types";

import { Motion } from "$legacyLib/components/objects/Motion/Motion";
import { BLUE, DASH, RED } from "$legacyLib/types/Constants";

import ArrowLocationManager from "$legacyLib/components/objects/Arrow/ArrowLocationManager";
import ArrowRotAngleManager from "$legacyLib/components/objects/Arrow/ArrowRotAngleManager";
import { pictographContainer } from "$legacyLib/state/stores/pictograph/pictographContainer";
...
export class PictographService {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Pictograph initialization failed";
  ...
  createPropData(motionData: MotionData, color: Color): PropData {
    ...
    const endOri = motion
      ? motion.calculateFinalEndOrientation()
      : motionData.endOri;
  ...
  createArrowData(motionData: MotionData, color: Color): ArrowData {
    ...
    const letterType = this.data.letter
      ? LetterType.getLetterType(this.data.letter)
      : null;
    ...
    const arrowData: ArrowData = {
      ...
      color,
      coords: { x: 0, y: 0 },
      loc: arrowLoc || motionData.startLoc, // Fallback to startLoc if arrowLoc is null
      rotAngle: 0,
      svgMirrored: false,
      svgCenter: { x: 0, y: 0 },
      svgLoaded: false,
  ...
  positionComponents(
    ...
    try {
      ...
      if (
        redProp &&
        blueProp &&
        this.checker.checkLetterCondition(LetterConditions.BETA_ENDING)
      ) {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Component positioning failed";
  ...
  private positionProp(prop: PropData, grid: GridData): void {
    ...
    prop.coords =
      grid.allHandPointsNormal?.[pointName]?.coordinates ??
      this.getFallbackPosition(prop.loc);
  }
    ...
    if (redArrow && this.data.redMotion) {
      ...
      if (isType3 && this.data.redMotion.motionType === DASH) {
        try {
          ...
          console.error(
            "Error recalculating Type 3 red dash arrow location:",
            error
          );
        }
      }
    ...
    if (blueArrow && this.data.blueMotion) {
      ...
      if (isType3 && this.data.blueMotion.motionType === DASH) {
        try {
          ...
          console.error(
            "Error recalculating Type 3 blue dash arrow location:",
            error
          );
        }
      }
  ...
  private getFallbackPosition(loc?: string): { x: number; y: number } {
    ...
    return loc && fallbackPositions[loc]
      ? fallbackPositions[loc]
      : { x: 475, y: 475 };
  }

  getShiftMotion(): Motion | null {
    const motions = [this.data.redMotion, this.data.blueMotion].filter(
      (m): m is Motion => !!m
    );

    // Find the shift motion
    const shiftMotion =
      motions.find((m) => ["pro", "anti", "float"].includes(m.motionType)) ??
      null;
  ...
  getOtherMotion(motion: Motion): Motion | null {
    ...
    const otherMotion =
      otherColor === RED
        ? (this.data.redMotion ?? null)
        : (this.data.blueMotion ?? null);
  ...
  updateData(newData: PictographData): void {
    try {
      ...
      if (shouldForceReset) {
        ...
        if (newData.redArrowData) {
          ...
          if (newData.redMotion) {
            ...
            newData.redArrowData.rotAngle =
              rotAngleManager.calculateRotationAngle(
                newData.redMotion,
          ...
          if (newData.redMotion && newData.redMotionData) {
            if (
              newData.redArrowData.motionType !==
              newData.redMotionData.motionType
            ) {
              newData.redArrowData.motionType =
                newData.redMotionData.motionType;
            }
        ...
        if (newData.blueArrowData) {
          ...
          if (newData.blueMotion) {
            ...
            newData.blueArrowData.rotAngle =
              rotAngleManager.calculateRotationAngle(
                newData.blueMotion,
          ...
          if (newData.blueMotion && newData.blueMotionData) {
            if (
              newData.blueArrowData.motionType !==
              newData.blueMotionData.motionType
            ) {
              newData.blueArrowData.motionType =
                newData.blueMotionData.motionType;
            }
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to update pictograph data";
