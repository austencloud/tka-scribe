<script lang="ts">
  ...
  import { useResizeObserver } from "$legacyLib/composables/useResizeObserver";
  import { defaultPictographData } from "$legacyLib/components/Pictograph/utils/defaultPictographData";
  import { autoAdjustLayout, calculateCellSize } from "./beatFrameHelpers";
  import { selectedStartPos } from "$legacyLib/stores/sequence/selectionStore";
  ...
  import { layoutStore } from "$legacyLib/stores/layout/layoutStore";
  import { createEventDispatcher, setContext } from "svelte";

  // Import the sequence container and integration utilities
  import { sequenceContainer } from "$legacyLib/state/stores/sequence/SequenceContainer";
  import { useContainer } from "$legacyLib/state/core/svelte5-integration.svelte";
  import type { BeatFrameLayoutOptions } from "$legacyLib/types/BeatFrameLayoutOptions"; // Added import
  ...
  function safeLog(message: string, data: any) {
    if (import.meta.env.DEV) {
      ...
      console.log(
        message,
        data instanceof Object ? $state.snapshot(data) : data
      );
    }
  }
  ...
  const selectedBeatIndex = $derived(
    selectedBeatIds.length > 0
      ? beats.findIndex((beat) => beat.id === selectedBeatIds[0])
      : -1
  );
  ...
  function convertContainerBeatsToLegacyFormat(
    containerBeats: any[]
  ): LegacyBeatData[] {
  ...
  $effect(() => {
    ...
    if (beatRows !== prevRows || beatCols !== prevCols) {
      // Log layout changes
      safeLog(`Layout changed`, {
        from: `${prevRows}x${prevCols}`,
        to: `${beatRows}x${beatCols}`,
      });
    ...
    if (beatFrameContainerRef) {
      ...
      if (typeof elementReceiver === "function") {
        try {
          ...
          console.log(
            "BeatFrame: Element passed to receiver:",
            beatFrameContainerRef
          );
        } catch (error) {
        ...
        console.error(
          "BeatFrame: elementReceiver is not a function:",
          elementReceiver
        );
      }
  ...
  onMount(() => {
    if (browser) {
      ...
      const observer = new MutationObserver((_mutations) => {
        ...
        if (import.meta.env.DEV) {
          safeLog(
            "BeatFrame: DOM mutation detected, updating element reference",
            {}
          );
        }
        ...
        if (typeof elementReceiver === "function") {
          try {
            ...
            console.error(
              "BeatFrame: Error calling elementReceiver after DOM mutation:",
              error
            );
          }
        }
  ...
  onMount(() => {
    ...
    const handleStartPosSelected = (event: CustomEvent) => {
      if (event.detail?.startPosition) {
        ...
        // Create a deep copy to avoid reference issues
        const newStartPos = JSON.parse(
          JSON.stringify(event.detail.startPosition)
        );
  ...
  function handleStartPosBeatClick() {
    ...
    const startPosCopy = startPosition
...
>
  ...
  >
    {#each Array(beatRows) as _, rowIndex}
      {#if rowIndex === 0}
        <div
          class="beat-container start-position"
...
<style>
  ...
  .beat-frame {
    ...
    padding: 0 calc(var(--safe-inset-right, 0px))
      calc(20px + var(--safe-inset-bottom, 0px))
