...
<script lang="ts">
  ...
  import { showError } from "$legacyLib/components/shared/ToastManager.svelte";
  import {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (downloadResult) {
        ...
        console.log(
          "ShareButton: Download was cancelled or failed without error"
        );
      }
    } catch (error) {
...
<style>
  ...
  :global(.sequence-widget > .main-layout > .share-button) {
    ...
    ) !important; /* Exactly match settings button */
    right: calc(
      var(--button-size-factor, 1) * 10px
  ...
  :global(.sequence-widget > .main-layout > .dropdown-container) {
    position: absolute !important;
    top: calc(
      var(--button-size-factor, 1) * 10px + var(--button-size-factor, 1) *
        45px + 5px
    ) !important;
  ...
  .share-button {
    ...
    color: var(
      --tkc-icon-color-share,
      #00bcd4
    ); /* Teal icon color, or a theme variable */
    border: none;
  ...
  .share-button:hover {
    ...
    color: var(
      --tkc-icon-color-share-hover,
      #6c9ce9
    ); /* Match settings button hover color */
  }
