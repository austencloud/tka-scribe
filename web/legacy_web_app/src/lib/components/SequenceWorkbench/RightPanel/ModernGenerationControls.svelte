...
<script lang="ts">
  ...
  import {
    settingsStore,
    type CAPType,
  } from "$legacyLib/state/stores/settingsStore";
  import GenerateButton from "$legacyLib/components/GenerateTab/components/GenerateButton.svelte";
  import CircularSequencer from "$legacyLib/components/GenerateTab/components/CircularSequencer.svelte";
  import FreeformSequencer from "$legacyLib/components/GenerateTab/components/FreeformSequencer.svelte";
  ...
  // Computed values
  const currentIntensityLabel = $derived(
    intensityLabels[turnIntensity - 1] || "Moderate"
  );
        ...
        >
          <div class="toggle-labels">
            {#each continuityOptions as option}
              <span
                class="toggle-label"
                class:selected={option.id === propContinuity}
              >
                {option.label}
              </span>
            {/each}
          ...
          <div
            class="toggle-thumb"
            class:right={propContinuity === "random"}
          ></div>
        </button>
      </div>
