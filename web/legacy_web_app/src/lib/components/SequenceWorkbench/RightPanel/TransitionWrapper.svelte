...
<script lang="ts">
  ...
  // Props using Svelte 5 runes with snippets
  const props = $props<{
    isSequenceEmpty: boolean;
    transitionDuration?: number;
    startPosPicker: Snippet;
    optionPicker: Snippet;
  }>();
  ...
  function updateHeights() {
    ...
    if (containerElement) {
      ...
      if (isFullScreen) {
        ...
        const containerParentHeight =
          containerElement.parentElement?.clientHeight || 0;
    ...
    const newHeight = props.isSequenceEmpty
      ? startPosPickerHeight
      : optionPickerHeight;
  ...
  function handleTransition() {
    ...
    if (startPosPickerElement) {
      startPosPickerHeight =
        startPosPickerElement.scrollHeight || startPosPickerHeight;
    }
    if (optionPickerElement) {
      optionPickerHeight =
        optionPickerElement.scrollHeight || optionPickerHeight;
    }
