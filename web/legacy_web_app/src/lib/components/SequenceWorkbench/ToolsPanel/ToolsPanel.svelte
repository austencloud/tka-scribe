...
<script lang="ts">
  ...
  // Props
  let { buttons = [], activeMode = null, onaction, onclose } = $props<{
  ...
  const modeButtons = buttons.filter((b: any) =>
    ["constructMode", "generateMode"].includes(b.id)
  );
  const sharingButtons = buttons.filter((b: any) =>
    ["viewFullScreen", "saveImage"].includes(b.id)
  );
  const manipulationButtons = buttons.filter((b: any) =>
    ["mirrorSequence", "swapColors", "rotateSequence"].includes(b.id)
  );
  const dictionaryButtons = buttons.filter((b: any) =>
    ["addToDictionary"].includes(b.id)
  );
  const destructiveButtons = buttons.filter((b: any) =>
  ...
  function updateGridLayout() {
    ...
    const minColumnsNeeded = Math.ceil(
      buttonCount / Math.floor(availableHeight / minButtonSize)
    );
  ...
  <div class="tools-content">
    <div class="tools-grid" bind:this={gridContainer}>
      {#each orderedButtons as button}
        <button
          class="tool-button {button.id.includes('delete') ||
          button.id.includes('clear')
            ? 'destructive'
            : ''} {(button.id === 'constructMode' &&
        ...
        >
          <i
            class="fa-solid {button.id === 'saveImage'
              ? 'fa-share-nodes'
              : button.icon}"
          ></i>
