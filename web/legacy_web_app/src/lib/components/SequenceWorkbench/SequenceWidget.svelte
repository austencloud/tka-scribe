<script lang="ts">
  import { useResponsiveLayout } from "$legacyLib/composables/useResponsiveLayout";
  import { onMount, onDestroy, getContext } from "svelte";
  import { useResizeObserver } from "$legacyLib/composables/useResizeObserver";
  import { browser } from "$app/environment";
  import {
    BEAT_FRAME_CONTEXT_KEY,
    type ElementContext,
  ...
  import { openSequenceOverlay } from "$legacyLib/state/sequenceOverlay/sequenceOverlayState";

  import type { ActionEventDetail } from "./ButtonPanel/types";
  import { sequenceActions } from "$legacyLib/state/machines/sequenceMachine";
  ...
  import SettingsButton from "$legacyLib/components/MenuBar/SettingsButton/SettingsButton.svelte";
  ...
  $effect(() => {
    ...
    if (beatFrameElement && browser) {
      try {
        ...
        console.error(
          "SequenceWidget: Error storing beatFrameElement availability:",
          error
        );
      }
    }
    ...
    if (browser) {
      ...
      const observer = new MutationObserver(() => {
        if (!beatFrameElement) {
          ...
          const beatFrameElements = document.querySelectorAll(
  ...
  onMount(() => {
    ...
    document.addEventListener(
      "beat-selected",
      handleBeatSelected as EventListener
    );
  });
  ...
  onDestroy(() => {
    if (buttonActionListener) {
      document.removeEventListener(
        "action",
        buttonActionListener as EventListener
      );
    }
    document.removeEventListener(
      "beat-selected",
      ...
      <div class="left-vbox">
        <SequenceContent
          ...
          onBeatSelected={(beatId) => {
            ...
            const customEvent = new CustomEvent<{ beatId: string }>(
              "beatselected",
              {
                detail: { beatId },
              }
