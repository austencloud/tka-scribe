...
<script lang="ts">
  ...
  import { exportSequenceImage } from "$legacyLib/components/Pictograph/export/SequenceImageExporter";
  import {
    createTemporaryRenderElement,
    cloneBeatFrameContent,
    removeTemporaryElement,
    logBeatFrameDetails,
  } from "$legacyLib/components/SequenceWorkbench/BeatFrame/beatFrameHelpers";
  ...
  export async function updatePreview() {
    ...
    if (!browser || !previewElement) {
      console.log(
        "⚠️ Skipping preview update: browser or previewElement not available"
      );
      return;
    }
...
<div class="preview-panel" bind:this={previewElement}>
  ...
  {:else if error}
    <div class="error-container">
      ...
      {:else}
        <div class="help-text">
          Try refreshing the page or creating a new sequence. If the problem
