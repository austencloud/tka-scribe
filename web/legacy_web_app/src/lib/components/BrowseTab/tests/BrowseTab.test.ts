 ...
 */
import { browseTabStore } from "$legacyLib/stores/browseTab/browseTabStore";
import { mockRender } from "$legacyLib/testing/svelte5-test-helpers";
...
vi.mock("$legacyLib/services/sequenceService", () => ({
...
import * as sequenceService from "$legacyLib/services/sequenceService";
...
vi.mock(
  "$legacyLib/components/MainWidget/loading/LoadingSpinner.svelte",
  () => ({
    default: vi.fn().mockImplementation(() => ({
      $$typeof: Symbol.for("svelte.component"),
      render: () => ({
...
describe("GalleryTab Component", () => {
  ...
  it("should render the component", () => {
    const { container } = mockRender(GalleryTab);
  ...
  it("should load data on mount", async () => {
    ...
    // Create a mock GalleryTab component with an onMount method
    const MockBrowseTab = {
      prototype: {
        onMount: () => {
          browseTabStore.loadInitialData();
  ...
  it("should handle sequence selection", async () => {
    ...
    vi.mocked(sequenceService.fetchSequences).mockResolvedValueOnce(
      mockSequences
    );
