...
function generateOrientations(): { blue: string; red: string } {
  ...
  const blueOrientation =
    orientations[Math.floor(Math.random() * orientations.length)];
  const redOrientation =
    orientations[Math.floor(Math.random() * orientations.length)];
...
export async function createFreeformSequence(
  options: SequenceGenerationOptions
) {
  try {
    generatorStore.startGeneration();
    generatorStore.updateProgress(
      10,
      "Initializing freeform sequence generation"
    );

    // Step 1: Determine rotation parameters
    const rotationDirection = determineRotationDirection(
