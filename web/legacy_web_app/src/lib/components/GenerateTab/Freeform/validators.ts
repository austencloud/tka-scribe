...
export function validateFreeformSequence(
  ...
  const validators: Array<
    (seq: BeatData[], types: string[]) => ValidationResult
  > = [
...
function validateLetterTypeCompatibility(
  ...
  const validTypes =
    selectedLetterTypes.length > 0 ? selectedLetterTypes : VALID_LETTER_TYPES;

  // Validate letter types
  for (const [index, beat] of sequence.entries()) {
    const letterType = (beat as any).letter || (beat as any).letterType || "";
    if (!validTypes.includes(letterType)) {
      errors.push(
        `Beat ${index} has incompatible letter type: ${letterType}`
      );
    }
  }
...
function validateOrientations(
  ...
  for (const [index, beat] of sequence.entries()) {
    ...
    if (!VALID_ORIENTATIONS.includes(beat.orientation.blue)) {
      errors.push(
        `Invalid blue orientation at beat ${index}: ${beat.orientation.blue}`
      );
    }
    ...
    if (!VALID_ORIENTATIONS.includes(beat.orientation.red)) {
      errors.push(
        `Invalid red orientation at beat ${index}: ${beat.orientation.red}`
      );
    }
  }
...
function validateTurnIntensity(
  ...
  for (const [index, beat] of sequence.entries()) {
    if (beat.turnIntensity < 0 || beat.turnIntensity > 3) {
      errors.push(
        `Invalid turn intensity at beat ${index}: ${beat.turnIntensity}`
      );
    }
  }
