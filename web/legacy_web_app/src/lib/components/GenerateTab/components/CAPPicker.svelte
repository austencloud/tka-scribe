<script
  lang="ts"
  generics="T extends { id: string; label: string; description: string }"
>
  ...
  const groupedCapTypes = $derived({
    mirror: props.capTypes.filter((cap: T) =>
      cap.id.toLowerCase().includes("mirrored")
    ),
    rotate: props.capTypes.filter((cap: T) =>
      cap.id.toLowerCase().includes("rotated")
    ),
    ...
    {#if typesInGroup.length > 0}
      <div class="cap-group">
        <h5 class="group-title">
          {groupName.charAt(0).toUpperCase() + groupName.slice(1)} Types
        </h5>
