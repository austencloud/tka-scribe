...
export class PropPlacementManager {
  ...
  constructor(
    ...
    if (!gridData) {
      ...
      throw new Error(
        "Grid data is required to initialize PropPlacementManager"
      );
    }
  ...
  public updatePropPlacement(props: PropData[]): PropData[] {
    if (!props.length) {
      console.warn(
        "⚠️ PropPlacementManager: No props provided to updatePropPlacement"
      );
      return props;
    }
    ...
    props.forEach((prop) => {
      if (!prop.loc) {
        console.error(
          `❌ PropPlacementManager: Prop missing location data:`,
          prop.id
        );
        return;
      }
    ...
    if ((this.checker as any).endsWithBeta()) {
  ...
  private validatePropPositions(props: PropData[]): void {
    const invalidProps = props.filter(
      (p) => !p.coords || (p.coords.x === 0 && p.coords.y === 0)
    );
