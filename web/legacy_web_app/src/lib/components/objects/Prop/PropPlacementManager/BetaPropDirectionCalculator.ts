...
export class BetaPropDirectionCalculator {
  ...
  getDirection(prop: PropData): Direction | null {
    ...
    if (
      this.pictographData.letter === Letter.G ||
      this.pictographData.letter === Letter.H
    ) {
      ...
      if (redMotion && (prop.color === RED || prop.color === BLUE)) {
        ...
        if ([PRO, ANTI, FLOAT].includes(redMotion.motionType)) {
          ...
          motionDirection = this.getShiftDirection(
            isRadial,
            redMotion.startLoc,
            redMotion.endLoc
          );
          ...
          if (isDiamond) {
            const map = isRadial
              ? this.diamondMapRadial
              : this.diamondMapNonRadial;
        ...
        return prop.color === RED
          ? motionDirection
          : this.getOppositeDirection(motionDirection);
      }
    }
  ...
  private handleShiftMotion(
    ...
    if (this.pictographData.letter === Letter.I) {
      ...
      if (this.endsWithNonRadialOrientation()) {
        return this.directionMapNonRadialI[prop.loc as Loc][
          prop.color as Color
        ];
      }
    }
