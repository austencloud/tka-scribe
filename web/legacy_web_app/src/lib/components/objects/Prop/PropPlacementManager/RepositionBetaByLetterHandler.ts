...
import type { Direction } from "$legacyLib/types/Types";
import type { MotionData } from "$legacyLib/components/objects/Motion/MotionData";
import { PRO, ANTI, FLOAT } from "$legacyLib/types/Constants";
...
export default class RepositionBetaByLetterHandler {
  ...
  reposition_G_H(): void {
    ...
    const furtherDirection =
      this.dirCalculator.getDirectionFromMotion(redMotion);
    if (!furtherDirection) return;

    // Get the opposite direction
    const otherDirection =
      this.dirCalculator.getOppositeDirection(furtherDirection);
  ...
  reposition_Y_Z(): void {
    ...
    const staticProp =
      staticMotion.color === "red" ? redPropData : bluePropData;
  ...
  private moveProp(prop: PropData, direction: Direction): void {
    ...
    const newPosition =
      this.betaOffsetCalculator.calculateNewPositionWithOffset(
        prop.coords,
        direction
      );
