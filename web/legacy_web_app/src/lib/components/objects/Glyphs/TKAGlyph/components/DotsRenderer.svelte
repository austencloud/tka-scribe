...
<script lang="ts">
  import { assetCache, type Rect } from "$legacyLib/stores/glyphStore";
  import { LetterType } from "$legacyLib/types/LetterType";
  import type { Letter } from "$legacyLib/types/Letter";
  import type { DirRelation, PropRotDir } from "$legacyLib/types/Types";
  ...
  $: dotPositions = calculateDotPositions(
    letterRect,
    $assetCache.dotSVG?.dimensions
  );
  $: canShowDots =
    letter && LetterType.getLetterType(letter) !== LetterType.Type1;
  $: dotsAvailable = $assetCache.dotSVG !== null;
  $: dotsVisible =
    canShowDots && dotsAvailable && direction !== null && shouldShowDots;
