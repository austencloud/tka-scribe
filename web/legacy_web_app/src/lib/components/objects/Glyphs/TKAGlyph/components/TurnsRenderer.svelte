...
<script lang="ts">
  ...
  import {
    ...
    fetchSVGDimensions,
    type Rect,
  } from "$legacyLib/stores/glyphStore";
  import type { TKATurns } from "$legacyLib/types/Types";
  ...
  function calculateTurnsPositions(
    rect: Rect,
    paddingX: number,
    paddingY: number
  ) {
...
<g class="turns-number-group">
  {#if topValue !== 0}
    ...
    {#if topDetails}
      <g
        transform={`translate(${turnsPositions.top.x}, ${turnsPositions.top.y})`}
      >
        <image
  ...
  {#if bottomValue !== 0}
    ...
    {#if bottomDetails}
      <g
        transform={`translate(${turnsPositions.bottom.x}, ${turnsPositions.bottom.y})`}
      >
        <image
