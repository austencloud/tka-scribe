...
export class MotionOriCalculator {
  ...
  private calculateHalfTurnOrientation(): Orientation {
    ...
    return (
      orientationMap[`${motionType},${propRotDir},${startOri}`] || startOri
    );
  }

  private calculateFloatOrientation(
    motion: Motion,
    motionData: ShiftMotionInterface
  ): Orientation {
    ...
    if (handrotDir && !["cw_shift", "ccw_shift"].includes(handrotDir)) {
      throw new Error(
        "Invalid handpath direction while calculating float orientation"
      );
    }

    const orientationMap: Record<
      Orientation,
      Record<ShiftHandRotDir, Orientation>
    > = {
      in: {
        [CW_SHIFT]: CLOCK,
        [CCW_SHIFT]: COUNTER,
