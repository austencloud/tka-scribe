import type { HandRotDir } from "$legacyLib/types/Types";
...
export class HandpathCalculator {
  ...
  constructor() {
    ...
    this.handRotDirMap = {
      ...Object.fromEntries(
        clockwisePairs.map((pair) => [pair.join(","), "cw_shift"])
      ),
      ...Object.fromEntries(
        counterClockwisePairs.map((pair) => [pair.join(","), "ccw_shift"])
      ),
      ...Object.fromEntries(
        diagonalClockwise.map((pair) => [pair.join(","), "cw_shift"])
      ),
      ...Object.fromEntries(
        diagonalCounterClockwise.map((pair) => [pair.join(","), "ccw_shift"])
      ),
      ...Object.fromEntries(dashPairs.map((pair) => [pair.join(","), "dash"])),
      ...Object.fromEntries(
        staticPairs.map((pair) => [pair.join(","), "static"])
      ),
    };
  }
