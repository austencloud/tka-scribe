import { DIAMOND, BOX } from "$legacyLib/types/Constants";

const diamondLayer1Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/diamond/special/from_layer1/*_placements.json",
  { eager: true }
);
const diamondLayer2Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/diamond/special/from_layer2/*_placements.json",
  { eager: true }
);
const diamondLayer3Blue1Red2Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/diamond/special/from_layer3_blue1_red2/*_placements.json",
  { eager: true }
);
const diamondLayer3Blue2Red1Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/diamond/special/from_layer3_blue2_red1/*_placements.json",
  { eager: true }
);

const boxLayer1Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/box/special/from_layer1/*_placements.json",
  { eager: true }
);
const boxLayer2Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/box/special/from_layer2/*_placements.json",
  { eager: true }
);
const boxLayer3Blue1Red2Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/box/special/from_layer3_blue1_red2/*_placements.json",
  { eager: true }
);
const boxLayer3Blue2Red1Files = import.meta.glob(
  "$legacyLib/data/arrow_placement/box/special/from_layer3_blue2_red1/*_placements.json",
  { eager: true }
);

export class SpecialPlacementLoader {
  private static instance: SpecialPlacementLoader;
  private placementsByFolder: Record<
    string,
  ...
  public getAdjustmentForLetter(
    ...
    if (letter !== "Q") {
      const letterData =
        this.placementsByFolder[gridMode]?.[
          this.getLayerFolder(oriKey, turnsTuple)
        ]?.[letter];
      ...
      const matchedTuple = Object.keys(actualLetterData || {}).find(
        (key) =>
          key === turnsTuple ||
          key.replace(/\s/g, "") === turnsTuple.replace(/\s/g, "")
      );
  ...
  public loadOrReturnSpecialPlacements(): Record<
    string,
    Record<string, Record<string, any>>
  > {
