...
import { Letter } from "$legacyLib/types/Letter";
import { LetterType } from "$legacyLib/types/LetterType";
import type { Loc, Color } from "$legacyLib/types/Types";
import { LetterUtils } from "$legacyLib/utils/LetterUtils";
import type { PictographService } from "$legacyLib/components/Pictograph/PictographService";
...
export function calculateDashLocation(
  ...
  if (letterType === LetterType.Type3 && motion.turns === 0 && getShiftMotion) {
    const dashLocation = calculateDashLocationBasedOnShift(
      motion,
      getShiftMotion,
      getOtherMotion
    );
    ...
    console.warn(
      "Type 3 dash location calculation failed, falling back to default calculation"
    );
  }

  if (
    currentLetter &&
    [Letter.Φ_DASH, Letter.Ψ_DASH].includes(currentLetter) &&
    getOtherMotion
  ) {
    return calculatePhiDashPsiDashLocation(
      motion,
      getOtherMotion,
      currentLetter
    );
  ...
  if (
    ...
    return calculateLambdaZeroTurnsLocation(
      motion,
      getOtherMotion,
      currentLetter
    );
  ...
  if (motion.turns === 0) {
    return calculateDefaultZeroTurnsDashLocation(
      motion.startLoc,
      motion.endLoc
    );
  }
...
function calculateDefaultZeroTurnsDashLocation(
  startLoc: Loc,
  endLoc: Loc
): Loc | null {
...
function calculateDashLocationNonZeroTurns(motion: Motion): Loc | null {
  return (
    DASH_LOCATION_NONZERO_TURNS_MAP[motion.propRotDir]?.[motion.startLoc] ||
    null
  );
}
...
function calculateDashLocationBasedOnShift(
  ...
  if (!shiftMotion || !dashMotion) {
    console.warn(
      "calculateDashLocationBasedOnShift: Missing shift or dash motion"
    );
    return null;
  }

  // Get shift location from the shift motion's start and end locations
  const shiftLocation = calculateShiftLocation(
    shiftMotion.startLoc,
    shiftMotion.endLoc
  );
  ...
  if (gridMode === DIAMOND) {
    if (!DIAMOND_DASH_LOCATION_MAP[dashStartLoc]) {
      console.warn(
        `No DIAMOND_DASH_LOCATION_MAP entry for dashStartLoc: ${dashStartLoc}`
      );
    ...
    if (!result) {
      console.warn(
        `No DIAMOND_DASH_LOCATION_MAP result for ${dashStartLoc}[${shiftLocation}]`
      );
      return dashMotion.endLoc; // Fall back to end location
    }
    ...
    if (!BOX_DASH_LOCATION_MAP[dashStartLoc]) {
      console.warn(
        `No BOX_DASH_LOCATION_MAP entry for dashStartLoc: ${dashStartLoc}`
      );
      return dashMotion.endLoc; // Fall back to end location
    }
    ...
    if (!result) {
      console.warn(
        `No BOX_DASH_LOCATION_MAP result for ${dashStartLoc}[${shiftLocation}]`
      );
      return dashMotion.endLoc; // Fall back to end location
    }
