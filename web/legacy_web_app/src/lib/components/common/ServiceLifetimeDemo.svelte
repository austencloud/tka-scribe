  ...
  onMount(() => {
    ...
    errorHandler.log({
      ...
      context: {
        idGen1:
          idGen1 === idGen2
            ? "Same instance as idGen2"
            : "Different instance from idGen2",
      },
    });

    // Generate some IDs to show
    for (let i = 0; i < 3; i++) {
      ids1.push((idGen1 as any).next());
      ids2.push((idGen2 as any).next());
    }
  });

  function generateMoreIds() {
    if (idGen1 && idGen2) {
      ids1 = [...ids1, (idGen1 as any).next()];
      ids2 = [...ids2, (idGen2 as any).next()];
    }
  }
</script>
