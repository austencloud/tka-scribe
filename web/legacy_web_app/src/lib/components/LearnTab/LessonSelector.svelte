<script lang="ts">
  ...
  import { lessonConfigs } from "$legacyLib/state/stores/learn/lesson_configs";
  import {
    progressStore,
    badgeStatus,
  } from "$legacyLib/state/stores/learn/progressStore";
    ...
    <div class="stats-container" in:fly={{ y: 20, duration: 500, delay: 500 }}>
      ...
      <div class="stat-card">
        <div class="stat-value">
          {$progressStore.averageScore}<span class="percent">%</span>
        </div>
...
<style>
  ...
  .lesson-card:hover {
    transform: translateY(
      -3px
    ); /* Reduced from -5px to -3px for subtler effect */
    box-shadow:
  ...
  .lesson-card:hover .card-content {
    transform: translateY(
      -1.5rem
    ); /* Reduced from -2.5rem to -1.5rem for subtler effect */
  }
