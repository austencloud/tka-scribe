<script lang="ts">
  ...
  import SettingsContent from "$legacyLib/components/SettingsDialog/SettingsDialog.svelte";
  ...
  const isSettingsOpenStore = useSelector(
    appService,
    (state) => state.context.isSettingsOpen
  );
      ...
      try {
        ...
        if (storedData) {
          ...
          if (
            parsedData &&
            parsedData.context &&
            parsedData.context.isSettingsOpen === true
          ) {
            console.log(
              "Resetting persisted settings dialog state on page load"
            );
