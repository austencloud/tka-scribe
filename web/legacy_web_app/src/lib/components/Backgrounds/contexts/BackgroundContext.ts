...
function createBackgroundContext(): BackgroundContext {
  ...
  const backgroundSystem = derived(
    [backgroundType, qualityLevel],
    ([$type, $quality]) => {
...
export function getBackgroundContext(): BackgroundContext {
  ...
  if (runesContext) {
    ...
    const performanceMetrics = writable<PerformanceMetrics>(
      runesContext.performanceMetrics
    );
    ...
    const backgroundType = writable<BackgroundType>(
      runesContext.backgroundType
    );
  ...
  throw new Error(
    "No background context found. Make sure to use BackgroundProvider."
  );
}
