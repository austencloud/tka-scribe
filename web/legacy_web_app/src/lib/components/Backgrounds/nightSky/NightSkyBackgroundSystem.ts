...
// import { NightSkyConfig } from '../config/nightSky'; // Temporarily disabled
...
export class NightSkyBackgroundSystem implements BackgroundSystem {
  ...
  // config handles ---------------------------------------------------------
  private cfg: any = getOptimizedConfig(this.quality).config as any; // Temporarily using any instead of NightSkyConfig
  ...
  public setQuality(q: QualityLevel) {
    ...
    this.cfg = optimized.config as any;
  ...
  private initParallax(dim: Dimensions) {
    const mkLayer = (key: "far" | "mid" | "near"): ParallaxLayer => {
      ...
      const count = Math.floor(dim.width * dim.height * pCfg.density);
  ...
  private drawSpaceship(ctx: CanvasRenderingContext2D) {
    ...
    // Thruster
    ctx.fillStyle = this.a11y.highContrast ? "#FFFF00" : "#FFA500";
    ...
    if (!this.cometState.element) {
      ...
      if (this.cometState.timer >= this.cometState.interval) {
        ...
        this.cometState.element = {
          x: dir > 0 ? dim.width + cCfg.size * 5 : -cCfg.size * 5, // Start off-screen
