...
// Ensure layoutUtils imports are correct
  ...
  import sequenceDataService, {
    type SequenceBeat,
  } from "$legacyLib/services/SequenceDataService";
  ...
  import type { TKAPosition } from "$legacyLib/types/TKAPosition";
  import type {
    Color,
    Loc,
    MotionType,
...
// Import the loading state store
  import transitionLoading from "$legacyLib/state/stores/ui/transitionLoadingStore";
  ...
  function sequenceBeatToPictographData(beat: SequenceBeat): PictographData {
    ...
    const createMotionData = (attrs: any): MotionData | null => {
      ...
      return {
        ...
        motionType: isValidMotionType(attrs.motion_type)
          ? attrs.motion_type
          : "static",
        ...
        endOri: isValidOrientation(attrs.end_ori || attrs.start_ori)
          ...
          : "in",
        propRotDir: isValidPropRotDir(attrs.prop_rot_dir)
          ? attrs.prop_rot_dir
          : "no_rot",
  ...
  const layoutContextValue: Readable<LayoutContextValue> = derived(
    ...
    ([
      ...
      const { deviceType: enhancedDeviceType, foldableInfo } =
        getEnhancedDeviceType(
  ...
  setContext<Readable<LayoutContextValue>>(
    LAYOUT_CONTEXT_KEY,
    layoutContextValue
  );
  ...
  function handleViewChange(event: CustomEvent<ViewModeDetail>) {
    ...
    if (detail.mode === "all") {
      ...
      console.log(
        "Showing all options while maintaining sort method:",
        currentSortMethod
      );
      ...
      const lastSelectedForNewMethod =
        uiStateValue.lastSelectedTab[
          newSortMethod as keyof typeof uiStateValue.lastSelectedTab
        ];
  ...
  onMount(() => {
    ...
    if (optionPickerElement) {
      ...
      optionPickerElement.addEventListener(
        "optionPickerViewChange",
        (event) => {
          if (event instanceof CustomEvent) {
            console.log(
              "OptionPicker received optionPickerViewChange event:",
              event.detail
            );
            handleViewChange(event as CustomEvent<ViewModeDetail>);
          }
    ...
    const loadOptionsFromSequence = async () => {
      // Get current sequence data (actual beats, not including start position)
      const fullSequence = sequenceDataService.getCurrentSequence();

      // Get start position from localStorage (since it's no longer in sequence beats)
      let startPosition: PictographData | null = null;
      try {
        const startPosData = localStorage.getItem("start_position");
        if (startPosData) {
          startPosition = JSON.parse(startPosData);
        }
      } catch (error) {
        console.error("Error getting start position from localStorage:", error);
      }

      // Build the sequence for option filtering
      const sequenceForOptions: PictographData[] = [];

      // Add start position if it exists
      if (startPosition) {
        sequenceForOptions.push(startPosition);
      }

      // Add actual sequence beats (converted to PictographData)
      const sequenceBeats = fullSequence.map(beat => sequenceBeatToPictographData(beat));
      sequenceForOptions.push(...sequenceBeats);

      // Load options based on the complete sequence (start position + beats)
      actions.loadOptions(sequenceForOptions);
    };
    ...
    return () => {
      ...
      if (optionPickerElement) {
        ...
        optionPickerElement.removeEventListener(
          "optionPickerViewChange",
          (event) => {
            if (event instanceof CustomEvent) {
              handleViewChange(event as CustomEvent<ViewModeDetail>);
