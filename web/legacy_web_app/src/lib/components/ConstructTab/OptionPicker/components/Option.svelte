<script lang="ts">
  ...
  import StyledBorderOverlay from "$legacyLib/components/Pictograph/components/StyledBorderOverlay.svelte";
  ...
  const scaleFactor = $derived(() => {
    const factor = $layoutContext.layoutConfig.scaleFactor;
    // Safeguard against NaN or undefined values
    return typeof factor === "number" && !isNaN(factor) && factor > 0
      ? factor
      : 1.0;
  });
  ...
  const ariaLabel = $derived(
    `Select option ${props.pictographData.letter || "Unnamed"}`
  );
