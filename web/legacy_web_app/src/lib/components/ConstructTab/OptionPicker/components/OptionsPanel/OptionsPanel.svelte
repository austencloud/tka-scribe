<script lang="ts">
  ...
  $effect(() => {
    ...
    const currentSortedGroupKeys = getSortedGroupKeys(
    ...
    while (i < currentSortedGroupKeys.length) {
      ...
      if (isCurrentSmall) {
        const smallGroupSequence = [
          { key: currentKey, options: currentOptions },
        ];
        let j = i + 1;
        ...
        rows.push({
          type: "single",
          groups: [{ key: currentKey, options: currentOptions }],
        });
        i++;
      }
    }
