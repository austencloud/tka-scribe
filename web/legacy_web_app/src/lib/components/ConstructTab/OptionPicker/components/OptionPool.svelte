<script lang="ts">
  ...
  let previousOptionsMap = $state<
    Map<string, { data: PictographData; position: number }>
  >(new Map());
  ...
  {#each pooledOptions as item (item.id)}
    {#if item.visible && item.data}
      <div
        ...
        style:opacity={isTransitioning
          ? getTransitionProps(item).opacity?.from
          : 1}
        style:transform={isTransitioning
          ? getTransitionProps(item).transform?.from
          : "scale(1)"}
