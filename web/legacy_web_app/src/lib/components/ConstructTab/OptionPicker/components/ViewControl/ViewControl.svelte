<script lang="ts">
  ...
  let selectedViewOption = $state<ViewOption>(
    viewOptions.find(
      (opt) => opt.value === optionPickerContainer.state.sortMethod
    ) ||
  ...
  $effect(() => {
    ...
    // Otherwise, sync with the current sort method
      ...
      const matchingOption = viewOptions.find(
        (opt) => opt.value === currentSortMethod
      );
      if (matchingOption) {
    ...
    const handleUpdateViewControl = (event: Event) => {
      if (event instanceof CustomEvent) {
        ...
        if (detail.mode === "all") {
          ...
          const methodOption = viewOptions.find(
            (opt) => opt.value === detail.method
          );
          if (methodOption) {
            console.log(
              `Updating view control to "${methodOption.label}" from event`
            );
  ...
  function handleViewSelect(option: ViewOption) {
    ...
    if (buttonElement) {
      ...
      console.warn(
        "Button element not available, using document for event dispatch"
      );
  ...
  function handleKeydown(event: KeyboardEvent) {
    ...
    const currentIndex = viewOptions.findIndex(
      (opt) => opt.value === selectedViewOption.value
    );
