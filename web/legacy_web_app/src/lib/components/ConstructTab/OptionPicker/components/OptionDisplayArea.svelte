<script lang="ts">
  ...
  // Create a crossfade transition for smooth tab switching with NaN protection
  const [send, receive] = crossfade({
    duration: 350,
    easing: quintOut,
    // Completely disable the default crossfade transform calculations to prevent NaN
    fallback: () => ({
      duration: 350,
