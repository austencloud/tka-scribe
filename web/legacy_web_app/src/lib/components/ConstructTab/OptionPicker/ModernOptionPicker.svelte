<script lang="ts">
  ...
  import { optionPickerContainer } from "$legacyLib/state/stores/optionPicker/optionPickerContainer";
  ...
  function handleViewChange(event: CustomEvent<ViewModeDetail>) {
    ...
    if (detail.mode === "all") {
      ...
      const lastSelectedForNewMethod =
        optionPickerContainer.state.lastSelectedTab[newSortMethod];
  ...
  onMount(() => {
    ...
    const loadOptionsFromSequence = async () => {
      ...
      optionPickerContainer.loadOptions(sequenceForOptions);
    };
