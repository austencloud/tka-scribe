<script lang="ts">
  ...
  import LoadingSpinner from "$legacyLib/components/MainWidget/loading/LoadingSpinner.svelte";
  import Pictograph from "$legacyLib/components/Pictograph/Pictograph.svelte";
  import hapticFeedbackService from "$legacyLib/services/HapticFeedbackService";
  import startPositionService from "$legacyLib/services/StartPositionService";
  ...
  } from "$legacyLib/state/stores/ui/transitionLoadingStore";
  import pictographDataStore from "$legacyLib/stores/pictograph/pictographStore";
  import { selectedStartPos } from "$legacyLib/stores/sequence/selectionStore";
  import type { PictographData } from "$legacyLib/types/PictographData.js";
  ...
  onMount(() => {
    document.addEventListener(
      ...
      handleStartPosClick as EventListener
    );
  ...
  const handleSelect = async (startPosPictograph: PictographData) => {
    try {
      ...
      if (startPosCopy.blueMotionData) {
        ...
        startPosCopy.blueMotionData.endLoc =
          startPosCopy.blueMotionData.startLoc;
        startPosCopy.blueMotionData.endOri =
          startPosCopy.blueMotionData.startOri;
