...
const createBrowseTabStore = () => {
  ...
  return {
    ...
    loadInitialData: async () => {
      ...
      try {
        const { fetchSequences } = await import(
          "$legacyLib/services/sequenceService"
        );
    ...
    selectSequence: (sequenceId: string) => {
      update((state) => {
        // Find the sequence
        const sequence = state.allSequences.find(
    ...
    selectVariation: (index: number) => {
      update((state) => {
        // Find the sequence
        const sequence = state.allSequences.find(
          (seq) => seq.id === state.selectedSequenceId
        );
    ...
    toggleFavorite: async (sequenceId: string, variationId: string) => {
      ...
      const variationIndex = sequence.variations.findIndex(
        (v) => v.id === variationId
      );
      ...
      const isFavorite =
        sequence.variations[variationIndex].metadata.isFavorite || false;

      // Update optimistically
      update((state) => {
        const newSequences = [...state.allSequences];
        const sequenceIndex = newSequences.findIndex(
          (seq) => seq.id === sequenceId
        );
      ...
      // Call API
      try {
        const { updateFavoriteStatus } = await import(
          "$legacyLib/services/sequenceService"
        );
    ...
    deleteVariation: async (sequenceId: string, variationId: string) => {
      ...
      // Call API
      try {
        const { deleteVariationApi } = await import(
          "$legacyLib/services/sequenceService"
        );
    ...
    deleteSequence: async (sequenceId: string) => {
      ...
      // Update optimistically
      update((state) => {
        const newSequences = state.allSequences.filter(
          (seq) => seq.id !== sequenceId
        );
      ...
      // Call API
      try {
        const { deleteSequenceApi } = await import(
          "$legacyLib/services/sequenceService"
        );
    ...
    return allSequences.filter((sequence) => {
      switch (currentFilter.type) {
        ...
        case "tag":
          ...
          return (
            sequence.metadata.tags?.includes(currentFilter.value as string) ||
            false
          );
  ...
  ([$filteredSequences, $browseTabStore]) => {
    ...
    return sequences.sort((a, b) => {
      ...
      switch (currentSort.field) {
        ...
        case "dateAdded":
          const dateA = a.metadata.dateAdded
            ? new Date(a.metadata.dateAdded).getTime()
            : 0;
          const dateB = b.metadata.dateAdded
...
export const groupedSequences: Readable<
  { section: string; sequences: SequenceData[] }[]
> = derived(
...
export const selectedSequenceData: Readable<{
  ...
  const { allSequences, selectedSequenceId, selectedVariationIndex } =
    $browseTabStore;

  // Find selected sequence
  const sequence =
    allSequences.find((seq) => seq.id === selectedSequenceId) || null;
