...
import type {
    ReversalFilter,
    SortMethod,
} from "$legacyLib/components/ConstructTab/OptionPicker/config";
import {
    determineGroupKey,
    getNextOptions,
    getSortedGroupKeys,
    getSorter,
} from "$legacyLib/components/ConstructTab/OptionPicker/services/OptionsService";
import hapticFeedbackService from "$legacyLib/services/HapticFeedbackService";
import {
    createContainer,
    createDerived,
} from "$legacyLib/state/core/container";
import { createPersistentObjectState } from "$legacyLib/state/core/runes.svelte";
import {
    sequenceActions,
    sequenceSelectors,
} from "$legacyLib/state/machines/sequenceMachine";
...
import type { OptionPickerState } from "./types";
      ...
      return {
        loadOptions: (sequence: PictographData[]) => {
          ...
          try {
            ...
            const nextOptions = getNextOptions(allPictographs, sequence);
...
export const filteredOptions = createDerived(() => {
  ...
  options.sort(
    getSorter(optionPickerContainer.state.sortMethod)
  );
  return options;
});
