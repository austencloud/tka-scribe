import { createContainer } from "$legacyLib/state/core/container";
import { createDerived } from "$legacyLib/state/core/container";
  ...
  return createContainer(initialState, (state, update) => ({
    ...
    removeBeat: (beatId: string) => {
      update((state) => {
        ...
        state.selectedBeatIds = state.selectedBeatIds.filter(
          (id) => id !== beatId
        );
        state.isModified = true;
        state.metadata.lastModified = new Date();
    ...
    saveToLocalStorage: () => {
      ...
      try {
        import("$legacyLib/utils/pictographUtils")
          .then(({ createSafeBeatCopy }) => {
            try {
              ...
              console.error(
                "Error in saveToLocalStorage inner function:",
                innerError
              );
            }
          })
    ...
    loadFromLocalStorage: () => {
      ...
      try {
        ...
        if (parsed.beats && Array.isArray(parsed.beats)) {
          parsed.beats = parsed.beats.map((beat: any) => {
            ...
            return {
              id:
                beat.id ||
                `beat-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
              ...
              redPropData:
                beat.redPropData || beat.pictographData?.redPropData || null,
              bluePropData:
                beat.bluePropData || beat.pictographData?.bluePropData || null,
              redArrowData:
                beat.redArrowData || beat.pictographData?.redArrowData || null,
              blueArrowData:
                beat.blueArrowData ||
                beat.pictographData?.blueArrowData ||
                null,
              redMotionData:
                beat.redMotionData ||
                beat.pictographData?.redMotionData ||
                null,
              blueMotionData:
                beat.blueMotionData ||
                beat.pictographData?.blueMotionData ||
                null,
        ...
        try {
          Promise.all([
            import("$legacyLib/stores/sequence/selectionStore"),
            import("$legacyLib/state/stores/pictograph/pictographContainer"),
            ...
            if (savedStartPos) {
              try {
                ...
                console.error(
                  "Failed to parse start position from localStorage:",
                  parseError
                );
              }
            }
            ...
            if (startPosData) {
              if (startPosData.redMotionData) {
                startPosData.redMotionData.endLoc =
                  startPosData.redMotionData.startLoc;
              }
              if (startPosData.blueMotionData) {
                startPosData.blueMotionData.endLoc =
                  startPosData.blueMotionData.startLoc;
              }
...
export const currentBeat = createDerived(
  () =>
    sequenceContainer.state.beats[sequenceContainer.state.currentBeatIndex] ||
    null
);
  ...
  window.addEventListener("beforeunload", () => {
    if (saveTimeoutId) {
      ...
      if (
        ...
        sequenceContainer.state.isModified
      ) {
        sequenceContainer.saveToLocalStorage();
      }
    }
