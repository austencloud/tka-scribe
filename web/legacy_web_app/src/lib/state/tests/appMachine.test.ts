...
vi.mock("$legacyLib/utils/appInitializer", () => ({
...
describe("App State Machine", () => {
  ...
  describe("Initial State and Transitions", () => {
    it("starts in initializingBackground state", () => {
      ...
      expect(appActor.getSnapshot().context.loadingMessage).toBe(
        "Loading background..."
      );
    });
    ...
    it("transitions to initializationFailed on error", async () => {
      ...
      const initializerMock = await import("$legacyLib/utils/appInitializer");
      ...
      await waitFor(
        appActor,
        (state) => state.matches("initializationFailed"),
        { timeout: 10000 }
      );
  ...
  describe("Context Updates", () => {
    ...
    it("updates loading progress during initialization", async () => {
      ...
      if (freshActor.getSnapshot().matches("ready")) {
        ...
        await waitFor(
          freshActor,
          (state) =>
            state.matches("ready") || state.context.loadingProgress > 0,
          { timeout: 5000 }
        );
    ...
    it("updates tab when changing tabs", async () => {
      ...
      await waitFor(appActor, (state) => state.context.currentTab === 2, {
        timeout: 5000,
      });
      ...
      // Wait for the state to update
      await waitFor(appActor, (state) => state.context.currentTab === 1, {
        timeout: 5000,
      });
    ...
    it("toggles fullscreen mode", async () => {
      ...
      // Wait for the state to update
      await waitFor(appActor, (state) => state.context.isFullScreen === true, {
        timeout: 5000,
      });
      ...
      // Wait for the state to update
      await waitFor(appActor, (state) => state.context.isFullScreen === false, {
        timeout: 5000,
      });
    ...
    it("toggles settings panel", async () => {
      ...
      await waitFor(
        appActor,
        (state) => state.context.isSettingsOpen === true,
        { timeout: 5000 }
      );
      ...
      await waitFor(
        appActor,
        (state) => state.context.isSettingsOpen === false,
        { timeout: 5000 }
      );
    ...
    it("updates background type", async () => {
      ...
      await waitFor(
        appActor,
        (state) => state.context.background === "snowfall",
        {
          timeout: 5000,
        }
      );
