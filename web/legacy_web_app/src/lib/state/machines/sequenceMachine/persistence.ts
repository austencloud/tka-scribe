...
import { pictographStore } from "$legacyLib/state/stores/pictograph/pictograph.store";
...
export function initializePersistence(sequenceActor: Actor<any>) {
  ...
  import("$legacyLib/state/stores/sequence/SequenceContainer").then(
    ({ sequenceContainer }) => {
          ...
          if (hasBeats) {
            ...
            if (firstBeat.pictographData) {
              ...
              pictographData = {
                ...
                startPos:
                  firstBeat.position || firstBeat.metadata?.startPos || null,
      ...
      if (!sequenceLoaded) {
        try {
          ...
          if (backupData) {
            ...
            if (
              backup.beats &&
              Array.isArray(backup.beats) &&
              backup.beats.length > 0
            ) {
              ...
              const processedBeats = backup.beats.map((beat: any) => {
                ...
                if (!processedBeat.pictographData && processedBeat.metadata) {
                  ...
                  processedBeat.pictographData = {
                    letter:
                      processedBeat.letter ||
                      processedBeat.metadata.letter ||
                      null,
                    startPos:
                      processedBeat.position ||
                      processedBeat.metadata.startPos ||
                      null,
              ...
              if (firstBeat) {
                ...
                if (firstBeat.pictographData) {
                  ...
                  pictographData = {
                    letter:
                      firstBeat.letter || firstBeat.metadata?.letter || null,
                    startPos:
                      firstBeat.position ||
                      firstBeat.metadata?.startPos ||
                      null,
                    endPos: firstBeat.metadata?.endPos || null,
                    gridMode: firstBeat.metadata?.gridMode || "diamond",
                    redPropData: firstBeat.redPropData || null,
  ...
  sequenceActor.subscribe((state) => {
    ...
    Promise.all([
      import("$legacyLib/state/stores/sequence/SequenceContainer"),
      ...
      try {
        ...
        console.log(
          "Saved sequence to both storage mechanisms with beats:",
          safeBeats.length
        );
      } catch (error) {
