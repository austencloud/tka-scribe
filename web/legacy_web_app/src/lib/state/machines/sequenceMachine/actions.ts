  ...
  try {
    ...
    if (currentStartPos) {
      ...
      if (currentStartPos.redMotionData) {
        ...
        currentStartPos.redMotionData.endLoc =
          currentStartPos.redMotionData.startLoc;
        currentStartPos.redMotionData.endOri =
          currentStartPos.redMotionData.startOri;
        currentStartPos.redMotionData.turns = 0;
      }

      if (currentStartPos.blueMotionData) {
        currentStartPos.blueMotionData.motionType = "static";
        currentStartPos.blueMotionData.endLoc =
          currentStartPos.blueMotionData.startLoc;
        currentStartPos.blueMotionData.endOri =
          currentStartPos.blueMotionData.startOri;
...
export function removeBeatAndFollowing({ event }: { event: any }) {
  const removeEvent = event as {
    type: "REMOVE_BEAT_AND_FOLLOWING";
    beatId: string;
  };
  ...
  if (beatIndex >= 0) {
    ...
    const beatsToRemove = sequenceContainer.state.beats
      .slice(beatIndex)
      .map((beat) => beat.id);
      ...
      if (currentStartPos) {
        ...
        try {
          localStorage.setItem(
            "start_position",
            JSON.stringify(currentStartPos)
          );
          console.log(
            "Saved start position to localStorage after beat removal"
          );
        } catch (error) {
          console.error(
            "Failed to save start position to localStorage:",
            error
          );
        }
