 ...
 */
export const generateSequenceActor = fromCallback<
    ...
    try {
      ...
      if (input.generationType === "circular") {
        ...
        const { createCircularSequence } = await import(
          "$legacyLib/components/GenerateTab/circular/createCircularSequence"
        );
        ...
        const { createFreeformSequence } = await import(
          "$legacyLib/components/GenerateTab/Freeform/createFreeformSequence"
        );
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Unknown error during sequence generation";
