...
import { createAppMachine } from "$legacyLib/state/core";
import { initializeApplication } from "$legacyLib/utils/appInitializer";
import type { BackgroundType } from "$legacyLib/components/MainWidget/state/appState";
...
} from "$legacyLib/utils/preferences";
          ...
          ({ self, context }) => {
            if (browser) {
              try {
                ...
                console.log(
                  "Checking saved background in ready state:",
                  savedBackground
                );
                ...
                console.error(
                  "Error enforcing background on ready state:",
                  error
                );
              }
            }
            ...
            if (browser) {
              try {
                ...
                if (savedTab !== context.currentTab) {
                  console.log(
                    "Enforcing tab from localStorage in ready state:",
                    savedTab
                  );
                  // Use a small timeout to ensure this happens after other initialization
                  setTimeout(() => {
            ...
            actions: [
              ...
              ({ event }) => {
                ...
                if (browser) {
                  try {
                    console.log(
                      "Saving tab to localStorage immediately:",
                      event.tab
                    );
            ...
            actions: [
              assign({
                background: ({ event, context }) => {
                  ...
                  return validBackgrounds.includes(
        ...
        try {
          ...
          if (storedData) {
            ...
            if (
              parsedData &&
              parsedData.context &&
              "isSettingsOpen" in parsedData.context
            ) {
              console.log(
                "Ensuring settings dialog is closed in persisted state"
              );
          ...
          console.error(
            "Error modifying persisted settings state:",
            storageError
          );
        }
      } catch (error) {
