...
function loadUIState() {
  ...
  try {
    ...
    return {
      ...
      error: null as string | null,
      lastSelectedTab:
        parsed.lastSelectedTab || ({ type: "all" } as LastSelectedTabState),
    };
  } catch (e) {
...
const lastSelectedTabStore = writable<LastSelectedTabState>(
  initialState.lastSelectedTab
);
...
export const filteredOptions = derived(
  ...
  ([$options, $sortMethod, $sequence]) => {
    ...
    filteredOpts.sort(getSorter($sortMethod));
    return filteredOpts;
  }
);
...
export const actions = {
  loadOptions: (newSequence: PictographData[]) => {
    ...
    try {
      ...
      const nextOptions = getNextOptions(allPictographs, newSequence);
      ...
      isLoadingStore.set(false);
      errorStore.set(
        err instanceof Error ? err.message : "Unknown error loading options"
      );
      optionsStore.set([]);
    }
  },
