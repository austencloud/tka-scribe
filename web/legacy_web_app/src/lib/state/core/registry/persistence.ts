...
export function loadPersistedState(persistenceKey: string): {
  ...
  try {
    ...
    console.error(
      "Failed to load or parse persisted state from localStorage:",
      error
    );
      ...
      try {
        ...
        if (corruptedData) {
          localStorage.setItem(
            `${persistenceKey}_corrupted_backup`,
            corruptedData
          );
...
export function performPersist(
  ...
  Array.from(containers.values()).forEach((container) => {
    ...
    try {
      ...
      if (
        ...
        if (actorInstance.getSnapshot().status !== "stopped") {
          ...
          if (persistedSnapshot !== undefined) {
            ...
            if (
              !lastPersistedValue ||
              JSON.stringify(lastPersistedValue) !==
      ...
      console.error(
        `Failed to get state for persistence for ${container.id}:`,
        error
      );
    }
  });
...
export function handleStorageQuotaError(persistenceKey: string): void {
  try {
    ...
    const allPersistedState = JSON.parse(
    ...
    console.warn(
      "Storage quota exceeded - reduced persisted state to critical data only"
    );
  } catch (error) {
