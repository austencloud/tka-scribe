  ...
  if (initialSnapshot) {
    ...
    try {
      ...
      if (snapshot && typeof snapshot === "object") {
        ...
        if ("nextEvents" in snapshot)
          initialState.nextEvents = snapshot.nextEvents;
      }
    } catch (error) {
  ...
  return createContainer(initialState, (state, update) => {
    ...
    return {
      ...
      can: (eventType: string) =>
        (actor.getSnapshot() as any).can({ type: eventType } as any),
      matches: (stateValue: string) =>
        (actor.getSnapshot() as any).matches(stateValue),
