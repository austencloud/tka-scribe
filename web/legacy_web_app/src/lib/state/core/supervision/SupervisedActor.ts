  ...
  implements ISupervisedActor<TMachine>
{
  ...
  constructor(
    ...
    const actor = createActor(machine, actorOptions) as ReturnType<
      typeof createActor<TMachine>
    >;
    actor.start();
  ...
  get strategy(): SupervisionStrategy {
    ...
    return (
      opts?.strategy ??
      opts?.supervisor?.defaultStrategy ??
      new RestartStrategy()
    );
  }
  ...
  getPersistedSnapshot(): SnapshotFrom<TMachine> | undefined {
    return this.actor.getPersistedSnapshot?.() as
