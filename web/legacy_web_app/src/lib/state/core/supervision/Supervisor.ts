...
export class Supervisor implements ISupervisor {
  ...
  registerActor(actor: SupervisedActor<any>): void {
    ...
    if (this.debug) {
      debug(
        "supervisor",
        `Registered actor ${actor.id} with supervisor ${this.id}`
      );
    }
  }
  ...
  unregisterActor(actorId: string): void {
    if (this.actors.has(actorId)) {
      ...
      if (this.debug) {
        debug(
          "supervisor",
          `Unregistered actor ${actorId} from supervisor ${this.id}`
        );
      }
    } else {
      warn(
  ...
  async handleActorError(
    ...
    if (this.onError) {
      try {
        ...
        error(
          "supervisor",
          `Error in supervisor ${this.id} error handler`,
          handlerError
        );
      }
    }
    ...
    try {
      ...
      error(
        "supervisor",
        `Error applying strategy for actor ${actor.id}`,
        strategyError
      );
  ...
  async escalateError(
    ...
    if (this.parent) {
      ...
      error(
        "supervisor",
        `Cannot escalate error: no parent supervisor for ${this.id}`
      );
