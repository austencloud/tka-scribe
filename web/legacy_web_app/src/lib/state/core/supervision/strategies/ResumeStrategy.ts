...
export class ResumeStrategy implements SupervisionStrategy {
  ...
  async handleError(
    ...
    if (
      this.config.maxErrors !== undefined &&
      this.config.withinTimeWindow !== undefined
    ) {
      if (
        this.lastErrorTime &&
        now - this.lastErrorTime > this.config.withinTimeWindow
      ) {
        this.errorCount = 1;
      }

      if (
        this.errorCount > this.config.maxErrors &&
        this.config.fallbackStrategy
      ) {
        return this.config.fallbackStrategy.handleError(
          supervisor,
          actor,
          error
        );
