...
import {
  ...
  copyToClipboard,
} from "$legacyLib/components/SequenceWorkbench/share/utils/ShareUtils";
...
// Mock window.URL
const mockURL = vi.fn();
mockURL.prototype.toString = vi
  .fn()
  .mockReturnValue("https://test.com/?seq=test");
...
vi.mock("$legacyLib/components/shared/ToastManager.svelte", () => ({
  showSuccess: vi.fn(),
  showError: vi.fn(),
}));

// Mock logger
vi.mock("$legacyLib/core/logging", () => ({
  logger: {
    info: vi.fn(),
    warn: vi.fn(),
    error: vi.fn(),
...
describe("shareUtils", () => {
  ...
  describe("URL encoding/decoding", () => {
    it("should encode and decode a sequence correctly", () => {
      ...
      expect(mockResult.originalBeats.length).toBe(
        mockResult.decodedBeats.length
      );
    ...
    it("should generate a shareable URL", () => {
      ...
      expect(mockURL.prototype.searchParams.set).toHaveBeenCalledWith(
        "seq",
        expect.any(String)
      );
    });
