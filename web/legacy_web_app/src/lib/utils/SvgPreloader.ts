...
import { PropType } from "$legacyLib/types/Types";
...
export default class SvgPreloader {
  ...
  private async getSvgManager() {
    if (!this.svgManager) {
      ...
      if (typeof window !== "undefined") {
        ...
        const { default: SvgManager } = await import(
          "../components/SvgManager/SvgManager"
        );
  ...
  async preloadArrowSvg(
    ...
    return this.getOrFetchSvg(key, () =>
      manager.getArrowSvg(motionType, startOri, turns, color)
    );
  }
  ...
  async preloadCommonArrows(): Promise<void> {
    ...
    const commonCombinations = [
      ...
      {
        motionType: "pro" as MotionType,
        startOri: "in" as Orientation,
        turns: 0 as TKATurns,
      },
      {
        motionType: "pro" as MotionType,
        startOri: "out" as Orientation,
        turns: 0 as TKATurns,
      },
      {
        motionType: "pro" as MotionType,
  ...
  async preloadCommonSvgs(): Promise<void> {
    try {
      await Promise.all([
        this.preloadCommonProps(),
        this.preloadCommonArrows(),
      ]);
    } catch (error) {
