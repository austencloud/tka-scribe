...
// import { loggingConfig } from '$legacyLib/config/logging';
...
const DEFAULT_MACHINE_LOGGER_OPTIONS: MachineLoggerOptions = {
  ...
  logTransitions: true, // loggingConfig.machineLogging.logTransitions,
  performanceTracking: {
    enabled: true, // loggingConfig.machineLogging.performanceTracking.enabled,
    transitionThreshold: 100, // loggingConfig.machineLogging.performanceTracking.transitionThreshold
  },
};
...
function mergeOptions(
  partial: Partial<MachineLoggerOptions>
): MachineLoggerOptions {
  ...
  return {
    ...
    performanceTracking: {
      ...
      transitionThreshold:
        partial.performanceTracking?.transitionThreshold ??
        defaultPerf.transitionThreshold,
    },
  };
}
  ...
  return (inspectionEvent: InspectionEvent) => {
    ...
    switch (inspectionEvent.type) {
      ...
      case "@xstate.actor": {
        ...
        machineLogger.debug("Actor event", {
          data: { actorRef: actorEvt.actorRef },
        });
        break;
      }
    }
...
function handleSnapshotInspection(
  ...
  if (options.logTransitions) {
    ...
    const slow =
      duration !== undefined &&
      duration > (options.performanceTracking?.transitionThreshold ?? 50);
...
export function createLoggedActor<T extends AnyStateMachine>(
  ...
  const stored = (machine as any).__loggingOptions as
    | Partial<MachineLoggerOptions>
    | undefined;
