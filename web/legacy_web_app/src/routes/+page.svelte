<script lang="ts">
  import { onMount } from "$legacyLib/utils/svelte-lifecycle";
  import { fade } from "svelte/transition";

  // Components
  import FullScreen from "$legacyLib/AppFullScreen.svelte";
  import MainLayout from "$legacyLib/components/MainWidget/layout/MainLayout.svelte";
  import LoadingOverlay from "$legacyLib/components/MainWidget/loading/LoadingOverlay.svelte";
  import BackgroundCanvas from "$legacyLib/components/Backgrounds/BackgroundCanvas.svelte";
  import BackgroundProvider from "$legacyLib/components/Backgrounds/BackgroundProvider.svelte";
  import FirstTimeSetupDialog from "$legacyLib/components/FirstTimeSetup/FirstTimeSetupDialog.svelte";
  import FirstTimeSetupButton from "$legacyLib/components/FirstTimeSetup/FirstTimeSetupButton.svelte";

  // State Management
  import { appActions } from "$legacyLib/state/machines/app/app.actions";
  import { useSelector } from "@xstate/svelte";
  import { appService } from "$legacyLib/state/machines/app/app.machine";
  import { uiStore } from "$legacyLib/state/stores/uiStore";
  import type { BackgroundType } from "$legacyLib/components/Backgrounds/types/types";
  import hapticFeedbackService from "$legacyLib/services/HapticFeedbackService";
  ...
  const windowHeight = $derived(
    $uiStore ? $uiStore.windowHeight + "px" : "100vh"
  );
  ...
  const hasFailedStore = useSelector(appService, (state) =>
    state.matches("initializationFailed")
  );
  const hasFailed = $derived($hasFailedStore);

  const isReadyStore = useSelector(appService, (state) =>
    state.matches("ready")
  );
  const isReady = $derived($isReadyStore);

  const currentBackgroundStore = useSelector(
    appService,
    (state) => state.context.background
  );
  ...
  const initializationErrorMsg = $derived(
    $initializationErrorMsgStore as string
  );

  const loadingProgressStore = useSelector(
    appService,
    (state) => state.context.loadingProgress
  );
  const loadingProgress = $derived($loadingProgressStore as number);

  const loadingMessageStore = useSelector(
    appService,
    (state) => state.context.loadingMessage
  );
...
<div id="main-widget" style="height: {windowHeight}" class="main-widget">
  <FullScreen on:toggleFullscreen={handleFullScreenToggle}>
    <div
      class="background"
      class:blur-background={isInitializingApp || hasFailed}
    >
      <BackgroundProvider
