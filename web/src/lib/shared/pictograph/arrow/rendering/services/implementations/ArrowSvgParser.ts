...
export class ArrowSvgParser implements IArrowSvgParser {
  ...
  parseArrowSvg(svgText: string): SVGDimensions {
    ...
    try {
      ...
      if (centerElement) {
        const rawCenterX = parseFloat(centerElement.getAttribute("cx") || "0") || center.x;
        const rawCenterY = parseFloat(centerElement.getAttribute("cy") || "0") || center.y;
        
        if (isDashArrow) {
          // Scale the center point too if we scaled the viewBox
          const targetSize = 250;
          const originalSize = Math.max(parseFloat(viewBoxValues[2] || "100"), parseFloat(viewBoxValues[3] || "100"));
