...
export class ErrorHandlingService implements IErrorHandlingService {
  ...
  reportCriticalError(error: Error, context?: string): void {
    ...
    console.error(`🔥 CRITICAL ERROR ${context ? `in ${context}` : ''}: ${error.message}`, error);
  ...
  getErrorStats() {
    return {
      ...
      lastError: this.errors.length > 0 ? this.errors[this.errors.length - 1].timestamp : undefined
    };
  }
