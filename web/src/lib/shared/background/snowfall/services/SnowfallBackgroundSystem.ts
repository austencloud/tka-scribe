...
export class SnowfallBackgroundSystem implements IBackgroundSystem {
  ...
  public update(dimensions: Dimensions): void {
    ...
    if (this.isInitialized) {
      ...
      const { qualitySettings } = this.configurationService.getOptimizedConfig(this.quality);
  ...
  public draw(ctx: CanvasRenderingContext2D, dimensions: Dimensions): void {
    const { config, qualitySettings } = this.configurationService.getOptimizedConfig(this.quality);
