...
export class SpaceshipSystem {
  ...
  constructor(config: SpaceshipConfig) {
    this.config = config;
    this.calculationService = container.get<INightSkyCalculationService>(BackgroundTypes.INightSkyCalculationService);
    this.spaceshipState = {
      element: null,
      timer: 0,
  ...
  update(dim: Dimensions, a11y: AccessibilitySettings, quality: QualityLevel) {
    ...
    if (!this.spaceshipState.element) {
      ...
      if (this.spaceshipState.timer >= this.spaceshipState.interval) {
        ...
        this.spaceshipState.interval = this.calculationService.randInt(15000, 30000);
      }
    } else {
