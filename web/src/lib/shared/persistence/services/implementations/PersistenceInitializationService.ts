...
export class PersistenceInitializationService implements IPersistenceInitializationService {
  ...
  async initialize(): Promise<void> {
    try {
      console.log('🔄 PersistenceInitializationService: Starting initialization...');
      ...
      console.log('✅ PersistenceInitializationService: Initialization complete');
    } catch (error) {
      this.initializationError = error instanceof Error ? error.message : String(error);
      console.error('❌ PersistenceInitializationService: Initialization failed:', error);
      throw error;
    }
  }
