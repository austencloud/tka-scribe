...
import type { FilterHistoryEntry, IFilterPersistenceService, SimpleBrowseState } from "../contracts/IFilterPersistenceService";
import { safeSessionStorageGet, safeSessionStorageRemove, safeSessionStorageSet } from "../../..";
...
export class FilterPersistenceService implements IFilterPersistenceService {
  ...
  async saveFilterToHistory(filter: FilterHistoryEntry): Promise<void> {
    try {
      ...
      const newHistory = [filter, ...filteredHistory].slice(0, this.MAX_HISTORY_SIZE);
