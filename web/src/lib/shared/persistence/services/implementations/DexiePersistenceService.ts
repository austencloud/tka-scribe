...
export class DexiePersistenceService implements IPersistenceService {
  ...
  async saveCurrentSequenceState(state: {
    ...
    try {
      ...
      localStorage.setItem(this.CURRENT_SEQUENCE_STATE_KEY, JSON.stringify(stateData));
      console.log("✅ DexiePersistenceService: Current sequence state saved");
    } catch (error) {
  ...
  async loadCurrentSequenceState(): Promise<{
    ...
    try {
      ...
      if (stateData.timestamp && (Date.now() - stateData.timestamp) > maxAge) {
        console.log("🧹 DexiePersistenceService: Clearing old sequence state");
        await this.clearCurrentSequenceState();
        return null;
      }

      console.log("✅ DexiePersistenceService: Current sequence state loaded");
      return {
  ...
  async clearCurrentSequenceState(): Promise<void> {
    try {
      ...
      console.log("✅ DexiePersistenceService: Current sequence state cleared");
    } catch (error) {
