...
export class BeatRenderingService implements IBeatRenderingService {
  ...
  async renderBeatToCanvas(
    ...
    try {
      ...
      // Handle empty/error beats with simple fallback
      if (beatData.isBlank || !beatData.pictographData) {
        ...
        ctx.fillText("Empty Beat", size / 2, size / 2);
        return canvas;
      }
      ...
      if (!svgString) {
        ...
        ctx.font = "12px Arial";
        ctx.textAlign = "center";
        ctx.fillText("SVG Error", size / 2, size / 2);
        return canvas;
      }
  ...
  async renderStartPositionToCanvas(
    ...
    try {
      // Create simple start position canvas
      const canvas = this.canvasManager.createCanvas(100, 100);
      const ctx = canvas.getContext("2d");
      if (!ctx) {
        throw new Error("Failed to get 2D context");
      }

      // Draw simple start position indicator
      ctx.fillStyle = "white";
