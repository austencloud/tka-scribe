...
export class SequenceRenderService implements ISequenceRenderService {
  ...
  async renderSequenceToCanvas(
    ...
    try {
      ...
      if (!validation.valid) {
        throw new Error(`Render validation failed: ${validation.errors.join(', ')}`);
      }
  ...
  async renderSequenceToBlob(
    ...
    try {
      ...
      const blob = await this.formatService.canvasToBlob(
        canvas,
        {
  ...
  async generatePreview(
    ...
    try {
      // Create preview options (smaller scale)
      ...
      // Render to canvas
      const canvas = await this.renderSequenceToCanvas(sequence, previewOptions);
  ...
  validateRender(
    ...
    if (!sequence) {
      ...
      if (!sequence.word || sequence.word.trim() === '') {
   ...
   */
  async batchRender(sequences: SequenceData[], options: SequenceExportOptions): Promise<HTMLCanvasElement[]> {
