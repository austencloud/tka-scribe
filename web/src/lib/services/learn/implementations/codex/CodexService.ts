...
export type { ICodexService } from "$services";
...
export class CodexService implements ICodexService {
  ...
  async getAvailableLessonTypes(): Promise<string[]> {
    ...
    return await this.lessonRepository.getAllLessonTypes();
  }

  /**
   * Get letters by category
   */
  async getLettersByCategory(category: LetterCategory): Promise<string[]> {
    this.ensureInitialized();

    return (await this.letterMappingRepository.getLettersByCategory(
      category.toString()
    )) as string[];
  }
