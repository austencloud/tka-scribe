...
export class SpotlightState {
  ...
  #displayState = $state<SpotlightDisplayState>(createDefaultSpotlightDisplayState());
  #imageState = $state<SpotlightImageState>(createDefaultImageState());
  #navigationState = $state<SpotlightNavigationState>(
    createDefaultNavigationState()
  );
  ...
  get isVisible() {
    return this.#displayState.isVisible;
  }
  get isClosing() {
    return this.#displayState.isClosing;
  }
  get isContentVisible() {
    return this.#displayState.isContentVisible;
  }
  get currentVariationIndex() {
    return this.#displayState.currentVariationIndex;
  }

  get isImageLoading() {
    return this.#imageState.isLoading;
  }
  get imageError() {
    return this.#imageState.hasError;
  }

  get hasMultipleVariations() {
    return this.#navigationState.hasMultipleVariations;
  }
  get canGoPrev() {
  ...
  get totalVariations() {
    return this.#navigationState.totalVariations;
  }
  ...
  get currentVariation() {
    if (
      !this.#currentSequence?.thumbnails ||
      this.#currentSequence.thumbnails.length === 0
    ) {
      return null;
    }
    return (
  ...
  private getThumbnailUrl(thumbnailPath: string): string {
    ...
    try {
      ...
      return this.#thumbnailService.getThumbnailUrl(
