...
<script lang="ts">
  ...
  function handleSectionClick(section: GalleryNavigationConfig) {
    onSectionToggle(section.id);
  }

  // Handle navigation item click
  function handleItemClick(
    section: GalleryNavigationConfig,
  ...
  {#if !isCollapsed}
    ...
    {#if sections.length > 0}
      {@const totalSequences = sections.reduce(
        (sum: number, section: GalleryNavigationConfig) =>
          sum + section.totalCount,
        0
      )}
      {@const expandedCount = sections.filter(
        (s: GalleryNavigationConfig) => s.isExpanded
      ).length}
