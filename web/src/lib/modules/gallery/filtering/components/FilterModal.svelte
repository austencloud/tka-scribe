...
Filter modal component for the gallery module.
...
<script lang="ts">
  ...
  const {
    ...
    isOpen?: boolean;
    currentFilter?: { type: string; value: GalleryFilterValue };
    availableSequenceLengths?: number[];
    onFilterChange?: (type: string, value?: GalleryFilterValue) => void;
    onClose?: () => void;
  }>();
  ...
  const sequenceLengths = $derived(() => availableSequenceLengths.sort((a: number, b: number) => a - b));
  ...
  // Apply a filter
  function applyFilter(type: string, value?: GalleryFilterValue) {
  ...
  >
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <button class="close-button" onclick={handleClose} aria-label="Close filter modal">
          ×
        </button>
      </div>
      ...
      <div class="filter-sections">
        {#each filterSections as section}
          <div class="filter-section">
            ...
            {#if section.id === "all"}
              <button 
                class="filter-button {currentFilter.type === 'all' ? 'active' : ''}"
            ...
            {:else if section.id === "favorites"}
              <button 
                class="filter-button {currentFilter.type === 'favorites' ? 'active' : ''}"
            ...
            {:else if section.id === "difficulty"}
              <div class="difficulty-buttons">
                {#each difficultyLevels as level}
                  <button 
                    class="filter-button {currentFilter.type === 'difficulty' && currentFilter.value === level ? 'active' : ''}"
            ...
            {:else if section.id === "startingPosition"}
              <div class="position-buttons">
                {#each startingPositions as position}
                  <button 
                    class="filter-button {currentFilter.type === 'startingPosition' && currentFilter.value === position ? 'active' : ''}"
            ...
            {:else if section.id === "startingLetter"}
              <div class="letter-grid">
                {#each letters as letter}
                  <button 
                    class="letter-button {currentFilter.type === 'startingLetter' && currentFilter.value === letter ? 'active' : ''}"
            ...
            {:else if section.id === "containsLetters"}
              <div class="letter-grid">
                {#each letters as letter}
                  <button 
                    class="letter-button {selectedLetters.includes(letter) ? 'active' : ''}"
            ...
            {:else if section.id === "length"}
              <div class="length-buttons">
                {#each sequenceLengths() as length}
                  <button
                    class="filter-button {currentFilter.type === 'length' && currentFilter.value === length ? 'active' : ''}"
            ...
            {:else if section.id === "gridMode"}
              <div class="grid-mode-buttons">
                {#each gridModes as mode}
                  <button
                    class="filter-button {currentFilter.type === 'gridMode' && currentFilter.value === mode ? 'active' : ''}"
