...
<script lang="ts">
  ...
  let activeSubTab = $state<"codex" | "quiz" | "read">("codex");

  // Load persisted sub-tab on mount
  onMount(() => {
    const savedSubTab = localStorage.getItem("tka-learn-active-subtab");
    if (savedSubTab && ["codex", "quiz", "read"].includes(savedSubTab)) {
      activeSubTab = savedSubTab as "codex" | "quiz" | "read";
    }
  });
  ...
  // Handle sub-tab switching with persistence
  function switchSubTab(tabId: "codex" | "quiz" | "read") {
    console.log(`🔄 Learn: Switching to ${tabId} sub-tab`);
    activeSubTab = tabId;
    localStorage.setItem("tka-learn-active-subtab", tabId);
  }
