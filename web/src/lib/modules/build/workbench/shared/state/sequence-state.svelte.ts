...
import type { ISequenceStateService } from "../services/contracts";
...
export function createSequenceState(
  ...
  return {
    ...
    getCurrentSequenceData: () => {
      ...
      if (sequence) {
        console.log("🔍 SequenceState: getCurrentSequenceData - sequence found:", sequence);

        const beats = sequence.beats || [];
        console.log("🔍 SequenceState: getCurrentSequenceData - sequence.beats:", beats);
        ...
        console.log("🔍 SequenceState: getCurrentSequenceData - startingPositionBeat:", sequence.startingPositionBeat);
        console.log("🔍 SequenceState: getCurrentSequenceData - startPosition:", sequence.startPosition);
        console.log("🔍 SequenceState: getCurrentSequenceData - startPosition (combined):", startPosition);
        ...
        if (beats.length > 0) {
          ...
          console.log("🔍 SequenceState: getCurrentSequenceData - using beats, result:", result);
        }
        // If no beats but we have a start position, include it
          ...
          console.log("🔍 SequenceState: getCurrentSequenceData - using start position, result:", result);
        }

        console.log("🔍 SequenceState: getCurrentSequenceData - returning:", result);
        return result;
      }
      console.log("🔍 SequenceState: getCurrentSequenceData - no sequence, returning empty array");
      return [];
    },
    getSequences,
    getIsLoading,
