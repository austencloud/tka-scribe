<script lang="ts">
  import { createAnimatorState, type AnimatorState } from "$state";
  ...
  $: {
    ...
    console.log(
      "🎯 AnimatorTab reactive - container:",
      container ? "available" : "null"
    );
    if (container && !state) {
      try {
        console.log("🎯 AnimatorTab attempting to create state...");
        state = createAnimatorState();
        console.log("🎯 AnimatorTab state initialized successfully!");
      } catch (error) {
        console.error("🎯 AnimatorTab failed to initialize state:", error);
        initializationError =
          error instanceof Error ? error.message : "Unknown error";
      }
    } else if (!container) {
      console.log("🎯 AnimatorTab waiting for container...");
    }
  }

  console.log("🎯 AnimatorTab rendered!");
</script>

<div class="animator-tab">
