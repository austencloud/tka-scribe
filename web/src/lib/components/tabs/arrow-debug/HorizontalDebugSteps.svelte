<script lang="ts">
	...
	let { debugData, currentStep, maxSteps, stepByStepMode, onStepChange }: Props = $props();
	...
	function getStepStatus(stepIndex: number): 'complete' | 'current' | 'pending' {
		if (stepByStepMode) {
			...
			case 1: // Location Calculation
				return {
					...
					data: {
						...
						method: debugData.locationDebugInfo?.calculationMethod || 'Standard',
				...
				return {
					...
					data: {
						initialPosition: debugData.initialPosition ?
							`(${debugData.initialPosition.x.toFixed(1)}, ${debugData.initialPosition.y.toFixed(1)})` : 'Not calculated',
						coordinateSet: debugData.coordinateSystemDebugInfo?.usedCoordinateSet || 'Default',
						systemType: debugData.coordinateSystemDebugInfo?.coordinateSystemType || 'Standard'
					}
				};
			case 3: // Rotation
				return {
					...
					data: {
						rotation: debugData.finalRotation !== undefined ? `${debugData.finalRotation.toFixed(1)}°` : 'Not calculated',
						location: debugData.calculatedLocation || 'Not calculated'
					}
				};
			case 4: // Adjustments
				return {
					...
					data: {
						adjustment: debugData.defaultAdjustment ?
							`(${debugData.defaultAdjustment.x.toFixed(1)}, ${debugData.defaultAdjustment.y.toFixed(1)})` : 'Not calculated',
						finalPosition: debugData.finalPosition ?
							`(${debugData.finalPosition.x.toFixed(1)}, ${debugData.finalPosition.y.toFixed(1)})` : 'Not calculated'
					}
				};
			default:
	...
	<div class="steps-header">
		...
		{#if stepByStepMode}
			<div class="step-navigation">
				...
				<span class="step-counter">Step {currentStep + 1} of {maxSteps + 1}</span>
				<button 
...
<style>
	...
	.errors-section h4, .timing-section h4 {
		color: #fbbf24;
		margin: 0 0 8px 0;
		font-size: 0.9rem;
	}
