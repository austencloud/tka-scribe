...
<script lang="ts">
	...
	let { selectedOrientation, onOrientationChange, label, color, disabled = false }: Props = $props();
...
<div class="orientation-button-container">
	...
	<button
		...
		{disabled}
		aria-label={disabled ? `${label} orientation: ${selectedOrientation.toUpperCase()}` : `Click to select ${label} orientation. Current: ${selectedOrientation.toUpperCase()}`}
		title={disabled ? `${selectedOrientation.toUpperCase()} (calculated)` : `Click to select orientation. Current: ${selectedOrientation.toUpperCase()}`}
	>
