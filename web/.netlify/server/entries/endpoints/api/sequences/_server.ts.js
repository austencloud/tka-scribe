  ...
  try {
    ...
    for (const dirent of sequenceDirectories) {
      if (dirent.isDirectory()) {
        ...
        if (sequenceName === "A_A" || sequenceName.length < 2 || sequenceName.includes("test")) {
          continue;
        }
    ...
    try {
      ...
      for (const file of thumbnailFiles) {
        if (file.endsWith(".png")) {
          ...
          if (word === "A_A" || word.length < 2 || word.length > 20 || // ✅ FIXED: Skip extremely long names
          word.includes("test") || word.includes("__") || // ✅ FIXED: Skip double underscores
