...
class TurnIntensityManager:
    ...
    def allocate_turns_for_blue_and_red(self) -> tuple[list[int | float | str], list[int | float | str]]:
        """
...
class TurnIntensityManagerFactory:
    ...
    @staticmethod
    def allocate_turns_for_blue_and_red(length: int, level: int, turn_intensity: float) -> tuple[list[int | float | str], list[int | float | str]]:
        """
