...
def analyze_redundancy():
    ...
    for category, files in candidates.items():
        ...
        if category_files > 0:
            print(f"  📊 Category total: {category_size:.2f} KB, {category_files} files")
    ...
    print(f"Average file size:        {total_size_saved/total_files:.2f} KB" if total_files > 0 else "No files found")
...
if __name__ == "__main__":
    ...
    print(f"Found {total_files} files totaling {total_size:.2f} KB for potential cleanup")
    print(f"Run this analysis again after testing to execute cleanup!")
