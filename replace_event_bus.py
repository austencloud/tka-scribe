...
def replace_event_bus_with_signals(file_path):
    ...
    content = re.sub(r'\s*# Subscribe to UI events\s*self\._setup_event_subscriptions\(\)', '', content)
    ...
    content = re.sub(pattern, replace_with_signal, content, flags=re.MULTILINE | re.DOTALL)
    ...
    content = re.sub(r'def _setup_event_subscriptions\(self\).*?(?=def|\Z)', '', content, flags=re.DOTALL)
    
    with open(file_path, 'w') as f:
        f.write(content)

if __name__ == "__main__":
    replace_event_bus_with_signals("src/shared/application/services/ui/ui_state_manager.py")
