...
import { ensureContainerInitialized, container } from '../src/lib/shared/inversify/container.js';
...
async function main() {
  ...
  try {
    ...
    console.log('Testing with 1-beat sequence (full 16-beat would take too long in Node)\n');

    // Test current settings
    console.log('📊 Testing CURRENT settings (beatScale: 0.15, quality: 0.4, JPEG)...');
    ...
    // Analysis
    console.log('\n📈 ANALYSIS\n');
    console.log(`Current: ${currentTime.toFixed(0)}ms, ${currentSize.toFixed(1)}KB`);
    console.log(`Recommended: ${recTime.toFixed(0)}ms (${((recTime - currentTime) / currentTime * 100).toFixed(0)}% ${recTime > currentTime ? 'slower' : 'faster'}), ${recSize.toFixed(1)}KB`);

    console.log('\n💡 IMPORTANT NOTE:');
    console.log('This is testing with 1 beat in Node.js environment.');
    console.log('In the browser with 16 beats, multiply these times by ~15-20x');
    console.log(`Estimated browser time for 16 beats: ${(currentTime * 17).toFixed(0)}ms`);

    if (currentTime * 17 > 500) {
      console.log('\n⭐ RECOMMENDATION: Implement background pre-rendering');
      console.log('   Estimated 16-beat render time > 500ms - users will notice delay');
    }
