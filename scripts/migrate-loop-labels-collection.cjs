...
async function migrateCollection() {
  ...
  try {
    ...
    if (snapshot.empty) {
      console.log('No documents found in loop-labels collection. Nothing to migrate.');
      process.exit(0);
    }
    ...
    console.log(`✓ Verification passed: ${newSnapshot.size} documents in loop-labels.\n`);
    ...
    if (deleteOld) {
      ...
      console.log('  node scripts/migrate-loop-labels-collection.js --delete-old\n');
    }
