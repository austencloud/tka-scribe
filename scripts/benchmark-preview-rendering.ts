  ...
  beats: Array(16).fill(null).map((_, i) => ({
    ...
    motions: {
      blue: {
        ...
        color: 'blue',
        gridMode: 'diamond',
        arrowPlacementData: { positionX: 0, positionY: 0, rotationAngle: 0, coordinates: null, svgCenter: null, svgMirrored: false },
        propPlacementData: { positionX: 0, positionY: 0, rotationAngle: 0, coordinates: null, svgCenter: null },
...
async function benchmark(
  ...
  config: { beatScale: number; quality: number; format: 'PNG' | 'JPEG' | 'WebP' },
  runs = 5
...
async function main() {
  ...
  const renderService = container.get<ISequenceRenderService>(TYPES.ISequenceRenderService);
  ...
  const configs = [
    ...
    { beatScale: 0.10, quality: 0.3, format: 'JPEG' as const, name: 'Aggressive Optimization' },
    { beatScale: 0.08, quality: 0.2, format: 'JPEG' as const, name: 'Ultra Low' },

    // Format comparison
    { beatScale: 0.15, quality: 0.4, format: 'PNG' as const, name: 'PNG Format' },
    { beatScale: 0.15, quality: 0.4, format: 'WebP' as const, name: 'WebP Format' },

    // Higher quality for comparison
    { beatScale: 0.20, quality: 0.6, format: 'JPEG' as const, name: 'Higher Quality' },
  ];
  ...
  console.log('\n📊 BENCHMARK RESULTS\n');
  console.log('┌─────────────────────────────┬──────────┬──────────┬──────────┬──────────┐');
  console.log('│ Configuration               │ Avg (ms) │ Min (ms) │ Max (ms) │ Size (KB)│');
  console.log('├─────────────────────────────┼──────────┼──────────┼──────────┼──────────┤');
  ...
  console.log(`Current Settings: ${current.avgTime.toFixed(0)}ms, ${current.sizeKB.toFixed(1)}KB`);
  console.log(`Fastest Config: ${fastest.name} - ${fastest.avgTime.toFixed(0)}ms (${((current.avgTime - fastest.avgTime) / current.avgTime * 100).toFixed(1)}% faster)`);
  console.log(`Smallest Size: ${smallest.name} - ${smallest.sizeKB.toFixed(1)}KB (${((current.sizeKB - smallest.sizeKB) / current.sizeKB * 100).toFixed(1)}% smaller)`);

  // Recommendations
  console.log('\n💡 RECOMMENDATIONS\n');

  if (fastest.avgTime < current.avgTime * 0.7) {
    console.log(`✅ RECOMMENDATION: Switch to "${fastest.name}" for ${((current.avgTime - fastest.avgTime) / current.avgTime * 100).toFixed(0)}% faster rendering`);
  } else {
    console.log('✅ Current settings are already well-optimized');
  }

  if (current.avgTime > 1000) {
    console.log('⚠️  WARNING: Preview generation takes >1s - consider background pre-rendering');
  } else if (current.avgTime > 500) {
    console.log('⚠️  WARNING: Preview generation takes >500ms - users may notice delay');
  } else {
    console.log('✅ Preview generation time is acceptable for real-time use');
  }

  if (current.sizeKB > 200) {
    console.log('⚠️  WARNING: Preview size is >200KB - may be slow on poor connections');
  } else {
    console.log('✅ Preview size is acceptable for phone screens');
  }

  console.log('\n🎯 OPTIMIZATION STRATEGIES:\n');
  console.log('1. Background Pre-rendering: Generate preview when sequence is opened, not when share button is clicked');
  console.log('2. Caching: Cache generated previews in memory to avoid regeneration');
  console.log('3. Progressive Loading: Show a low-quality placeholder immediately, then enhance');
  console.log('4. Lazy Generation: Only generate preview when share panel is actually opened');
  console.log('5. Web Workers: Move rendering to a background thread (if supported by your rendering library)');
