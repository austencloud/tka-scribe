...
async function releaseToProduction() {
  ...
  const productionSection = netlifyConfig.match(
  ...
  const modules = {
    ...
    ML_TRAINING: netlifyConfig.includes(
  ...
  console.log("\n");
  const changeModules = await question(
  ...
  if (changeModules.toLowerCase() === "y") {
    ...
    for (const [name, enabled] of Object.entries(updates)) {
      ...
      const regex = new RegExp(
        `(\\[context\\.production\\.environment\\][\\s\\S]*?${varName}\\s*=\\s*)"(?:true|false)"`,
        "g"
      );
    ...
    if (confirm.toLowerCase() === "y") {
      ...
      const message =
        commitMessage.trim() || "chore: Update production module visibility";
      ...
      try {
        ...
        const pushAction = await question(
          "What next? (1) Push to current branch, (2) Create PR, (3) Nothing: "
        );
        ...
        if (pushAction === "1") {
          ...
          console.log(
            `   Open GitHub to create the PR manually, or use gh CLI:\n`
          );
          console.log(
            `   gh pr create --title "${prTitle}" --body "Updates production module visibility" --base main\n`
          );
        }
