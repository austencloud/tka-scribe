...
async function findEmptyItems() {
  ...
  snapshot.forEach(doc => {
    ...
    if (!hasTitle && !hasDesc) {
      ...
      missingTitle.push({ id: doc.id, desc: (d.description || '').substring(0, 50), status: d.status });
    } else if (!hasDesc) {
      missingDesc.push({ id: doc.id, title: (d.title || '').substring(0, 50), status: d.status });
    }
  });
  ...
  missingDesc.forEach(item => {
    console.log('  -', item.id, '| title:', item.title, '| status:', item.status);
  });
