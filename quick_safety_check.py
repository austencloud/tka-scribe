...
def test_focused_services_basic():
    ...
    try:
        from src.desktop.modern.application.services.sequence.beat_creation_service import BeatCreationService
        from src.desktop.modern.application.services.sequence.beat_sequence_service import BeatSequenceService
        from src.desktop.modern.application.services.sequence.sequence_word_calculator import SequenceWordCalculator
        from src.desktop.modern.application.services.sequence.sequence_persistence_adapter import SequencePersistenceAdapter
        from src.desktop.modern.application.services.sequence.beat_operation_coordinator import BeatOperationCoordinator
    ...
    try:
        ...
        if simplified != "ABC":
            issues_found.append(f"Word calculation failed: expected 'ABC', got '{simplified}'")
        else:
        ...
        if next_beat != 1:
            issues_found.append(f"Beat number calculation failed: expected 1, got {next_beat}")
        else:
    ...
    try:
        from src.desktop.modern.application.services.sequence.sequence_beat_operations import SequenceBeatOperations
