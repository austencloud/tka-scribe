...
describe('ArrowAdjustmentLookup (advanced orchestration)', () => {
  ...
  it('returns special adjustment when available', async () => {
    ...
    expect(mocks.mockSpecial.getSpecialAdjustment).toHaveBeenCalledWith(motion, pictograph, undefined);
  });
  ...
  it('falls back to default when special not available', async () => {
    ...
    expect(mocks.mockDefault.getDefaultAdjustment).toHaveBeenCalledWith('pro', 0, 'pro', 'diamond');
  });
