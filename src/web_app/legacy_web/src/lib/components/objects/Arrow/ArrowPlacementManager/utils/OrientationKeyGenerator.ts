...
export class OrientationKeyGenerator {
	...
	public generateOriKeyFromMotion(arrow: ArrowData, checker?: any): string {
		// Provide default checker if none provided
		if (!checker) {
	...
	/**
	 * Creates a default checker object with the required methods
	 */
	private createDefaultChecker() {
		return {
			endsWithLayer3: () => false,
			endsWithRadialOri: () => false,
			endsWithNonRadialOri: () => false
		};
	}
}
