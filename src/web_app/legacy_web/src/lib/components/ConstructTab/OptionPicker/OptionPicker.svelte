<script lang="ts">
  ...
  onMount(() => {
    ...
    const loadOptionsFromSequence = async () => {
      try {
        // First try to get start position from localStorage
        ...
        if (startPositionData) {
          ...
          if (endPosition) {
            ...
            // Use OptionDataService to get real options from CSV data
            const nextOptions = optionDataService.getNextOptions(
              endPosition,
              "diamond"
            );

            console.log(
              `✅ Loaded ${nextOptions.length} real options from CSV data`
            );

            // Load the real options
            actions.loadOptions(nextOptions);
            return;
          }
        }

        // Fallback: try to get from sequence data
        const fullSequence = sequenceDataService.getCurrentSequence();

        if (fullSequence && Array.isArray(fullSequence)) {
          ...
          if (startPosBeat) {
            // Get the end position from the start position beat
            const endPosition = startPosBeat.endPos;

            if (endPosition) {
              console.log(
                `🎯 Loading options for end position: ${endPosition}`
              );
        ...
        // No valid data found
        console.warn("⚠️ No valid start position or sequence data found");
        actions.loadOptions([]);
      } catch (error) {
