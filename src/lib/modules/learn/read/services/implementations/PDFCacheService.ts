...
export class PDFCacheService {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.PAGES_STORE)) {
          const pagesStore = db.createObjectStore(this.PAGES_STORE, {
            keyPath: "id",
          });
  ...
  async hasCachedPages(url: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      request.onsuccess = () => {
        ...
        const isValid =
          Date.now() - metadata.timestamp < this.CACHE_DURATION_MS;
        resolve(isValid);
      };
  ...
  async cachePage(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          `PDFCacheService: Failed to cache page ${pageData.pageNumber}`
        );
        reject(request.error);
      };
    });
  ...
  async clearPDFCache(url: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.db!.transaction(
        [this.PAGES_STORE, this.METADATA_STORE],
        "readwrite"
      );
