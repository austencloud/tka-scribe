...
<script lang="ts">
  ...
  let activeSubTab = $state<"codex" | "quiz" | "read">(
    navigationState.currentLearnMode as "codex" | "quiz" | "read"
  );

  // Sync with navigation state
  $effect(() => {
    const navMode = navigationState.currentLearnMode as
      | "codex"
      | "quiz"
      | "read";
  ...
  onMount(async () => {
    ...
    activeSubTab = navigationState.currentLearnMode as
      | "codex"
      | "quiz"
      | "read";
