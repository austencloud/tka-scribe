 ...
 */
export const CONCEPT_CATEGORIES: Record<ConceptCategory, {
  name: string;
  description: string;
  icon: string;
  color: string;
 ...
 */
export function getConceptsByCategory(category: ConceptCategory): LearnConcept[] {
 ...
 */
export function getPreviousConcept(currentId: string): LearnConcept | undefined {
  const currentIndex = TKA_CONCEPTS.findIndex(c => c.id === currentId);
