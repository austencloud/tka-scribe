...
<script lang="ts">
  ...
  // Setup services, event listeners, and resize observers
  onMount(() => {
    hapticService = resolve<IHapticFeedbackService>(TYPES.IHapticFeedbackService);
    deviceDetector = resolve<IDeviceDetector>(TYPES.IDeviceDetector);

    // Event handlers for global animation control
    const handleAnimationModeChange = (event: CustomEvent) => {
      displayState.setAnimationMode(event.detail.isSequential ? "sequential" : "all-at-once");
    };
    ...
    window.addEventListener("animation-mode-change", handleAnimationModeChange as EventListener);
    window.addEventListener("clear-sequence-animation", handleClearSequenceAnimation as EventListener);
    window.addEventListener("prepare-sequence-animation", handlePrepareSequenceAnimation as EventListener);

    // Setup resize observers and scroll container
    if (containerRef && scrollContainerRef) {
      // Container resize observer for grid layout
      ...
      // Scroll content resize observer
      ...
      return () => {
        ...
        window.removeEventListener("animation-mode-change", handleAnimationModeChange as EventListener);
        window.removeEventListener("clear-sequence-animation", handleClearSequenceAnimation as EventListener);
        window.removeEventListener("prepare-sequence-animation", handlePrepareSequenceAnimation as EventListener);
      };
    }
