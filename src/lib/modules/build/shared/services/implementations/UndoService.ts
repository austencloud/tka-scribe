...
export class UndoService implements IUndoService {
  ...
  pushUndo(
    ...
    this.saveHistory().catch((error) => {
      console.error('❌ UndoService: Failed to save history after push:', error);
    });

    console.log(`✅ UndoService: Pushed ${type} to undo history (ID: ${id})`);
  ...
  undo(): UndoHistoryEntry | null {
    if (!this.canUndo) {
      console.warn('⚠️ UndoService: No actions to undo');
      return null;
    }
    ...
    // Persist to storage
    this.saveHistory().catch((error) => {
      console.error('❌ UndoService: Failed to save history after undo:', error);
    });

    console.log(`⏪ UndoService: Undoing ${entry.type} (ID: ${entry.id})`);
  ...
  redo(): UndoHistoryEntry | null {
    if (!this.canRedo) {
      console.warn('⚠️ UndoService: No actions to redo');
      return null;
    }
    ...
    // Persist to storage
    this.saveHistory().catch((error) => {
      console.error('❌ UndoService: Failed to save history after redo:', error);
    });

    // Notify subscribers of change
    this.notifyChange();

    console.log(`⏩ UndoService: Redoing ${entry.type} (ID: ${entry.id})`);
  ...
  clearHistory(): void {
    ...
    // Persist to storage
    this.saveHistory().catch((error) => {
      console.error('❌ UndoService: Failed to save history after clear:', error);
    });

    console.log('🧹 UndoService: Cleared all history');
  ...
  clearRedoHistory(): void {
    ...
    // Persist to storage
    this.saveHistory().catch((error) => {
      console.error('❌ UndoService: Failed to save history after clearing redo:', error);
    });

    console.log('🧹 UndoService: Cleared redo history');
  ...
  async loadHistory(): Promise<void> {
    try {
      ...
      if (undoData) {
        ...
        console.log(`✅ UndoService: Loaded ${this._undoHistory.length} undo entries`);
      }
      ...
      if (redoData) {
        ...
        console.log(`✅ UndoService: Loaded ${this._redoHistory.length} redo entries`);
      }

      // Notify subscribers after loading
      this.notifyChange();
    } catch (error) {
      console.error('❌ UndoService: Failed to load history from storage:', error);
  ...
  async saveHistory(): Promise<void> {
    try {
      localStorage.setItem(UNDO_HISTORY_STORAGE_KEY, JSON.stringify(this._undoHistory));
      localStorage.setItem(REDO_HISTORY_STORAGE_KEY, JSON.stringify(this._redoHistory));
    } catch (error) {
      console.error('❌ UndoService: Failed to save history to storage:', error);
      throw error;
    }
  }
