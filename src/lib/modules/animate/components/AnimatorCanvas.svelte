...
<script lang="ts">
  ...
  function updateButtonPosition() {
    ...
    if (canvasRect.width === 0 || canvasRect.height === 0) {
      console.log(
        "[AnimatorCanvas] updateButtonPosition: canvas has no dimensions"
      );
      return;
    }
  ...
  function renderLoop(currentTime?: number): void {
    ...
    if (
      canvasState.needsRender ||
      (canvasState.trailSettings.enabled &&
        canvasState.trailSettings.mode !== TrailMode.OFF)
    ) {
      render(now);
