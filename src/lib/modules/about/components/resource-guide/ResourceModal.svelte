<script lang="ts">
  ...
  function handleClose() {
    // Trigger selection haptic feedback for modal close button
  ...
  function handleBackdropClick(event: MouseEvent) {
    if (event.target === modalContainer) {
      // Trigger selection haptic feedback for backdrop close
      hapticService?.trigger("selection");
      handleClose();
    }
  }
  ...
  function handleKeydown(event: KeyboardEvent) {
    if (event.key === "Escape") {
      // Trigger selection haptic feedback for escape key close
      hapticService?.trigger("selection");
      handleClose();
