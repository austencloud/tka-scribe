...
export interface IGalleryDisplayStateService {
 ...
 */
export function createGalleryDisplayStateService(): IGalleryDisplayStateService {
  // Private reactive state using Svelte 5 runes
  const loadingState = $state<GalleryLoadingState>(createDefaultLoadingState());
  let displayState = $state<GalleryDisplayState>(createDefaultDisplayState());
  ...
  return {
    ...
    get hasError() {
      return (
        (loadingState as GalleryLoadingState & { error: string | null })
          .error !== null
      );
    },
    ...
    setLoading(loading: boolean, operation?: string): void {
      ...
      if (operation) {
        (
          loadingState as GalleryLoadingState & { currentOperation: string }
        ).currentOperation = operation;
      }
      if (loading) {
        (loadingState as GalleryLoadingState & { error: string | null }).error =
...
export class GalleryDisplayStateService implements IGalleryDisplayStateService {
