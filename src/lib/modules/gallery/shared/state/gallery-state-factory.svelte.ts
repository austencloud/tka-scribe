...
export function createGalleryState() {
  ...
  const filterService = resolve(
    TYPES.IGalleryFilterService
  ) as IGalleryFilterService;
  const sortService = resolve(TYPES.IGallerySortService) as IGallerySortService;
  ...
  let currentSortMethod = $state<GallerySortMethod>(
    GallerySortMethod.ALPHABETICAL
  );
  let sortDirection = $state<"asc" | "desc">("asc");
  const showSections = $state<boolean>(true);
  let currentFilter = $state<{ type: string; value: GalleryFilterValue }>({
    type: "all",
    value: null,
  });
  ...
  async function loadAllSequences(): Promise<void> {
    try {
      ...
      console.log(
        `✅ GalleryState: Loaded ${sequences.length} sequences and ${sections.length} navigation sections`
      );
    } catch (err) {
  ...
  async function setActiveGalleryNavigationItem(
    ...
    console.log(
      "🎯 GalleryState: Set active navigation item:",
      sectionId,
      itemId
    );
    ...
    try {
      ...
      if (currentFilter.type !== "all") {
        filtered = filterService.applyFilter(
          allSequences,
          currentFilter.type as GalleryFilterType,
          currentFilter.value
        );
      }
  ...
  function openAnimationModal(sequence: SequenceData): void {
    console.log("🎬 GalleryState: Opening animation modal for:", sequence.id);
    sequenceToAnimate = sequence;
    isAnimationModalOpen = true;
  }

  function closeAnimationModal(): void {
    console.log("🎬 GalleryState: Closing animation modal");
