<script lang="ts">
  ...
  function getDisplayName(sequence: LibrarySequence): string {
    ...
    const isAutoGeneratedName = sequence.name?.match(/^Sequence\s+\d+:\d+:\d+/i);
              ...
              <div class="sequence-meta">
                {#if sequence.createdAt}
                  <span class="meta-item">
                    ...
                    {sequence.createdAt instanceof Date ? sequence.createdAt.toLocaleDateString() : new Date(sequence.createdAt).toLocaleDateString()}
                  </span>
                {/if}
                {#if sequence.beats && sequence.beats.length > 0}
                  <span class="meta-item">
