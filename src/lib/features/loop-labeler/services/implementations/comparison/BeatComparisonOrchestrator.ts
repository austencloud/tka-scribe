...
export class BeatComparisonOrchestrator implements IBeatComparisonOrchestrator {
	...
	extractBeats(sequence: SequenceEntry): ExtractedBeat[] {
		...
		return raw
			.filter(
				(b): b is typeof b & { beat: number } => typeof b.beat === 'number' && b.beat >= 1
			)
			.map((b) => ({
				beatNumber: b.beat,
				letter: b.letter || '',
	...
	compareBeatPair(beat1: ExtractedBeat, beat2: ExtractedBeat): string[] {
		...
		const repeated = this.swapInvertService.checkRepeated(b1Blue, b1Red, b2Blue, b2Red);
		allTransformations.push(...repeated.transformations);

		// Check rotations
		const rotations = this.rotationService.checkRotations(b1Blue, b1Red, b2Blue, b2Red);
		allTransformations.push(...rotations.transformations);

		// Check reflections
		const reflections = this.reflectionService.checkReflections(b1Blue, b1Red, b2Blue, b2Red);
		allTransformations.push(...reflections.transformations);

		// Check swap/invert
		const swapInvert = this.swapInvertService.checkSwapInvert(b1Blue, b1Red, b2Blue, b2Red);
	...
	generateHalvedBeatPairs(beats: ExtractedBeat[]): InternalBeatPair[] {
		...
		for (let i = 0; i < halfLength; i++) {
			...
			const { primary, all } = this.formattingService.formatBeatPairTransformations(rawTransformations);
