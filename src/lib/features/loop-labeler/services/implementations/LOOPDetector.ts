...
export class LOOPDetector implements ILOOPDetector {
	...
	detectLOOP(sequence: SequenceEntry): LOOPDetectionResult {
		...
		const polyrhythmic: PolyrhythmicLOOPResult = this.polyrhythmicService?.detectPolyrhythmic(
			rawSequence
		) ?? {
	...
	private detectQuarteredPattern(
		...
		const loopType = compoundPattern
			? this.buildCompoundLoopType(derivedDirection, compoundPattern.halvedTransformations)
	...
	private detectModularQuarteredPattern(
		...
		// Build loopType
		const rotDir = rotationDirection === 'ccw' ? '_ccw' : rotationDirection === 'cw' ? '_cw' : '';
		const loopType = modularAnalysis.swapRhythm !== 'uniform'
