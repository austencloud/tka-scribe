...
<script lang="ts">
  ...
  function loadSavedStatus(): FeedbackStatus {
  ...
  onMount(() => {
    ...
    // Load persisted status after service is available
    activeStatus = loadSavedStatus();
  });

  // Persist active status when it changes
  $effect(() => {
    if (storageService) {
      storageService.safeLocalStorageSet(STORAGE_KEY, activeStatus);
    }
  });
