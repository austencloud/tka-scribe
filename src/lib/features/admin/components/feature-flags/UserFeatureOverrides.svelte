<script lang="ts">
  ...
  function toggleEnabledFeature(featureId: FeatureId) {
    if (editedEnabledFeatures.includes(featureId)) {
      editedEnabledFeatures = editedEnabledFeatures.filter(
        (f) => f !== featureId
      );
    } else {
      editedEnabledFeatures = [...editedEnabledFeatures, featureId];
      editedDisabledFeatures = editedDisabledFeatures.filter(
        (f) => f !== featureId
      );
    }
  }
  ...
  async function saveUserOverrides() {
    ...
    const hasChanges =
      JSON.stringify(editedEnabledFeatures.sort()) !==
        JSON.stringify(
          (selectedUser.featureOverrides?.enabledFeatures || []).sort()
        ) ||
      JSON.stringify(editedDisabledFeatures.sort()) !==
        JSON.stringify(
          (selectedUser.featureOverrides?.disabledFeatures || []).sort()
        );
  ...
  {#snippet detail()}
    {#if selectedUser}
      <UserOverridesDetail
        ...
        onReset={() => {
          editedEnabledFeatures =
            selectedUser?.featureOverrides?.enabledFeatures || [];
          editedDisabledFeatures =
            selectedUser?.featureOverrides?.disabledFeatures || [];
        }}
        onClose={() => (selectedUser = null)}
      />
    {/if}
