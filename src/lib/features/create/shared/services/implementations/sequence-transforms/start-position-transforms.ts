...
import { rotateLocation, invertMotionType, reverseRotationDirection } from "./rotation-helpers";

/**
 * Mirror a start position across the vertical axis (E ↔ W).
 */
export function mirrorStartPosition(startPos: StartPositionData): StartPositionData {
	const mirroredMotions = { ...startPos.motions };

	if (startPos.motions[MotionColor.BLUE]) {
		mirroredMotions[MotionColor.BLUE] = mirrorMotion(startPos.motions[MotionColor.BLUE]);
	}
	if (startPos.motions[MotionColor.RED]) {
		mirroredMotions[MotionColor.RED] = mirrorMotion(startPos.motions[MotionColor.RED]);
	}
 ...
 */
export function flipStartPosition(startPos: StartPositionData): StartPositionData {
	const flippedMotions = { ...startPos.motions };

	if (startPos.motions[MotionColor.BLUE]) {
		flippedMotions[MotionColor.BLUE] = flipMotion(startPos.motions[MotionColor.BLUE]);
	}
	if (startPos.motions[MotionColor.RED]) {
		flippedMotions[MotionColor.RED] = flipMotion(startPos.motions[MotionColor.RED]);
	}
...
export function rotateStartPosition(
	...
	if (startPos.motions[MotionColor.BLUE]) {
		rotatedMotions[MotionColor.BLUE] = rotateMotion(startPos.motions[MotionColor.BLUE], rotationAmount);
	}
	if (startPos.motions[MotionColor.RED]) {
		rotatedMotions[MotionColor.RED] = rotateMotion(startPos.motions[MotionColor.RED], rotationAmount);
	}

	const rotatedGridPosition = startPos.gridPosition
		? (rotateLocation(startPos.gridPosition as GridPosition, rotationAmount) as GridPosition)
		: null;
 ...
 */
export function colorSwapStartPosition(startPos: StartPositionData): StartPositionData {
	...
	return createStartPositionData({
		...
		gridPosition: startPos.gridPosition ? SWAPPED_POSITION_MAP[startPos.gridPosition] : null
	});
}
