...
import type { GridLocation, GridPosition } from "$lib/shared/pictograph/grid/domain/enums/grid-enums";
...
export function mirrorBeat(beat: BeatData): BeatData {
	...
	if (beat.motions[MotionColor.BLUE]) {
		mirroredMotions[MotionColor.BLUE] = mirrorMotion(beat.motions[MotionColor.BLUE]);
	}
	if (beat.motions[MotionColor.RED]) {
		mirroredMotions[MotionColor.RED] = mirrorMotion(beat.motions[MotionColor.RED]);
	}

	return createBeatData({
		...beat,
		startPosition: beat.startPosition ? VERTICAL_MIRROR_POSITION_MAP[beat.startPosition] : null,
		endPosition: beat.endPosition ? VERTICAL_MIRROR_POSITION_MAP[beat.endPosition] : null,
...
export function flipBeat(beat: BeatData): BeatData {
	...
	if (beat.motions[MotionColor.BLUE]) {
		flippedMotions[MotionColor.BLUE] = flipMotion(beat.motions[MotionColor.BLUE]);
	}
	if (beat.motions[MotionColor.RED]) {
	...
	return createBeatData({
		...beat,
		startPosition: beat.startPosition ? HORIZONTAL_MIRROR_POSITION_MAP[beat.startPosition] : null,
		endPosition: beat.endPosition ? HORIZONTAL_MIRROR_POSITION_MAP[beat.endPosition] : null,
...
export function rotateBeat(
	...
	if (beat.motions[MotionColor.BLUE]) {
		rotatedMotions[MotionColor.BLUE] = rotateMotion(beat.motions[MotionColor.BLUE], rotationAmount);
	}
	if (beat.motions[MotionColor.RED]) {
		rotatedMotions[MotionColor.RED] = rotateMotion(beat.motions[MotionColor.RED], rotationAmount);
	}
...
export function colorSwapBeat(beat: BeatData): BeatData {
	...
	return createBeatData({
		...beat,
		startPosition: beat.startPosition ? SWAPPED_POSITION_MAP[beat.startPosition] : null,
		endPosition: beat.endPosition ? SWAPPED_POSITION_MAP[beat.endPosition] : null,
...
export async function invertBeat(
	...
	if (beat.motions[MotionColor.BLUE]) {
		invertedMotions[MotionColor.BLUE] = invertMotion(beat.motions[MotionColor.BLUE]);
	}
	if (beat.motions[MotionColor.RED]) {
		invertedMotions[MotionColor.RED] = invertMotion(beat.motions[MotionColor.RED]);
	}
	...
	if (invertedMotions[MotionColor.BLUE] && invertedMotions[MotionColor.RED]) {
		try {
			...
			console.warn(`Failed to find letter for inverted beat ${beat.beatNumber}:`, error);
		}
	}
...
export async function rewindBeat(
	...
	if (beat.motions[MotionColor.BLUE]) {
		rewindMotions[MotionColor.BLUE] = rewindMotion(beat.motions[MotionColor.BLUE]);
	}
	if (beat.motions[MotionColor.RED]) {
		rewindMotions[MotionColor.RED] = rewindMotion(beat.motions[MotionColor.RED]);
	}
