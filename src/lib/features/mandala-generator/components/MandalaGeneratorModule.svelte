<script lang="ts">
	...
	function handleAssetSelect(assetType: string, motionType: string, color: string) {
		...
		controller.addElement(assetType as 'arrow' | 'staff' | 'gridDot', position, {
			color,
	...
	function handleCanvasDrop(event: DragEvent) {
		...
		try {
			...
			controller.addElement(asset.type, position, {
				color: asset.color,
				arrowSpec: asset.type === 'arrow' ? { motionType: asset.motionType } : undefined
			});
		} catch (e) {
	...
	{#if isMobile}
		...
		<div class="mobile-layout">
			...
			<div class="action-bar">
				<button class="action-btn" onclick={handleUndo} disabled={!mandalaState.canUndo} title="Undo">
					↩
				</button>
				<button class="action-btn" onclick={handleRedo} disabled={!mandalaState.canRedo} title="Redo">
					↪
				</button>
				<button class="action-btn" onclick={handleClear} disabled={!mandalaState.hasElements} title="Clear">
					🗑
				</button>
				<button class="action-btn primary" onclick={handleExport} disabled={!mandalaState.hasElements} title="Export">
					💾
				</button>
			</div>
		...
		<div class="desktop-layout">
			...
			<main class="center-panel">
				<div class="panel-header">
					<h2>Canvas</h2>
					<div class="action-bar">
						<button class="action-btn" onclick={handleUndo} disabled={!mandalaState.canUndo} title="Undo (Ctrl+Z)">
							↩ Undo
						</button>
						<button class="action-btn" onclick={handleRedo} disabled={!mandalaState.canRedo} title="Redo (Ctrl+Y)">
							↪ Redo
						</button>
						<button class="action-btn" onclick={handleClear} disabled={!mandalaState.hasElements} title="Clear canvas">
							🗑 Clear
						</button>
						<button class="action-btn primary" onclick={handleExport} disabled={!mandalaState.hasElements} title="Export">
							💾 Export
				...
				{#if mandalaState.selectedElement}
					<div class="properties-panel">
						...
						<div class="property-row">
							...
							<span class="property-value">{mandalaState.selectedElement.type}</span>
						</div>
						<div class="property-row">
							...
							<input
								...
								oninput={(e) => {
									...
									if (el) {
										controller.rotateElement(el.id, parseInt(e.currentTarget.value));
									}
								}}
							/>
							<span class="property-value">{Math.round(mandalaState.selectedElement.rotation)}°</span>
						</div>
						<div class="property-row">
							...
							<input
								...
								oninput={(e) => {
									const el = mandalaState.selectedElement;
									if (el) {
										controller.scaleElement(el.id, parseFloat(e.currentTarget.value));
									}
								}}
							/>
							<span class="property-value">{mandalaState.selectedElement.scale.toFixed(1)}x</span>
						</div>
						<div class="property-actions">
							<button class="action-btn" onclick={() => controller.duplicateSelectedElement()}>
								Duplicate
							</button>
							<button class="action-btn danger" onclick={() => controller.removeSelectedElement()}>
								Delete
							</button>
						</div>
					</div>
