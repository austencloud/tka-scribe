...
import type { MandalaElement, Point, ArrowSpec } from '../domain/models/mandala-element';
import type { MandalaConfig } from '../domain/models/mandala-config';
import type { MandalaPreset } from '../domain/models/mandala-preset';
import type { SymmetryFold, MirrorAxis, ArrowMotionType, ArrowTurns } from '../domain/enums/mandala-enums';
...
export class MandalaController {
	...
	duplicateElement(id: string, offset: Point = { x: 20, y: 20 }) {
		...
		const duplicate = createMandalaElement(element.type, {
	...
	handleKeyDown(event: KeyboardEvent) {
		...
		if ((key === 'delete' || key === 'backspace') && this.state.selectedElementId) {
			event.preventDefault();
