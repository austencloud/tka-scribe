...
export class MandalaTransformer implements IMandalaTransformer {
	...
	applyRotationalSymmetry(
		...
		for (let i = 0; i < foldCount; i++) {
			...
			const rotatedPosition = this.rotatePoint(element.position, rotationAngle, center);
	 ...
	 */
	applyMirror(element: MandalaElement, axisAngle: number, center: Point): TransformResult {
		const mirroredPosition = this.mirrorPoint(element.position, axisAngle, center);

		// Mirror the element's rotation as well
		// Reflection inverts the rotation direction relative to the axis
		const mirroredRotation = this.normalizeAngle(2 * axisAngle - element.rotation);
	...
	generateKaleidoscopePattern(
		...
		const rotatedCopies = this.applyRotationalSymmetry(element, config.foldCount, center);
		...
		for (const rotated of rotatedCopies) {
			...
			if (config.enableMirror && config.mirrorAxis !== 'none') {
				const mirrorAngles = this.getMirrorAngles(config.mirrorAxis, config.foldCount);
	 ...
	 */
	private getMirrorAngles(mirrorAxis: string, foldCount: SymmetryFold): number[] {
