...
<script lang="ts">
	...
	const showCanvas = $derived(displayView === "camera-canvas" || displayView === "camera-canvas-grid");
	const showGrid = $derived(displayView === "camera-grid" || displayView === "camera-canvas-grid");
...
<style>
	...
	/* View A & B: Two panels stacked (default flex column is used) */
