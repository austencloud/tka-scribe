	...
	return new Promise((resolve, reject) => {
		...
		request.onupgradeneeded = (event) => {
			...
			if (!db.objectStoreNames.contains(STORES.FRAMES)) {
				...
				frameStore.createIndex('sessionId_frameNumber', ['sessionId', 'frameNumber'], { unique: true });
			}
...
export class MLTrainingStorageService {
	 ...
	 */
	async addFramesBatch(frames: Array<{ frame: CapturedFrame; imageBlob: Blob }>): Promise<void> {
		const db = this.getDb();
		const tx = db.transaction([STORES.FRAMES, STORES.IMAGES], 'readwrite');
	...
	async getSessionStats(sessionId: string): Promise<DatasetStats> {
		...
		const directions: HeadDirection[] = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
