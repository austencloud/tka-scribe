...
export class DataCaptureService {
	...
	private async captureFrame(): Promise<void> {
		...
		if (!this.videoElement || !this.canvas || !this.canvasCtx || !this.state.currentSession) return;
		...
		if (this.pendingFrames.length >= this.MAX_PENDING_FRAMES) {
			...
			const message = 'Storage is catching up; temporarily pausing frame capture.';
		...
		try {
			...
			const message = error instanceof Error ? error.message : 'Frame capture error';
			this.state.lastError = message;
			this.notifyStateChange();
	...
	private async convertCanvasToBlob(canvas: OffscreenCanvas): Promise<Blob | null> {
		if (typeof canvas.convertToBlob === 'function') {
			return canvas.convertToBlob({ type: 'image/jpeg', quality: 0.9 });
		}

		// HTMLCanvasElement fallback
		const htmlCanvas: HTMLCanvasElement = canvas as unknown as HTMLCanvasElement;
	...
	private async flushPendingFrames(): Promise<void> {
		...
		try {
			...
			if (this.state.isBackpressured && this.backpressureMessage && this.state.lastError === this.backpressureMessage) {
				this.state.lastError = null;
				this.state.isBackpressured = false;
	...
	async takeSnapshot(): Promise<CapturedFrame | null> {
		if (!this.videoElement || !this.canvas || !this.canvasCtx || !this.state.currentSession) {
