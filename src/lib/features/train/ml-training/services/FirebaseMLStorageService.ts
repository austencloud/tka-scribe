...
export interface FirebaseMLStorageService {
	 ...
	 */
	listCloudSessions(): Promise<Array<{
		sessionId: string;
		name: string;
		createdAt: Date;
		frameCount: number;
	...
	return {
		async uploadSession(
			...
			try {
				...
				const metadataBlob = new Blob([JSON.stringify(sessionMetadata)], { type: 'application/json' });
				await uploadBytes(metadataRef, metadataBlob);

				// Upload frames metadata
				const framesMetadataRef = ref(firebaseStorage, `${basePath}/frames.json`);
				const framesBlob = new Blob([JSON.stringify(frames)], { type: 'application/json' });
				...
				for (let i = 0; i < frames.length; i++) {
					...
					const imageRef = ref(firebaseStorage, `${basePath}/images/${frame.id}.jpg`);
		...
		async listCloudSessions(): Promise<Array<{
			sessionId: string;
			name: string;
			createdAt: Date;
			frameCount: number;
			...
			try {
				...
				for (const folder of result.prefixes) {
					try {
						const metadataRef = ref(firebaseStorage, `${folder.fullPath}/metadata.json`);
						const metadataBlob = await getBlob(metadataRef);
