/**
 * CAPLabelerController
 *
 * All behavior/actions for the CAP Labeler.
 * Operates on CAPLabelerState, uses services via CAPLabelerServiceLocator.
 */

import type { CAPLabelerState } from "./cap-labeler-state.svelte";
import type { CAPLabelerServiceLocator } from "./CAPLabelerServiceLocator";
import type { LabeledSequence, FilterMode } from "../domain/models/label-models";
import type { LabelingMode } from "./cap-labeler-state.svelte";

const STORAGE_KEY = "tka-cap-labeler-state";

export class CAPLabelerController {
  constructor(
    private state: CAPLabelerState,
    private services: CAPLabelerServiceLocator
  ) {}
  ...
  persistState(): void {
    try {
      ...
      console.warn("[CAPLabelerController] Failed to persist state:", error);
    }
  }
  ...
  persistFilterMode(): void {
    try {
      ...
      console.warn("[CAPLabelerController] Failed to persist filterMode:", error);
    }
  }
  ...
  async initialize(): Promise<void> {
    ...
    if (!sequenceLoader || !labelsRepo) {
      console.error("[CAPLabelerController] Required services not available");
    ...
    try {
      ...
      console.error("[CAPLabelerController] Failed to initialize:", error);
      this.state.setLoading(false);
    }
  }
  ...
  async saveLabel(label: LabeledSequence): Promise<void> {
    ...
    try {
      ...
      console.error("[CAPLabelerController] Failed to save label:", error);
  ...
  async removeLabel(word: string): Promise<void> {
    ...
    try {
      ...
      console.error("[CAPLabelerController] Failed to delete label:", error);
      this.state.setSyncStatus("error");
    }
  }
  ...
  async importLabels(file: File): Promise<void> {
    ...
    try {
      ...
      console.error("[CAPLabelerController] Failed to import labels:", error);
    }
  }
  ...
  async syncLocalStorageToFirebase(): Promise<void> {
    ...
    try {
      ...
      console.error("[CAPLabelerController] Failed to sync:", error);
      this.state.setSyncStatus("error");
    }
  }
