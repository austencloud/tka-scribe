...
class CAPLabelerStateManager {
	...
	async initialize() {
		...
		try {
			...
			if (urlFilter && ["all", "labeled", "unlabeled", "unknown"].includes(urlFilter)) {
			...
			if (navigationService && this.currentSequence) {
				navigationService.updateUrlWithSequence(this.currentSequence.id, this.state.filterMode);
			}
		} catch (error) {
	...
	private navigateToSequenceId(sequenceId: string) {
		...
		if (targetIndex >= 0) {
			...
			if (navigationService) {
				navigationService.updateUrlWithSequence(sequenceId, this.state.filterMode);
			}
			console.log(
