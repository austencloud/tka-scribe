 ...
 * Section Mode State
 *
 * Reactive state for section-based CAP labeling.
...
export interface SectionModeState {
	...
	actions: {
		...
		removeSection(currentWord: string, index: number, notes: string): Promise<void>;
		clearSelection(): void;
...
/**
 * Create section mode state for a CAP labeler instance
 */
	...
	const actions = {
		...
		async addSection(
			...
			if (selectedBeats.size === 0) {
				console.warn("[SectionModeState] Cannot add section: no beats selected");
				return;
			}
			if (selectedComponents.size === 0 && !selectedBaseWord) {
				console.warn("[SectionModeState] Cannot add section: no components or base word selected");
				return;
			}
