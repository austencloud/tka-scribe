...
<svg viewBox="0 0 100 100" class="motion-grid">
	...
	{#each Object.entries(GRID_POINTS_4) as [key, point]}
		...
		<g class="grid-point">
			<circle cx={point.x} cy={point.y} r="3" fill="rgba(255, 255, 255, 0.25)" />
			{#if showLabels}
				<text
	...
	<circle cx={leftCurrentPos.x} cy={leftCurrentPos.y} r="6" fill={LEFT_HAND_COLOR} class="hand-point" />

	<!-- Right hand -->
	<circle cx={rightCurrentPos.x} cy={rightCurrentPos.y} r="10" fill={RIGHT_HAND_COLOR} class="hand-glow" />
	<circle cx={rightCurrentPos.x} cy={rightCurrentPos.y} r="6" fill={RIGHT_HAND_COLOR} class="hand-point" />
</svg>
