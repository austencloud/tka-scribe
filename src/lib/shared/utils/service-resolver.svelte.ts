...
export function createServiceResolver<T>(
  ...
  const { maxRetries = 5, retryDelay = 100, autoResolve = true } = options;
  ...
  function attemptResolve(): void {
    ...
    try {
      ...
      const isContainerError =
        resolveError.message.includes("Container not initialized") ||
...
export function allServicesReady(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  resolvers: Record<string, ServiceResolver<any>>
): boolean {
