...
export class MoonSystem {
  ...
  private drawMoonSurfaceProcedural(
    ctx: CanvasRenderingContext2D,
    x: number,
    y: number,
    R: number
  ...
  private drawMoonSurfaceFallback(
    ctx: CanvasRenderingContext2D,
    x: number,
    y: number,
    R: number
  ...
  private drawEarthshine(
    ...
    const earthshineGradient = ctx.createRadialGradient(
      darkSideX,
      y,
      0,
      darkSideX,
    ...
    earthshineGradient.addColorStop(
      0.5,
      `rgba(80, 90, 120, ${intensity * 0.5})`
    );
