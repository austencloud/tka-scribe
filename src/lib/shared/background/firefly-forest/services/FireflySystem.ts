...
export function createFireflySystem() {
	function createFirefly(dimensions: Dimensions, randomizePosition: boolean = true): Firefly {
		...
		const size = isSpecial ? baseSize * SPECIAL_FIREFLY.SIZE_MULTIPLIER : baseSize;
		const glowMultiplier = isSpecial ? SPECIAL_FIREFLY.GLOW_MULTIPLIER : FIREFLY_PHYSICS.GLOW_MULTIPLIER;

		const wanderAngle = Math.random() * Math.PI * 2;
		const wanderSpeed = randomInRange(FIREFLY_PHYSICS.WANDER_SPEED_BASE, FIREFLY_PHYSICS.WANDER_SPEED_RANGE);

		return {
			x: randomizePosition ? Math.random() * dimensions.width : -RESPAWN_BUFFER,
			y: randomizePosition ? zoneTop + Math.random() * zoneHeight : zoneTop + Math.random() * zoneHeight,
			size,
			vx: Math.cos(wanderAngle) * wanderSpeed,
			vy: Math.sin(wanderAngle) * wanderSpeed,
			glowRadius: size * glowMultiplier,
			glowIntensity: Math.random(),
			blinkPhase: Math.random() * (FIREFLY_PHYSICS.BLINK_CYCLE_MIN + FIREFLY_PHYSICS.BLINK_CYCLE_RANGE),
			blinkCycleLength: FIREFLY_PHYSICS.BLINK_CYCLE_MIN + Math.random() * FIREFLY_PHYSICS.BLINK_CYCLE_RANGE,
	...
	function initialize(dimensions: Dimensions, quality: QualityLevel): Firefly[] {
	...
	function update(fireflies: Firefly[], dimensions: Dimensions, frameMultiplier: number = 1.0): Firefly[] {
		const zoneTop = dimensions.height * FIREFLY_PHYSICS.ZONE_TOP;
		const zoneBottom = dimensions.height * FIREFLY_PHYSICS.ZONE_BOTTOM;
		...
		return fireflies.map((firefly) => {
			...
			const angleChange =
				(Math.random() - 0.5) * FIREFLY_PHYSICS.WANDER_ANGLE_RANGE * frameMultiplier;
	...
	function draw(fireflies: Firefly[], ctx: CanvasRenderingContext2D): void {
		for (const firefly of fireflies) {
			...
			const { x, y, size, glowRadius, glowIntensity, color, baseOpacity } = firefly;
			...
			gradient.addColorStop(0.6, `rgba(${color.r}, ${color.g}, ${color.b}, ${opacity * 0.2})`);
