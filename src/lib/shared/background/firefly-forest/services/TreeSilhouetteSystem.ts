...
export function createTreeSilhouetteSystem() {
	 ...
	 */
	function drawPine(ctx: RenderContext, x: number, baseY: number, width: number, height: number): void {
	 ...
	 */
	function drawFir(ctx: RenderContext, x: number, baseY: number, width: number, height: number): void {
		...
		ctx.lineTo(x - width * 0.30, bodyStart - height * 0.53);
	 ...
	 */
	function drawSpruce(ctx: RenderContext, x: number, baseY: number, width: number, height: number): void {
		...
		ctx.lineTo(x - width * 0.30, bodyStart - height * 0.18);
		ctx.lineTo(x - width * 0.20, bodyStart - height * 0.35);
		...
		ctx.lineTo(x - width * 0.10, bodyStart - height * 0.75);
	...
	function createTrees(dimensions: Dimensions): Tree[] {
		...
		for (const xRatio of foregroundPositions) {
			trees.push({
				...
				type: ["pine", "fir", "spruce"][Math.floor(Math.random() * 3)] as Tree["type"],
			});
		}
		...
		for (const xRatio of midPositions) {
			trees.push({
				...
				height: height * (0.30 + Math.random() * 0.15),
		...
		for (const xRatio of farPositions) {
			trees.push({
				...
				height: height * (0.18 + Math.random() * 0.10),
	...
	function handleResize(_oldDimensions: Dimensions, newDimensions: Dimensions): void {
