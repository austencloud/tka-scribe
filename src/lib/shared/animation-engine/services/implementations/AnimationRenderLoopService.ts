...
export class AnimationRenderLoopService implements IAnimationRenderLoopService {
	...
	updateConfig(config: Partial<RenderLoopConfig>): void {
		if (config.pixiRenderer !== undefined) this.pixiRenderer = config.pixiRenderer;
		if (config.trailCaptureService !== undefined) this.trailCaptureService = config.trailCaptureService;
	...
	private renderLoop = (currentTime: number): void => {
		...
		if (trailSettings.enabled && trailSettings.mode !== TrailMode.OFF && this.trailCaptureService) {
			const currentBeat = params.beatData && "beatNumber" in params.beatData
		...
		if (this.needsRender || (trailSettings.enabled && trailSettings.mode !== TrailMode.OFF)) {
	...
	private render(params: RenderFrameParams, currentTime: number): void {
		...
		const { beatData, currentBeat, trailSettings, gridVisible, gridMode, letter, props, visibility } = params;
		...
		this.pixiRenderer.renderScene({
			...
			redProp: effectivePropsVisible && effectiveRedMotionVisible ? props.redProp : null,
			secondaryBlueProp: effectivePropsVisible && effectiveBlueMotionVisible ? props.secondaryBlueProp : null,
			secondaryRedProp: effectivePropsVisible && effectiveRedMotionVisible ? props.secondaryRedProp : null,
	...
	private gatherTrailPoints(currentBeat: number): {
		...
		if (this.pathCache && this.pathCache.isValid() && currentBeat !== null) {
			...
			const blueLeft = transformTrailPoints(this.pathCache.getTrailPoints(0, 0, 0, currentBeat));
			const blueRight = transformTrailPoints(this.pathCache.getTrailPoints(0, 1, 0, currentBeat));
			blueTrailPoints = [...blueLeft, ...blueRight];

			// Red prop trails (both left and right endpoints)
			const redLeft = transformTrailPoints(this.pathCache.getTrailPoints(1, 0, 0, currentBeat));
			const redRight = transformTrailPoints(this.pathCache.getTrailPoints(1, 1, 0, currentBeat));
