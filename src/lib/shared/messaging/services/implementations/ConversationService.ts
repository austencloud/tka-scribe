...
export class ConversationService implements IConversationService {
	...
	async getOrCreateConversation(
		...
		if (existingDoc.exists()) {
			return {
				conversation: this.mapDocToConversation(existingDoc.id, existingDoc.data()),
				isNew: false
			};
		}
	...
	private mapDocToConversation(
		...
		const rawLastMessage = data["lastMessage"] as Record<string, unknown> | undefined;
		const lastMessage = rawLastMessage ? {
			...
			createdAt: (rawLastMessage["createdAt"] as Timestamp)?.toDate() || new Date(),
	...
	private async refreshParticipantInfo(conversationId: string, userId: string): Promise<void> {
		try {
			...
			await updateDoc(conversationRef, {
				...
				...(userInfo.photoURL && { [`participantInfo.${userId}.avatar`]: userInfo.photoURL })
			});
			console.log("[ConversationService] Refreshed participant info for", userId);
		} catch (error) {
			console.error("[ConversationService] Failed to refresh participant info:", error);
		}
	}
