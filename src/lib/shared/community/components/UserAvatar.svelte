<script lang="ts">
	...
	import { getRoleIcon, getRoleColor, getRoleLabel } from '$lib/shared/auth/utils/role-display-utils';
	...
	let { user, variant = 'color', size = 'md', accentColor, role, onload, onerror }: Props = $props();
...
<div
	...
	style={variant === 'color' && accentColor ? `--accent-color: ${accentColor}` : ''}
>
	<RobustAvatar
		src={imageUrl}
	...
	{#if variant === 'role' && role && role !== 'user'}
		<span class="role-badge" title={getRoleLabel(role)} style="background: {getRoleColor(role)}">
