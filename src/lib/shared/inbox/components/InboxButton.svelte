<script lang="ts">
	...
	$effect(() => {
		...
		unsubscribeMessages = conversationService.subscribeToConversations((conversations) => {
			inboxState.setConversations(conversations);
		});

		// Subscribe to notifications
		unsubscribeNotifications = notificationService.subscribeToNotifications(userId, (notifications) => {
...
<button
	...
	onclick={handleClick}
	aria-label="Open inbox{inboxState.totalUnreadCount > 0 ? `, ${inboxState.totalUnreadCount} unread` : ''}"
