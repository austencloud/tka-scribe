<script lang="ts">
	...
	import { TYPE_CONFIG, STATUS_CONFIG } from "$lib/features/feedback/domain/models/feedback-models";
	...
	const statusConfig = $derived(STATUS_CONFIG[feedbackStatus as keyof typeof STATUS_CONFIG]);
	...
	onMount(async () => {
		...
		try {
			...
			console.error("[FeedbackMessageCard] Error checking feedback existence:", err);
...
<div class="feedback-card" class:own={isOwn} class:deleted={isDeleted} class:clickable={!isDeleted && !isChecking}>
	{#if isDeleted}
		<!-- Deleted state -->
		...
		<h4 class="feedback-title deleted-title">{attachment.metadata?.feedbackTitle || "Untitled Feedback"}</h4>
		<p class="deleted-notice">This feedback item no longer exists</p>
	{:else}
		<!-- Normal state -->
		<button class="card-content" onclick={handleClick} type="button" disabled={isChecking}>
			<div class="card-header">
			...
			<h4 class="feedback-title">{attachment.metadata?.feedbackTitle || "Untitled Feedback"}</h4>

			{#if attachment.metadata?.feedbackDescription}
				<p class="feedback-preview">
					{attachment.metadata.feedbackDescription.slice(0, 120)}{attachment.metadata
						.feedbackDescription.length > 120
						? "..."
						: ""}
				</p>
			{/if}
