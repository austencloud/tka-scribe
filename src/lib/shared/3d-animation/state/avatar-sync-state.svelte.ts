...
export function createAvatarSyncState(
	 ...
	 */
	function calculateFollowerBeat(masterBeat: number, totalBeats: number): number {
		if (totalBeats === 0) return 0;
		// Add offset and wrap using modulo (handles negatives correctly)
		const result = ((masterBeat + beatOffset) % totalBeats + totalBeats) % totalBeats;
		return result;
	}
	...
	function syncFollowerBeat() {
		...
		const followerBeat = calculateFollowerBeat(master.currentBeatIndex, follower.totalBeats);
