...
const CACHE_STRATEGIES = {
	...
	static: ['/', '/app.css', '/manifest.webmanifest', '/pwa/', '/_app/immutable/'],
		...
		(async () => {
			try {
				...
				const oldCaches = cacheNames.filter(
					(name) => name.includes('tka-launcher') && !name.includes(CACHE_VERSION)
				);
...
self.addEventListener('push', (event) => {
	...
	try {
		...
		event.waitUntil(self.registration.showNotification(data.title || 'TKA Launcher', options));
	} catch (error) {
...
function matchesPattern(pathname, patterns) {
	return patterns.some((pattern) => pathname.startsWith(pattern) || pathname.includes(pattern));
}
