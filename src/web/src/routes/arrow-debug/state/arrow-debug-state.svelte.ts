...
export function createArrowDebugState(): ArrowDebugState {
	...
	const coordinateSystemService = resolve('IArrowCoordinateSystemService');
	const locationCalculator = resolve('IArrowLocationCalculator');
	const rotationCalculator = resolve('IArrowRotationCalculator');
	const adjustmentCalculator = resolve('IArrowAdjustmentCalculator');
	...
	let currentDebugData = $state<ArrowPositioningDebugData>(createEmptyDebugData());
	...
	$effect(() => {
		if (autoUpdate && selectedPictograph && currentMotionData && currentArrowData) {
		...
		try {
			...
			try {
				if (debugData.calculatedLocation) {
					debugData.initialPosition = coordinateSystemService.getInitialPosition(
						currentMotionData,
			...
			debugData.timing.stepDurations.initial_position = performance.now() - positionStart;
			...
			debugData.timing.stepDurations.rotation = performance.now() - rotationStart;
			...
			try {
				if (debugData.calculatedLocation) {
					...
					await calculateIndividualAdjustments(debugData, selectedPictograph, currentMotionData);
			...
			debugData.timing.stepDurations.adjustment = performance.now() - adjustmentStart;
	...
	function getUsedCoordinateSet(motion: MotionData): 'hand_points' | 'layer2_points' | 'center' {
		const motionType = motion.motion_type?.toLowerCase();
	...
	return {
		...
		set selectedPictograph(value) { selectedPictograph = value; },
		
		get selectedArrowColor() { return selectedArrowColor; },
		set selectedArrowColor(value) { selectedArrowColor = value; },
		
		get availablePictographs() { return availablePictographs; },
		
		get stepByStepMode() { return stepByStepMode; },
		set stepByStepMode(value) { stepByStepMode = value; },
		...
		get showCoordinateGrid() { return showCoordinateGrid; },
		set showCoordinateGrid(value) { showCoordinateGrid = value; },
		
		get showHandPoints() { return showHandPoints; },
		set showHandPoints(value) { showHandPoints = value; },
		
		get showLayer2Points() { return showLayer2Points; },
		set showLayer2Points(value) { showLayer2Points = value; },
		
		get showAdjustmentVectors() { return showAdjustmentVectors; },
		set showAdjustmentVectors(value) { showAdjustmentVectors = value; },
		
		get currentDebugData() { return currentDebugData; },
		...
		// Computed values
		get currentMotionData() { return currentMotionData; },
		get currentArrowData() { return currentArrowData; },
