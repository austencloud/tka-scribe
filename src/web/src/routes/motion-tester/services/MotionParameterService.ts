import { MotionType, Orientation, RotationDirection, Location } from "$lib/domain/enums";
...
export class MotionParameterService {
  ...
  calculateRotationDirection(motionType: string, startLoc: string, endLoc: string): string {
  ...
  mapMotionTypeToEnum(motionType: string): MotionType {
    switch (motionType.toLowerCase()) {
      ...
      case 'fl': case 'float': return MotionType.FLOAT;
      default: return MotionType.PRO;
    }
  }
  ...
  mapOrientationToEnum(orientation: string): Orientation {
    switch (orientation.toLowerCase()) {
      ...
      case 'n': return Orientation.IN; // Map cardinal directions to in/out for now
      case 'e': return Orientation.IN;
      case 's': return Orientation.IN;
      case 'w': return Orientation.IN;
  ...
  mapRotationDirectionToEnum(rotDir: string): RotationDirection {
    switch (rotDir.toLowerCase()) {
      ...
      case 'ccw': case 'counter_clockwise': case 'counterclockwise': return RotationDirection.COUNTER_CLOCKWISE;
      default: return RotationDirection.CLOCKWISE;
    }
  }
  ...
  updateMotionTypeForLocations(params: MotionTestParams): MotionTestParams {
    ...
    const availableTypes = this.getAvailableMotionTypes(params.startLoc, params.endLoc);
