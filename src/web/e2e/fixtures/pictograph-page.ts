...
export class PictographPage {
	...
	async waitForPictographLoaded(tabName = 'construct', timeout = 10000) {
		...
		await this.page.locator(`.${tabName}-tab`).waitFor({ state: 'visible', timeout });

		// Then look for pictograph elements within this tab using multiple possible selectors
		try {
			// Try the primary selector first
			const tabPictograph = this.page.locator(`.${tabName}-tab .pictograph-wrapper`).first();
			...
			if (!found) {
				console.log(`Could not find pictograph in ${tabName} tab with any selector`);
				// Take a screenshot to debug
				await this.page.screenshot({ path: `test-results/pictograph-not-found-${tabName}.png` });
			}
		}
	...
	async takeScreenshot(name: string) {
		await this.pictographWrapper.screenshot({ path: `./test-results/pictograph-${name}.png` });
	}
