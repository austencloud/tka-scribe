...
export class AppPage {
		...
		try {
			...
			try {
				...
				try {
					...
					await this.page.screenshot({ path: 'test-results/app-load-error.png' });
	...
	async navigateToTab(tabName: 'write' | 'generate' | 'construct' | 'browse' | 'learn') {
		...
		try {
			...
			const isActive = await this.page.evaluate((tab) => {
				...
				const activeButtons = Array.from(
					document.querySelectorAll('button.active, button.blue, .active-button')
				);
		...
		try {
			...
			await this.tabButtons[tabName].waitFor({ state: 'visible', timeout: 5000 });
			...
			console.log(`Could not click ${tabName} tab button using tabButtons locator:`, e);
		}
			...
			try {
				await this.page.evaluate((tab) => {
					...
					const possibleButtons = [
						...
						// By class
						...Array.from(document.querySelectorAll(`.${tab}-button, .${tab}-tab-button`)),
						// By data attribute
					...
					if (possibleButtons.length > 0) {
						console.log(`Found ${possibleButtons.length} possible buttons for ${tab} tab`);
				...
				clicked = true;
				console.log(`Clicked ${tabName} tab button using JavaScript evaluation`);
			} catch (e) {
				console.log(`Could not click ${tabName} tab button using JavaScript:`, e);
			}
		}
		...
		if (!clicked) {
			try {
				...
				console.log(`Could not click ${tabName} tab button using general selector:`, e);
				throw new Error(`Failed to navigate to ${tabName} tab: Could not click tab button`);
			}
		}

		// Take a screenshot to help debug
		try {
			await this.page.screenshot({ path: `test-results/after-${tabName}-tab-click.png` });
		} catch (e) {
		...
		try {
			...
			const tabLoaded = await this.page.evaluate((tab) => {
				...
				const activeButtons = Array.from(
					document.querySelectorAll('button.active, button.blue, .active-button, .selected')
				);
				for (const button of activeButtons) {
					if (button.textContent?.toLowerCase().includes(tab.toLowerCase())) {
			...
			if (!tabLoaded) {
				console.log(`Could not verify ${tabName} tab has loaded, continuing anyway`);

				// Take a screenshot to help debug
				try {
					await this.page.screenshot({ path: `test-results/${tabName}-tab-load-failed.png` });
				} catch (e) {
					console.log('Could not take screenshot:', e);
				}
			}
	...
	async isTabActive(tabName: 'write' | 'generate' | 'construct' | 'browse' | 'learn') {
		try {
			...
			try {
				...
				const tabContentVisible = await this.page.locator(`.${tabName}-tab`).isVisible();
	...
	async waitForAppReady() {
		...
		try {
			...
			try {
				// Take a screenshot to see what's happening
				await this.page.screenshot({ path: 'test-results/app-ready-error.png' });
			} catch (screenshotError) {
				console.log('Could not take screenshot:', screenshotError);
			}
		}
		...
		try {
			...
			if (!buttonFound) {
				console.log('Could not find any tab buttons, trying to continue anyway');

				try {
					// Take a screenshot to see what's happening
					await this.page.screenshot({ path: 'test-results/tab-buttons-not-found.png' });
				} catch (screenshotError) {
					console.log('Could not take screenshot:', screenshotError);
				}
			}
