...
test('explore DOM structure', async ({ page }) => {
	...
	const tabContents = await page.$$eval('[class*="-tab"]', (elements) => {
		return elements.map((el) => {
			...
			return {
				classes: el.className,
				isVisible: isHtmlElement ? el.offsetWidth > 0 && el.offsetHeight > 0 : true
			};
		});
	});
	...
	await page.screenshot({ path: 'test-results/generate-tab.png', fullPage: true });
	...
	console.log('Generate tab elements:', JSON.stringify(generateTabElements, null, 2));
	...
	console.log('Generator type elements:', JSON.stringify(generatorTypeElements, null, 2));
