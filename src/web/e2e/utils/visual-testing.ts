...
export class VisualTestingUtils {
	...
	static async compareSvgContent(
		...
		if (options.ignoreTransforms) {
			...
			normalizedActual = normalizedActual.replace(transformRegex, 'transform="IGNORED"');
			normalizedExpected = normalizedExpected.replace(transformRegex, 'transform="IGNORED"');
		}
	...
	static async takeComponentScreenshot(
		...
		try {
			...
			if (count === 0) {
				...
				await page.screenshot({ path: `./test-results/${screenshotName}-not-found.png` });
				...
				const found = await page.evaluate(
					(selector) => {
						...
						if (element) {
							...
							element.setAttribute('style', `${originalStyle}; border: 3px solid red !important;`);
							return true;
						}
				...
				if (found) {
					console.log(`Found component using JavaScript, taking screenshot of whole page`);
					await page.screenshot({ path: `./test-results/${screenshotName}-highlighted.png` });
				}
			...
			await componentLocator
				...
				.catch(async (e) => {
					...
					await page.screenshot({ path: `./test-results/${screenshotName}-fallback.png` });
				});
			...
			await page.screenshot({ path: `./test-results/${screenshotName}-error.png` });
		}
	}
	 ...
	 */
	static async compareComponentWithBaseline(componentLocator: Locator, baselineImagePath: string) {
			...
			for (const [attr, expectedValue] of Object.entries(expectedAttributes)) {
				try {
					...
					if (attr === 'preserveAspectRatio') {
						...
						if (actualValue?.includes('meet') && expectedValue?.includes('meet')) {
							console.log(
