...
test.describe('Sequence Generation Flow', () => {
	...
	test('should navigate to the Generate tab', async ({ page, appPage, generateTabPage }) => {
	...
	test('should generate a sequence when clicking Generate', async ({ page, generateTabPage }) => {
		// Navigate to the Generate tab
		await generateTabPage.navigateTo();
		...
		try {
			if (buttonCount > 0) {
				...
				if (isVisible) {
					...
					console.log('Generate button not visible, trying to find it in the DOM');
