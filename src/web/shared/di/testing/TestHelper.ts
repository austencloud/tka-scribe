...
export class TKAWebTestHelper {
    ...
    async createSequence(name: string, length: number): Promise<TestResult> {
        ...
        try {
            ...
            const sequenceService = this._container.tryResolve(
                defineService('ISequenceManagementService', class {})
            );
            ...
            const sequence = {
                ...
                name,
                length,
                beats: Array(length).fill(null).map((_, i) => ({ beatNumber: i + 1 })),
                createdAt: new Date()
            };
     ...
     */
    async createBeatWithMotions(beatNumber: number, letter: string): Promise<TestResult> {
        const startTime = performance.now();
    ...
    async testServiceDependencyInjection(): Promise<TestResult> {
        ...
        try {
            ...
            const IRepositoryService = defineService('IRepositoryService', class {
                save(data: any): any { return data; }
            });

            const IBusinessService = defineService('IBusinessService', class {
                process(data: any): any { return data; }
            });

            // Register services
            this._container.registerSingleton(IRepositoryService, class TestRepository {
                save(data: any): any {
            ...
            const isValid = result.test === 'data' && result.processed === true && result.saved === true;

            return {
                success: isValid,
                message: isValid ? 'Dependency injection working correctly' : 'Dependency injection failed',
                details: result,
                duration: performance.now() - startTime
            };
    ...
    async testContainerIsolation(): Promise<TestResult> {
        ...
        try {
            ...
            const ITestService = defineService('ITestService', class {
                getValue(): string { return ''; }
                setValue(value: string): void {}
            });

            // Register different implementations
            container1.registerSingleton(ITestService, class TestService1 {
                private value = 'container1';
                getValue(): string { return this.value; }
                setValue(value: string): void { this.value = value; }
            });

            container2.registerSingleton(ITestService, class TestService2 {
            ...
            const isIsolated = value1 !== value2 && value1 === 'container1' && value2 === 'container2';
            ...
            return {
                success: isIsolated,
                message: isIsolated ? 'Container isolation working correctly' : 'Container isolation failed',
    ...
    private async _testContainerCreation(): Promise<TestResult> {
        ...
        try {
            ...
            const isValid = !!diagnostics.containerId && diagnostics.containerId.length > 0;

            return {
                success: isValid,
                message: isValid ? 'Container created successfully' : 'Container creation failed',
                details: diagnostics,
                duration: performance.now() - startTime
            };
        } catch (error) {
    ...
    private async _testServiceRegistration(): Promise<TestResult> {
        ...
        try {
            ...
            this._container.registerSingleton(ITestService, class TestServiceImpl {
                test(): string { return 'working'; }
            });
            ...
            return {
                success: isValid,
                message: isValid ? 'Service registration and resolution working' : 'Service registration failed',
                details: { result },
                duration: performance.now() - startTime
            };
        } catch (error) {
    ...
    private async _testLifecycleManagement(): Promise<TestResult> {
        ...
        try {
            ...
            const ILifecycleService = defineService('ILifecycleService', class {
                test(): string { return 'test'; }
            });

            this._container.registerSingleton(ILifecycleService, class LifecycleService {
                initialize(): void {
    ...
    private async _testScopingAndIsolation(): Promise<TestResult> {
        ...
        try {
            ...
            this._container.registerScoped(IScopedService, class ScopedService {
            ...
            return {
                success: isIsolated,
                message: isIsolated ? 'Scoping and isolation working' : 'Scoping failed',
                details: { value1, value2 },
                duration: performance.now() - startTime
            };
        } catch (error) {
    ...
    private async _testLazyLoading(): Promise<TestResult> {
        ...
        try {
            ...
            const ILazyService = defineService('ILazyService', class {
                test(): string { return 'test'; }
            });

            this._container.registerLazy(ILazyService, class LazyService {
                constructor() {
                    instantiated = true;
                }
            ...
            const isLazy = !wasInstantiatedBeforeUse && wasInstantiatedAfterUse && result === 'lazy';

            return {
                success: isLazy,
                message: isLazy ? 'Lazy loading working correctly' : 'Lazy loading failed',
    ...
    private async _testErrorHandling(): Promise<TestResult> {
        ...
        try {
            ...
            return {
                success: errorCaught,
                message: errorCaught ? 'Error handling working correctly' : 'Error handling failed',
                duration: performance.now() - startTime
            };
        } catch (error) {
            return {
    ...
    private async _testPerformanceMetrics(): Promise<TestResult> {
        ...
        try {
            ...
            const IMetricsService = defineService('IMetricsService', class {
                test(): string { return 'test'; }
            });

            this._container.registerSingleton(IMetricsService, class MetricsService {
                test(): string { return 'metrics'; }
            });
            ...
            return {
                success: hasMetrics,
                message: hasMetrics ? 'Performance metrics working' : 'Performance metrics failed',
                details: metrics,
                duration: performance.now() - startTime
            };
        } catch (error) {
    ...
    private _logTestSuiteResults(result: TestSuiteResult): void {
        ...
        if (result.failedTests > 0) {
            console.log('\n❌ Failed Tests:');
            result.results.filter(r => !r.success).forEach((test, index) => {
