...
export function processSequenceWithOrientationMappings(sequenceData: SequenceData): SequenceData {
	console.log('Processing sequence with finalized orientation mappings...');
	...
	console.log(`Processed ${processedSteps.length} sequence steps with orientation mappings`);
...
function applyOrientationMappingToAttributes(
	...
	if (!attributes.start_ori || !attributes.end_ori) {
		console.warn(`${context}: Missing orientation data, skipping orientation mapping`);
		return attributes;
	}
	...
	console.log(`${context}: ${attributes.start_loc}(${attributes.start_ori}) = ${(startAngleRadians * 180 / Math.PI).toFixed(1)}°, ${attributes.end_loc}(${attributes.end_ori}) = ${(endAngleRadians * 180 / Math.PI).toFixed(1)}°`);
...
export function generateOrientationProcessingReport(
	...
	for (let i = 0; i < Math.min(originalSteps.length, processedSteps.length); i++) {
		...
		report.push(`- Position: ${originalStep.blue_attributes.start_loc} → ${originalStep.blue_attributes.end_loc}`);
		report.push(`- Orientation: ${originalStep.blue_attributes.start_ori} → ${originalStep.blue_attributes.end_ori}`);
		...
		report.push(`- Position: ${originalStep.red_attributes.start_loc} → ${originalStep.red_attributes.end_loc}`);
		report.push(`- Orientation: ${originalStep.red_attributes.start_ori} → ${originalStep.red_attributes.end_ori}`);
