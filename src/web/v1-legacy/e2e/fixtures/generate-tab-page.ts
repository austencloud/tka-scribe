...
export class GenerateTabPage {
	...
	async navigateTo() {
		...
		try {
			...
			const clicked = await this.page.evaluate(() => {
				...
				const possibleButtons = [
					...
					// By class
					...Array.from(document.querySelectorAll('.generate-button, .generate-tab-button')),
					// By data attribute
				...
				if (possibleButtons.length > 0) {
					console.log(`Found ${possibleButtons.length} possible buttons for Generate tab`);
			...
			if (clicked) {
				...
				for (const selector of selectors) {
					try {
						...
						if (count > 0) {
							...
							console.log(`Clicked Generate tab button using selector: ${selector}`);
							break;
						}
					} catch (e) {
				...
				if (!selectorClicked) {
					...
					await this.page.screenshot({ path: 'test-results/generate-tab-navigation-failed.png' });
		...
		try {
			...
			console.log('Could not find Generate tab content, trying alternative approaches');
			...
			if (!tabLoaded) {
				console.log('Could not verify Generate tab has loaded, continuing anyway');

				// Take a screenshot to debug
				await this.page.screenshot({ path: 'test-results/generate-tab-content-not-found.png' });
			}
		}
	...
	async setPropContinuity(value: 'continuous' | 'discontinuous') {
		...
		const isSelected = await this.page.locator('.toggle-label.selected').textContent();
	...
	async setCapType(value: 'mirrored' | 'matched' | 'random') {
		...
		await this.page.locator(`.cap-button:has-text("${buttonTextMap[value]}")`).first().click();
	}
	...
	async generateSequence() {
		...
		try {
			...
			console.log('Could not detect "Ready" indicator, waiting for sequence output instead');
	...
	async dragSequenceToWriteTab() {
		try {
			...
			await this.page.locator('.write-tab').waitFor({ state: 'visible', timeout: 15000 });
			...
			await this.page.screenshot({ path: 'test-results/before-sequence-click.png' });
			...
			await this.page.screenshot({ path: 'test-results/drag-sequence-error.png' });
			throw e;
		}
	}
}
