...
test.describe('Pictograph Component', () => {
	...
	test('should render pictograph components', async ({ page, appPage }) => {
		...
		await page.locator('.construct-tab').waitFor({ state: 'visible', timeout: 10000 });
		...
		if (svgCount > 0) {
			...
			for (let i = 0; i < svgCount; i++) {
				...
				const svgInfo = await svg.evaluate((el: SVGElement) => ({
					...
					childElements: Array.from(el.querySelectorAll('*')).map((child: Element) => ({
						tag: child.tagName,
						classes:
							child instanceof SVGElement && typeof child.className !== 'string'
				...
				if (circleCount > 0) {
					...
					await page.screenshot({ path: `test-results/after-circle-click-${i}.png` });
				}
			}
		}
	...
	test.skip('should render props when added to the pictograph', async ({ page, appPage }) => {
	...
	test.skip('should render arrows when motion is added', async ({ page, appPage }) => {
		// This test is skipped until we can determine the exact selectors needed
		// for your specific implementation
