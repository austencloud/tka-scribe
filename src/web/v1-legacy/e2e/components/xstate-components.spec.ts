...
test.describe('XState-Driven Components', () => {
	...
	test('should transition through states during sequence generation', async ({
		...
		try {
			const generatingState = page.locator('.indicator-label:has-text("Generating")');
			await expect(generatingState).toBeVisible({ timeout: 5000 });
		} catch (e) {
			console.log('Could not detect "Generating" indicator, checking for "Ready" instead');
	...
	test('should handle error states gracefully', async ({ page, generateTabPage }) => {
		// Navigate to the Generate tab
		await generateTabPage.navigateTo();
		...
		await page.evaluate(() => {
			...
			if (win.sequenceActor) {
				win.sequenceActor.send({ type: 'GENERATION_ERROR', error: 'Test error' });
			} else {
		...
		const retryButton = page.locator('button:has-text("Retry"), [data-test="retry-button"]');
