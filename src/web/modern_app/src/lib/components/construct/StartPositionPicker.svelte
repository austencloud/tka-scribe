  ...
  {:else}
    <div class="pictograph-row">
      {#each startPositionPictographs as pictograph (pictograph.id)}
        ...
        >
          <!-- Render pictograph using modern service -->
          <div class="pictograph-wrapper">
            {#if pictographRenderingService}
              {#await pictographRenderingService.renderPictograph(pictograph)}
              <div class="pictograph-loading">Loading...</div>
            {:then svgElement}
              <!-- Insert the rendered SVG -->
              <div class="pictograph-svg-container">
                {@html svgElement.outerHTML}
              </div>
            {:catch error}
              <div class="pictograph-error">
                <p>Error rendering pictograph</p>
                <small>{error.message}</small>
              </div>
            {/await}
            {:else}
              <div class="pictograph-loading">Service not ready...</div>
            {/if}
          </div>
