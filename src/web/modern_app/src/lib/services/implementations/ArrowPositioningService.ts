...
export class ArrowPositioningService implements IArrowPositioningService {
	...
	private motionToArrowData(motion: MotionData, color: 'blue' | 'red'): ArrowData {
		...
		return {
			...
			location,
			startOrientation: this.normalizeOrientation(motion.startOrientation),
			endOrientation: this.normalizeOrientation(motion.endOrientation),
	...
	private extractLocationFromMotion(motion: MotionData): Location {
		// Use startLocation if available, otherwise default to center
		if (motion.startLocation) {
			return this.normalizeLocation(motion.startLocation);
		}
