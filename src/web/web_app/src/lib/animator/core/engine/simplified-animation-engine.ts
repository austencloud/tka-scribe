...
import { adaptSequenceData, extractStepsFromUnified } from '../../types/core.js';
...
export class SimplifiedAnimationEngine {
	...
	initialize(data: AnySequenceData): boolean {
		try {
			...
			console.log('SimplifiedAnimationEngine: Adapted to unified format:', this.sequenceData);
			...
			console.log('SimplifiedAnimationEngine: Extracted steps:', this.steps.length);
	...
	calculateState(beat: number): void {
		...
		if (this.sequenceData.start_position?.pictograph_data?.motions) {
			...
			const startMotions = this.sequenceData.start_position.pictograph_data.motions;
			startPosition = {
				beat: 0,
				blue_attributes: {
	...
	private updateCoordinatesFromAngle(propState: PropState): void {
		...
		propState.x =
			centerX +
			Math.cos(propState.centerPathAngle) * this.GRID_HALFWAY_POINT_OFFSET * inwardFactor;
		propState.y =
			centerY +
			Math.sin(propState.centerPathAngle) * this.GRID_HALFWAY_POINT_OFFSET * inwardFactor;
	}
