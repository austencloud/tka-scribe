...
export function slideTransition(
    node: Element,
    params: { direction?: 'up' | 'down' | 'left' | 'right'; duration?: number; delay?: number } = {}
): TransitionReturn {
    ...
    const getTransform = (progress: number) => {
        ...
        switch (direction) {
            case 'up': return `translateY(${movement}px)`;
            case 'down': return `translateY(-${movement}px)`;
            case 'left': return `translateX(${movement}px)`;
            case 'right': return `translateX(-${movement}px)`;
            default: return `translateY(${movement}px)`;
        }
    };
...
export class ModeTransitionOrchestrator {
    ...
    private async executeFoldTransition(direction: 'fold-in' | 'fold-out'): Promise<void> {
