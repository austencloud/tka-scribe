 ...
 */
export interface FileSystemDirectoryHandleWithPermissions
...
export function saveDirHandleToLocalStorage(
  ...
  try {
    ...
    logger.info(
      `Directory handle metadata saved to localStorage: ${handle.name}`,
    );
  } catch (error) {
    logger.error(
...
export function createExportFileName(
  ...
  const categoryPrefix = category
    ? `${createCategoryFolderName(category)}_`
    : "";
