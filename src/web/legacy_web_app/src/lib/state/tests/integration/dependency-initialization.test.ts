...
describe("Dependency-Aware Initialization", () => {
  ...
  it("initializes state containers in the correct order based on dependencies", () => {
    ...
    const baseStore = createStore<
      ...
      { initialize: () => void }
    >("baseStore", { isInitialized: false }, (set) => ({
      initialize: () => {
    ...
    const dependentStore1 = createStore<
      { isInitialized: boolean },
      { initialize: () => void }
    >("dependentStore1", { isInitialized: false }, (set) => ({
      initialize: () => {
    ...
    const dependentStore2 = createStore<
      { isInitialized: boolean },
      { initialize: () => void }
    >("dependentStore2", { isInitialized: false }, (set) => ({
      initialize: () => {
