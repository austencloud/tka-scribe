...
describe("Component Interaction with State", () => {
      ...
      >(
        ...
        (set) => ({
          setActiveTab: (tabIndex: number) =>
            set({ ...get(uiStateStore), activeTab: tabIndex }),
        ...
        (set) => ({
          updateField: (field: keyof FormState["fields"], value: string) => {
            ...
            if (field === "name" && (!value || value.length < 2)) {
              ...
              field === "age" &&
              (!value || isNaN(Number(value)) || +value < 18)
            ) {
              valid = false;
            ...
            const formValid = Object.values(newFields).every(
              (field) => field.valid,
            );
          ...
          setSubmitting: (submitting: boolean) =>
            set({ ...get(formStore), submitting }),
          setSubmitted: (submitted: boolean) =>
            set({ ...get(formStore), submitted }),
        }),
      );
  ...
  describe("UI Component Integration with XState", () => {
    ...
    it("synchronizes UI state with XState machine", async () => {
      ...
      const wizardActor = stateRegistry.registerMachine(
        "wizardMachine",
        wizardMachine,
      );
      ...
      const contextAfterStep1 = wizardActor.getSnapshot()
      ...
      const formData = (wizardActor.getSnapshot().context as WizardContext)
        .formData;
