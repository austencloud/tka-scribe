...
describe("Store Factory", () => {
  ...
  describe("createStore", () => {
    ...
    it("tracks subscriptions for cleanup", () => {
      ...
      expect(trackSubscriptionSpy).toHaveBeenCalledWith(
        "subscriptionStore",
        expect.any(Function),
      );
  ...
  describe("createPersistentStore", () => {
    ...
    it("creates a store that persists to localStorage", () => {
      const persistentStore = createPersistentStore("persistentStore", {
        count: 0,
      });
    ...
    it("restores state from localStorage on initialization", () => {
      ...
      localStorage.setItem(
        "existingStore",
        JSON.stringify({ value: "restored" }),
      );

      // Create store with the same ID
      const store = createPersistentStore("existingStore", {
        value: "initial",
      });
    ...
    it("supports selective field persistence", () => {
      ...
      localStorage.setItem(
        "selectiveStore",
        JSON.stringify({ persist: "restored" }),
      );
