...
function createBackgroundStoreAdapter(): Writable<BackgroundState> & {
  ...
  const { subscribe, set } = writable<BackgroundState>(
  ...
  if (
    ...
    const unsubscribe = (backgroundContainer as any).subscribe(
  ...
  return {
    ...
    update: (updater: (value: BackgroundState) => BackgroundState) => {
      ...
      if (newValue.performanceMetrics) {
        backgroundContainer.updatePerformanceMetrics(
          newValue.performanceMetrics,
        );
      }
