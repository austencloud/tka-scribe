...
const createLearnStore = () => {
  ...
  return {
    ...
    submitAnswer: (answer: any) =>
      update((state) => {
        const isCorrect = checkAnswerLogic(
          answer,
          state.correctAnswer,
          state.lessonConfig,
        );
    ...
    nextQuestionOrEnd: () =>
      update((state) => {
        ...
        if (
          ...
          nextIndex >= state.totalQuestions
        ) {
          if (timerInterval) {
            clearInterval(timerInterval);
            timerInterval = null;
...
export const currentQuestion = derived(
  learnStore,
  ($store) => $store.currentQuestionData,
);

export const currentAnswers = derived(
  learnStore,
  ($store) => $store.currentAnswerOptions,
);
...
export const quizResults = derived(learnStore, ($store) => ({
  score: $store.score,
  total:
    $store.selectedMode === "fixed_question"
      ? $store.totalQuestions
      : $store.lessonHistory.length,
