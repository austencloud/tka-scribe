...
export class RestartStrategy implements SupervisionStrategy {
  ...
  async handleError(
    ...
    if (
      this.config.maxRestarts !== undefined &&
      this.config.withinTimeWindow !== undefined
    ) {
      if (
        this.lastRestartTime &&
        now - this.lastRestartTime > this.config.withinTimeWindow
      ) {
        this.restartCount = 1;
      }
  ...
  private calculateBackoffDelay(): number {
    if (
      !this.config.backoffType ||
      this.config.backoffType === BackoffType.NONE
    ) {
      return 0;
    }
    ...
    switch (this.config.backoffType) {
      ...
      case BackoffType.EXPONENTIAL:
        return Math.min(
          baseDelay * Math.pow(factor, this.restartCount - 1),
          maxDelay,
        );
