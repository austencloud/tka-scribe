...
export function initializeStateManagement(): void {
  ...
  const registeredContainers = stateRegistry
    .getAll()
  ...
  if (!hasSequenceActor || !hasAppActor) {
    try {
      ...
      const updatedContainers = stateRegistry
        .getAll()
        .map((container) => container.id);
  ...
  for (const id of initOrder) {
    ...
    if (
      container &&
      "getSnapshot" in container &&
      typeof (container as any).start === "function"
    ) {
