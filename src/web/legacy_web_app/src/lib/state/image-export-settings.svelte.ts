...
export function getImageExportSettings(): ImageExportSettings {
  return {
    ...
    customNote:
      imageExportState.customNote || defaultImageExportSettings.customNote,
    rememberLastSaveDirectory:
      imageExportState.rememberLastSaveDirectory === true,
    lastSaveDirectory: imageExportState.lastSaveDirectory || "",
    useCategories: imageExportState.useCategories === true,
    defaultCategory:
      imageExportState.defaultCategory ||
      defaultImageExportSettings.defaultCategory,
  };
}

// Function to update the settings
export function updateImageExportSettings(
  newSettings: Partial<ImageExportSettings>,
): void {
  const cleanSettings = {
    ...imageExportState,
    ...newSettings,
  ...
  cleanSettings.rememberLastSaveDirectory =
    cleanSettings.rememberLastSaveDirectory === true;
  // Start position is now always included
    ...
    if (savedSettings) {
      try {
        ...
        validatedSettings.defaultCategory =
          typeof parsed.defaultCategory === "string" &&
          parsed.defaultCategory.trim() !== ""
        ...
        console.error(
          "Failed to parse image export settings, using defaults",
          parseError,
        );
...
export function saveImageExportSettings(): void {
  try {
    ...
    if (!imageExportState) {
      console.error(
        "Cannot save image export settings: settings object is null or undefined",
      );
      return;
    }
    ...
    const cleanSettings: ImageExportSettings = {
      ...
      addReversalSymbols: imageExportState.addReversalSymbols === true,
      userName:
        imageExportState.userName || defaultImageExportSettings.userName,
      customNote:
        imageExportState.customNote || defaultImageExportSettings.customNote,
      rememberLastSaveDirectory:
        imageExportState.rememberLastSaveDirectory === true,
