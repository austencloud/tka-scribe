...
class AutoMockPatcher:
    ...
    def _patch_widget_pool_managers(self):
        ...
        # OptionPickerWidgetPoolManager has been removed - no longer needed
    ...
    def _patch_di_container(self):
        ...
        # Patch DIContainer
        patcher = patch(
            "core.dependency_injection.di_container.DIContainer", DIContainerMock
        )
        self.active_patches.append(patcher)
        mock = patcher.start()
...
def mock_section_manager():
    ...
    from desktop.modern.tests.fixtures.test_isolation import (
        create_section_mock_with_pictographs,
    )
...
def patch_data_cache_manager():
    ...
    return TemporaryMockPatch(
        "application.services.data.cache_manager.DataCacheManager", DataCacheManagerMock
    )
...
def patch_widget_pool_manager():
    """Context manager to patch OptionPickerWidgetPoolManager - DEPRECATED: Widget pool manager removed."""
    # Return a no-op context manager since the widget pool manager no longer exists
