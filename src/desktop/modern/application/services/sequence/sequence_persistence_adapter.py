...
class SequencePersistenceAdapter:
    ...
    def _convert_beats_to_legacy(self, sequence: SequenceData) -> List[Dict[str, Any]]:
        ...
        for i, beat in enumerate(sequence.beats):
            try:
                ...
                print(f"❌ [PERSISTENCE] Failed to convert beat {i} ({beat.letter}): {e}")
                import traceback
                traceback.print_exc()
        
        return legacy_beats
    
    def _create_sequence_metadata(self, sequence: SequenceData, word: str = "") -> Dict[str, Any]:
        """
    ...
    def preserve_start_position_from_existing(self, existing_sequence: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
