...
class BeatOperationCoordinator(QObject):
    ...
    def add_beat_to_sequence(self, beat_data: BeatData) -> None:
        ...
        try:
            ...
            if current_sequence is None:
                ...
                print(f"🎯 [COORDINATOR] Creating empty sequence")
            ...
            if self.workbench_setter:
                print(f"🎯 [COORDINATOR] Updating workbench with sequence length: {len(new_sequence.beats)}")
            ...
            print(f"🎯 [COORDINATOR] Emitting beat_added signal for position: {position}")
    ...
    def update_beat_turns(self, beat_index: int, color: str, new_turns: int) -> None:
        ...
        try:
            ...
            print(f"✅ Updated {color} turns for beat {updated_beat.letter} to {new_turns}")
    ...
    def update_beat_orientation(self, beat_index: int, color: str, new_orientation: int) -> None:
        ...
        try:
            ...
            print(f"✅ Updated {color} orientation for beat {updated_beat.letter} to {new_orientation}")
    ...
    def remove_beat(self, beat_index: int) -> None:
        ...
        try:
            current_sequence = self.get_current_sequence()
            if not current_sequence:
                print(f"⚠️ Cannot remove beat at index {beat_index}: no current sequence")
                return
            ...
            updated_sequence = self.sequence_service.remove_beat(current_sequence, beat_index)
            ...
            print(f"✅ [COORDINATOR] Successfully removed beat {beat_to_remove.letter} from position {beat_index}")
    ...
    def delete_beat(self, sequence: SequenceData, beat_index: int) -> SequenceData:
        ...
        """
        try:
            print(f"🗑️ [COORDINATOR] Deleting beat at index {beat_index} and all following beats")
            
            # Use focused service to delete beat and following
            updated_sequence = self.sequence_service.delete_beat_and_following(sequence, beat_index)
            
            print(f"✅ [COORDINATOR] Successfully deleted beat and following beats")
            print(f"📊 [COORDINATOR] Sequence now has {len(updated_sequence.beats)} beats")
    ...
    def get_current_sequence(self) -> Optional[SequenceData]:
        ...
        """
        print(f"🔍 [COORDINATOR] Getting current sequence...")
