...
class ConstructTabSignalCoordinator(QObject):
    ...
    beat_added = pyqtSignal(str, object, int, int)  # sequence_id, beat_data, position, total_beats
    beat_updated = pyqtSignal(str, int, str, object, object)  # sequence_id, beat_number, field, old_value, new_value
    beat_removed = pyqtSignal(str, int, int)  # sequence_id, beat_number, remaining_beats
    ...
    layout_transition_requested = pyqtSignal(str)  # target_view ("start_position_picker", "option_picker", etc.)
    ...
    def emit_beat_added(self, sequence_id: str, beat_data: BeatData, position: int, total_beats: int) -> None:
        """Emit beat addition signal."""
        self.beat_added.emit(sequence_id, beat_data, position, total_beats)
        self.logger.info(f"Emitted beat_added: sequence {sequence_id}, position {position}")
    
    def emit_beat_updated(self, sequence_id: str, beat_number: int, field: str, old_value: Any, new_value: Any) -> None:
        """Emit beat update signal."""
        self.beat_updated.emit(sequence_id, beat_number, field, old_value, new_value)
        self.logger.info(f"Emitted beat_updated: sequence {sequence_id}, beat {beat_number}, field {field}")
    
    def emit_beat_removed(self, sequence_id: str, beat_number: int, remaining_beats: int) -> None:
        """Emit beat removal signal."""
        self.beat_removed.emit(sequence_id, beat_number, remaining_beats)
        self.logger.info(f"Emitted beat_removed: sequence {sequence_id}, beat {beat_number}")
    ...
    def emit_option_picker_selection(self, pictograph_data: Any) -> None:
        ...
        self.logger.info(f"Emitted option_picker_selection: {getattr(pictograph_data, 'letter', 'unknown')}")
    
    def emit_component_visibility_changed(self, component_name: str, visible: bool) -> None:
        """Emit component visibility change signal."""
        self.component_visibility_changed.emit(component_name, visible)
        self.logger.info(f"Emitted component_visibility_changed: {component_name} = {visible}")
