...
class SignalIntegrationAdapter(QObject):
    ...
    def __init__(self, signal_coordinator: Optional[ConstructTabSignalCoordinator] = None):
        super().__init__()
    ...
    def _connect_coordinator_signals(self):
        ...
        # UI signals
        self.signal_coordinator.start_position_selected.connect(self._handle_start_position_selected)
        self.signal_coordinator.option_picker_selection.connect(self._handle_option_picker_selection)
        
        # Layout signals
        self.signal_coordinator.layout_recalculated.connect(self._handle_layout_recalculated)
        self.signal_coordinator.component_resized.connect(self._handle_component_resized)
        self.signal_coordinator.layout_transition_requested.connect(self._handle_layout_transition)
    ...
    def _handle_beat_added(self, sequence_id: str, beat_data: Any, position: int, total_beats: int):
        """Handle beat addition signal."""
        self.logger.info(f"Handling beat added: sequence {sequence_id}, position {position}")
        
        # Update option picker
        option_picker_manager = self.components.get("option_picker_manager")
        if option_picker_manager and hasattr(option_picker_manager, "refresh_from_beat_addition"):
        ...
        layout_manager = self.components.get("layout_manager")
        if layout_manager and total_beats > 0 and hasattr(layout_manager, "transition_to_option_picker"):
            layout_manager.transition_to_option_picker()
    
    def _handle_beat_updated(self, sequence_id: str, beat_number: int, field: str, old_value: Any, new_value: Any):
        """Handle beat update signal."""
        self.logger.info(f"Handling beat updated: sequence {sequence_id}, beat {beat_number}")
    ...
    def _handle_beat_removed(self, sequence_id: str, beat_number: int, remaining_beats: int):
        """Handle beat removal signal."""
        self.logger.info(f"Handling beat removed: sequence {sequence_id}, beat {beat_number}")
        
        if remaining_beats == 0:
            # No beats left, transition to start position picker
            layout_manager = self.components.get("layout_manager")
            if layout_manager and hasattr(layout_manager, "transition_to_start_position_picker"):
    ...
    def _handle_start_position_selected(self, position_key: str):
        ...
        option_picker_manager = self.components.get("option_picker_manager")
        if option_picker_manager and hasattr(option_picker_manager, "prepare_from_start_position"):
    ...
    def _handle_option_picker_selection(self, pictograph_data: Any):
        ...
        self.logger.info(f"Handling option picker selection: {getattr(pictograph_data, 'letter', 'unknown')}")
    ...
    def _handle_layout_transition(self, target_view: str):
        ...
        layout_manager = self.components.get("layout_manager")
        if layout_manager:
            if target_view == "option_picker" and hasattr(layout_manager, "transition_to_option_picker"):
                layout_manager.transition_to_option_picker()
            elif target_view == "start_position_picker" and hasattr(layout_manager, "transition_to_start_position_picker"):
                layout_manager.transition_to_start_position_picker()
    ...
    def publish_beat_added(self, beat_data, position, sequence):
        ...
        self.signal_coordinator.emit_beat_added(sequence_id, beat_data, position, total_beats)
    
    def publish_beat_updated(self, sequence_id, beat_number, field, old_value, new_value):
        """Publish beat update (emits Qt signal)."""
        self.signal_coordinator.emit_beat_updated(sequence_id, beat_number, field, old_value, new_value)
