...
def validate_service_functionality():
    ...
    try:
        ...
        basic_positions = ui_service.get_positions_for_mode("diamond", is_advanced=False)
        advanced_positions = ui_service.get_positions_for_mode("diamond", is_advanced=True)
        ...
        if len(advanced_positions) == 16:
            ...
            logger.error(f"❌ Advanced positions count incorrect: {len(advanced_positions)}")
            return False
