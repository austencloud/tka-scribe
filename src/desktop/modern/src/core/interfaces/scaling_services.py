...
try:
    ...
    QSize = type('QSize', (), {'width': lambda self: 0, 'height': lambda self: 0})
...
class IScalingCalculator(ABC):
    ...
    @abstractmethod
    def clamp_scale_factor(self, scale: float, min_scale: float, max_scale: float) -> float:
        """
...
class IVisualPresenter(ABC):
    ...
    def calculate_optimal_viewing_scale(
        self, content_size: Tuple[int, int], viewer_size: Tuple[int, int], context: ScalingContext
    ) -> float:
        """
