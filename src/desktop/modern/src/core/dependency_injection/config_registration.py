...
from core.config.app_config import AppConfig, PositioningConfig, UIConfig, LoggingConfig, create_app_config
...
def register_configurations(container, config_override: Optional[AppConfig] = None) -> Result[bool, AppError]:
    """
    ...
    try:
        ...
        container.register_factory(
            DataService,
            lambda: DataService(app_config.data_config)
        )
...
def register_data_config_only(container, data_config: Optional[DataConfig] = None) -> Result[bool, AppError]:
    """
...
def register_positioning_services_with_config(container, positioning_config: Optional[PositioningConfig] = None) -> Result[bool, AppError]:
    """
    ...
    try:
        ...
        from application.services.positioning.arrows.orchestration.arrow_adjustment_lookup_service import ArrowAdjustmentLookupService
        from application.services.positioning.arrows.orchestration.directional_tuple_processor import DirectionalTupleProcessor
        from application.services.positioning.arrows.orchestration.arrow_adjustment_calculator_service import ArrowAdjustmentCalculatorService
        
        # Register the focused services
        container.register_singleton(ArrowAdjustmentLookupService, ArrowAdjustmentLookupService)
        container.register_singleton(DirectionalTupleProcessor, DirectionalTupleProcessor)
        container.register_singleton(ArrowAdjustmentCalculatorService, ArrowAdjustmentCalculatorService)
...
def create_configured_container(config_override: Optional[AppConfig] = None):
    ...
    if positioning_result.is_failure():
        raise Exception(f"Failed to register positioning services: {positioning_result.error}")
...
def validate_configuration_registration(container) -> Result[bool, AppError]:
    ...
    """
    try:
        required_configs = [AppConfig, DataConfig, PositioningConfig, UIConfig, LoggingConfig]
