...
class TKAAPIException(Exception):
    ...
    def __init__(self, message: str, error_code: str = "TKA_ERROR", details: Optional[Dict[str, Any]] = None):
...
class ConversionError(TKAAPIException):
    ...
    def __init__(self, message: str, source_type: str, target_type: str, details: Optional[Dict[str, Any]] = None):
...
class OperationError(TKAAPIException):
    ...
    def __init__(self, operation: str, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(f"Operation '{operation}' failed: {message}", "OPERATION_ERROR", details)
...
async def tka_api_exception_handler(request: Request, exc: TKAAPIException) -> JSONResponse:
    """
    ...
    logger.error(f"TKA API Error {status_code}: {exc.message} - Path: {request.url.path}")
