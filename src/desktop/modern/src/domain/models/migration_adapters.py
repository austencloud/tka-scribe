...
try:
    from ..core_models import BeatData as OldBeatData
    from ..core_models import Location as OldLocation
    from ..core_models import MotionData as OldMotionData
    from ..core_models import MotionType as OldMotionType
    from ..core_models import Orientation as OldOrientation
    from ..core_models import RotationDirection as OldRotationDirection
    from ..core_models import SequenceData as OldSequenceData
...
from .core_models import BeatData as NewBeatData
from .core_models import MotionData as NewMotionData
from .core_models import SequenceData as NewSequenceData
...
class ModelMigrationAdapter:
    ...
    def new_motion_to_old(cls, new_motion: NewMotionData) -> "OldMotionData":
        ...
        return OldMotionData(
            ...
            prop_rot_dir=cls.string_to_enum(
                new_motion.prop_rot_dir, OldRotationDirection
            ),
...
class GradualMigrationHelper:
    ...
    def create_motion_data(self, **kwargs) -> "MotionData":
        ...
        if self.use_new_models:
            ...
            return NewMotionData(**kwargs)  # Simplified for now

    def create_beat_data(self, **kwargs) -> "BeatData":
        """Create beat data using current model preference."""
        if self.use_new_models:
            return NewBeatData(**kwargs)
        else:
            if not OLD_MODELS_AVAILABLE:
                return NewBeatData(**kwargs)
            return NewBeatData(**kwargs)  # Simplified for now

    def create_sequence_data(self, **kwargs) -> "SequenceData":
        """Create sequence data using current model preference."""
        if self.use_new_models:
            return NewSequenceData(**kwargs)
        else:
            if not OLD_MODELS_AVAILABLE:
                return NewSequenceData(**kwargs)
            return NewSequenceData(**kwargs)  # Simplified for now
