...
class ArrowAdjustmentCalculatorService(IArrowAdjustmentCalculator):
    ...
    def calculate_adjustment(
        ...
        logger.info(
            f"   Step 4 - Final adjustment: ({final_adjustment.x():.1f}, {final_adjustment.y():.1f})"
        )
    ...
    def _lookup_base_adjustment(
        ...
        try:
            ...
            if special_adjustment is not None:
                logger.info(
                    f"Using special placement: ({special_adjustment.x():.1f}, {special_adjustment.y():.1f})"
                )
            ...
            logger.info(
                f"Using default calculation: ({default_adjustment.x():.1f}, {default_adjustment.y():.1f})"
            )
