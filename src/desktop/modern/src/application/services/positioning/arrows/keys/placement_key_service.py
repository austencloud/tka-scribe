...
class PlacementKeyService:
    ...
    def generate_placement_key_legacy(
        self,
        motion_data: MotionData,
        grid_mode: str = "diamond",
        ...
        """
        Legacy-style method signature for backward compatibility.

        This method provides a simplified interface that matches the old API
        while internally using the full placement key generation logic.
        """
        # Create minimal arrow and pictograph data for compatibility
        from domain.models.pictograph_models import ArrowData, PictographData

        arrow_data = ArrowData(motion_data=motion_data)
        pictograph_data = PictographData(letter=None)  # No letter context

        # Use empty default placements - will fall back to motion type
        default_placements = {}

        return self.generate_placement_key(
            arrow_data, pictograph_data, default_placements, grid_mode
        )
    ...
    def debug_key_generation(
        ...
        motion_data = arrow_data.motion_data
        if not motion_data:
            return "static"
        ...
        # Show pictograph analysis
        pictograph_checker = PictographCheckerService(pictograph_data)
        print(f"  Alpha ending: {pictograph_checker.ends_with_alpha()}")
        print(f"  Beta ending: {pictograph_checker.ends_with_beta()}")
        print(f"  Gamma ending: {pictograph_checker.ends_with_gamma()}")
        print(f"  Layer3 (hybrid): {pictograph_checker.ends_with_layer3()}")
        print(f"  Radial props: {pictograph_checker.ends_with_radial_ori()}")
        print(f"  Nonradial props: {pictograph_checker.ends_with_nonradial_ori()}")

        motion_end_ori = self._get_motion_end_orientation(motion_data)
        print(f"  End Orientation: {motion_end_ori}")

        key = self.generate_placement_key(
