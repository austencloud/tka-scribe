...
class OptionPickerDataService(IOptionPickerDataService):
    ...
    def refresh_from_sequence_data(self, sequence_data: List[Dict[str, Any]]) -> List[BeatData]:
        """
    ...
    def refresh_from_modern_sequence(self, sequence: SequenceData) -> List[BeatData]:
        ...
        try:
            ...
            if hasattr(self.beat_loader, 'refresh_options_from_modern_sequence'):
                options = self.beat_loader.refresh_options_from_modern_sequence(sequence)
            else:
            ...
            if option_id.startswith("beat_"):
                ...
                for beat_data in current_options:
                    if beat_data.letter == target_letter:
                        logger.debug(f"Found beat data for option {option_id}: {beat_data.letter}")
                        return beat_data

                logger.debug(f"No beat data found for option {option_id} (letter: {target_letter})")
                return None
            else:
    ...
    def search_options(self, search_term: str) -> List[BeatData]:
        ...
        try:
            ...
            for beat_data in current_options:
                # Search by letter
                if beat_data.letter and beat_data.letter.lower().startswith(search_term_lower):
                    filtered_options.append(beat_data)
                # Could add more search criteria here (e.g., by motion type)

            logger.debug(f"Found {len(filtered_options)} options matching '{search_term}'")
