...
from core.interfaces.organization_services import ICodePatternAnalysisService
...
class CodePatternAnalysisService(ICodePatternAnalysisService):
    ...
    def __init__(self):
        ...
        self.external_library_prefixes = [
            "typing", "pathlib", "os", "sys", "json", "logging", "dataclasses",
            "enum", "abc", "collections", "itertools", "functools", "operator",
            "time", "datetime", "threading", "asyncio", "concurrent", "multiprocessing",
            "PyQt6", "PySide6", "pytest", "hypothesis", "numpy", "pandas",
            "requests", "fastapi", "uvicorn", "pydantic", "sqlalchemy", "alembic"
        ]
    ...
    def is_external_library(self, module_name: str) -> bool:
        ...
        return any(
            ...
            for prefix in self.external_library_prefixes
        )
    ...
    def generate_import_recommendations(self, module_name: str, violation_type: str) -> List[str]:
        """
        ...
        if violation_type == "src_prefix_violations":
            ...
            recommendations.append(f"Remove 'src.' prefix: {module_name} -> {fixed_import}")
        
        elif violation_type == "modern_prefix_violations":
            # Remove modern. prefix
            fixed_import = module_name.replace("modern.", "", 1)
            recommendations.append(f"Remove 'modern.' prefix: {module_name} -> {fixed_import}")
        
        elif violation_type == "non_standard_patterns":
            # Suggest standard TKA patterns
            if "models" in module_name:
                recommendations.append(f"Use standard pattern: from domain.models.{module_name.split('.')[-1]} import ...")
            elif "services" in module_name:
                recommendations.append(f"Use standard pattern: from application.services.{module_name.split('.')[-1]} import ...")
            elif "components" in module_name:
                recommendations.append(f"Use standard pattern: from presentation.components.{module_name.split('.')[-1]} import ...")
            else:
            ...
            recommendations.append("Consider using absolute imports for deep hierarchies")
    ...
    def calculate_pattern_compliance_score(
        ...
        violation_penalty = (violations / total_imports) * 50  # Up to 50% penalty for src. prefix
        inconsistency_penalty = (inconsistencies / total_imports) * 30  # Up to 30% penalty for other issues
    ...
    def validate_import_pattern(self, import_statement: str) -> dict:
        ...
        if violations:
            ...
            recommendations = self.generate_import_recommendations(module_name, violation_type)
