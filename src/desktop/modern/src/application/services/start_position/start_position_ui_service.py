...
class StartPositionUIService(IStartPositionUIService):
    ...
    # Advanced picker positions (16 positions like legacy)
    DIAMOND_ADVANCED_POSITIONS = [
        "alpha1_alpha1", "alpha2_alpha2", "alpha3_alpha3", "alpha4_alpha4",
        "alpha5_alpha5", "alpha6_alpha6", "alpha7_alpha7", "alpha8_alpha8",
        "beta1_beta1", "beta2_beta2", "beta3_beta3", "beta4_beta4",
        "beta5_beta5", "beta6_beta6", "beta7_beta7", "beta8_beta8"
    ]
    
    BOX_ADVANCED_POSITIONS = [
        "gamma1_gamma1", "gamma2_gamma2", "gamma3_gamma3", "gamma4_gamma4",
        "gamma5_gamma5", "gamma6_gamma6", "gamma7_gamma7", "gamma8_gamma8",
        "gamma9_gamma9", "gamma10_gamma10", "gamma11_gamma11", "gamma12_gamma12",
        "gamma13_gamma13", "gamma14_gamma14", "gamma15_gamma15", "gamma16_gamma16"
    ]
    ...
    def calculate_option_size(self, container_width: int, is_advanced: bool = False) -> int:
        ...
        """
        try:
            # Use the same legacy sizing formula: main_window_width // 10 for regular picker
            if is_advanced:
                # Advanced picker uses slightly smaller size (main_window_width // 12)
            ...
            logger.debug(f"Calculated option size: {size}px (advanced={is_advanced}, container={container_width}px)")
            return size
    ...
    def get_positions_for_mode(self, grid_mode: str, is_advanced: bool = False) -> List[str]:
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(positions)} positions for {grid_mode} mode (advanced={is_advanced})")
    ...
    def should_use_single_row_layout(self, container_width: int, position_count: int, option_size: int, spacing: int) -> bool:
        """
        ...
        try:
            ...
            total_width_needed = (position_count * option_size) + ((position_count - 1) * spacing)
            
            # Add some buffer for margins
            buffer = 100
            
            result = (total_width_needed + buffer) <= container_width
            logger.debug(f"Single row layout check: needed={total_width_needed}, available={container_width}, use_single_row={result}")
