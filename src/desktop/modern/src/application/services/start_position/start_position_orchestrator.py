...
class StartPositionOrchestrator(IStartPositionOrchestrator):
        ...
        try:
            ...
            if command_processor:
                try:
                    ...
                    if result.success:
                        logger.info(f"✅ Position selection completed successfully: {position_key}")
                        return True
                    else:
                        logger.error(f"❌ Command execution failed: {result.error_message}")
                        return False
                ...
                logger.warning("⚠️ Command processor not available, using fallback approach")
                # Fallback: execute command directly
                try:
                    command.execute()
                    logger.info(f"✅ Position selection completed via fallback: {position_key}")
                    return True
                except Exception as e:
    ...
    def get_position_data_for_display(self, position_key: str, grid_mode: str) -> Optional[PictographData]:
        ...
        """
        try:
            logger.debug(f"Getting position data for display: {position_key} ({grid_mode})")
            
            # Retrieve data from data service
            pictograph_data = self.data_service.get_position_data(position_key, grid_mode)
            
            if pictograph_data:
                logger.debug(f"Successfully retrieved position data for display: {position_key}")
    ...
    def calculate_responsive_layout(self, container_size: QSize, position_count: int) -> Dict[str, Any]:
        ...
        """
        try:
            logger.debug(f"Calculating responsive layout for {position_count} positions in {container_size}")
            
            # Delegate to UI service
            layout_params = self.ui_service.calculate_responsive_layout(container_size, position_count)
    ...
    def get_positions_and_layout_for_mode(self, grid_mode: str, is_advanced: bool, container_size: QSize) -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.debug(f"Getting positions and layout for {grid_mode} mode (advanced={is_advanced})")
            
            # Get positions from UI service
            positions = self.ui_service.get_positions_for_mode(grid_mode, is_advanced)
            
            # Get layout configuration
            layout_config = self.ui_service.get_grid_layout_config(grid_mode, is_advanced)
            
            # Calculate responsive layout
            responsive_layout = self.calculate_responsive_layout(container_size, len(positions))
    ...
    def validate_and_normalize_position(self, position_key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"Validated and normalized position: {position_key} -> {normalized_key}")
    ...
    def preload_position_data(self, grid_mode: str, is_advanced: bool = False) -> int:
        ...
        """
        try:
            logger.debug(f"Preloading position data for {grid_mode} mode (advanced={is_advanced})")
            ...
            logger.info(f"Preloaded {loaded_count}/{len(positions)} positions for {grid_mode} mode")
            return loaded_count
