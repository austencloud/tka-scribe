...
class ArrowRenderingService(IArrowRenderingService):
    ...
    def calculate_arrow_position(
        ...
        if positioning_orchestrator and pictograph_data:
            try:
                return positioning_orchestrator.calculate_arrow_position(
                    arrow_data, pictograph_data
                )
            except Exception as e:
                logger.warning(f"Positioning orchestrator failed: {e}")
        
        # Fallback to basic positioning
        logger.debug("Using fallback arrow positioning")
    ...
    def get_location_position(
        ...
        Args:
            location: Location to get coordinates for
            coordinate_system: Optional coordinate system service
        ...
        if coordinate_system:
            try:
                # Use the coordinate system service if available
                ...
                point = coordinate_system.get_initial_position(dummy_motion, location)
                return (point.x(), point.y())
            except Exception as e:
                logger.warning(f"Coordinate system service failed: {e}")
