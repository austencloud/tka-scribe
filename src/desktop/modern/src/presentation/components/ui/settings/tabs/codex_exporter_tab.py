...
class CodexExporterTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Description
        description = QLabel("Export all pictographs with customizable turn configurations")
    ...
    def _load_settings(self):
        ...
        grid_mode = self.ui_state_service.get_setting("codex_exporter/grid_mode", "diamond")
        generate_all = self.ui_state_service.get_setting("codex_exporter/generate_all", False)
        quality = self.ui_state_service.get_setting("codex_exporter/quality", 95)
        include_metadata = self.ui_state_service.get_setting("codex_exporter/include_metadata", True)
    ...
    def _save_settings(self):
        ...
        self.ui_state_service.set_setting("codex_exporter/red_turns", self.red_turns_spin.value())
        self.ui_state_service.set_setting("codex_exporter/blue_turns", self.blue_turns_spin.value())
        self.ui_state_service.set_setting("codex_exporter/grid_mode", self.grid_mode_combo.currentText().lower())
        self.ui_state_service.set_setting("codex_exporter/generate_all", self.generate_all_checkbox.isChecked())
        self.ui_state_service.set_setting("codex_exporter/quality", self.quality_spin.value())
        self.ui_state_service.set_setting("codex_exporter/include_metadata", self.include_metadata_checkbox.isChecked())
