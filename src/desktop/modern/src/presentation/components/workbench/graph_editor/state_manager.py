...
class GraphEditorStateManager(QObject):
    ...
    def set_visibility(self, is_visible: bool, emit_signal: bool = True) -> None:
        ...
        if self._is_visible != is_visible:
            ...
            print(f"📊 State: Visibility changed to {is_visible}")
    ...
    # Sequence State
    def set_current_sequence(self, sequence: Optional[SequenceData], emit_signal: bool = True) -> None:
        """
        ...
        if self._current_sequence != sequence:
            ...
            print(f"📊 State: Sequence changed to {sequence}")
    ...
    def set_selected_beat(self, 
        ...
        """
        state_changed = (self._selected_beat != beat_data or 
        ...
        if state_changed:
            ...
            print(f"📊 State: Selected beat changed to index {beat_index}: {beat_data}")
    ...
    def set_selected_beat_data(self, beat_data: Optional[BeatData], beat_index: Optional[int] = None) -> None:
        """
        ...
        """
        index_to_use = beat_index if beat_index is not None else self._selected_beat_index
    ...
    def clear_selected_beat(self, emit_signal: bool = True) -> None:
        ...
        if self._selected_beat is not None or self._selected_beat_index is not None:
            ...
            print("📊 State: Selected beat cleared")
            
            if emit_signal:
                self.selected_beat_changed.emit(None, -1)
    
    # Arrow State
    def set_selected_arrow_id(self, arrow_id: Optional[str], emit_signal: bool = True) -> None:
        """
        ...
        if self._selected_arrow_id != arrow_id:
            ...
            print(f"📊 State: Selected arrow changed to {arrow_id}")
    ...
    def clear_selected_arrow(self, emit_signal: bool = True) -> None:
        ...
        if self._selected_arrow_id is not None:
            ...
            print(f"📊 State: Selected arrow cleared (was {old_arrow})")
    ...
    def get_state_summary(self) -> dict:
        ...
        return {
            ...
            'sequence_id': getattr(self._current_sequence, 'id', None) if self._current_sequence else None,
    ...
    def _validate_state(self) -> bool:
        ...
        if (self._current_sequence and 
            hasattr(self._current_sequence, 'beats') and 
            self._selected_beat_index is not None):
            
            beats_count = len(self._current_sequence.beats)
            if self._selected_beat_index < 0 or self._selected_beat_index >= beats_count:
                errors.append(f"Beat index {self._selected_beat_index} out of bounds (0-{beats_count-1})")
        ...
        if not self._state_consistent:
            print(f"⚠️ State validation failed: {self._last_validation_error}")
        
        return self._state_consistent
    
    def reset_all_state(self) -> None:
        """Reset all state to initial values"""
        print("🔄 State: Resetting all state")
    ...
    def update_data_flow_context(self, data_flow_service) -> None:
        ...
        """
        if (self._current_sequence and 
            self._selected_beat_index is not None and 
            data_flow_service):
            
            data_flow_service.set_context(self._current_sequence, self._selected_beat_index)
            print(f"📊 Updated data flow context: sequence with beat index {self._selected_beat_index}")
    ...
    def sync_with_graph_service(self, graph_service) -> None:
        ...
        if graph_service:
            ...
            if self._selected_beat and self._selected_beat_index is not None:
                graph_service.set_selected_beat(self._selected_beat, self._selected_beat_index)
            
            if self._selected_arrow_id:
                graph_service.set_arrow_selection(self._selected_arrow_id)
            
            print("📊 Synchronized state with graph service")
