...
class StartPositionOption(QWidget):
    ...
    def _setup_ui(self):
        ...
        if self._pictograph_component:
            ...
            if pictograph_data:
                logger.debug(f"✅ Loaded pictograph data for {self.position_key}")
    ...
    def paintEvent(self, event):
        ...
        if self._is_selected:
            ...
            selection_rect = QRectF(self.rect().adjusted(1, 1, -1, -1))
            painter.drawRoundedRect(selection_rect, 20, 20)
    ...
    def _update_pictograph_size(self, size: int = None):
        """Update the pictograph size to match the container."""
        if self._pictograph_component:
            if size is None:
                # Calculate size based on container size with padding
                container_size = min(self.width(), self.height())
                size = container_size - 40  # Leave 40px padding

            # Ensure minimum size
            size = max(size, 60)

            # Update the pictograph component size
            self._pictograph_component.setFixedSize(size, size)

            # Force update
            self._pictograph_component.update()
            self.update()

            logger.debug(
                f"Updated pictograph size to {size}px for position {self.position_key}"
            )
    ...
    def resizeEvent(self, event):
        ...
        # Update pictograph size when the widget is resized
        self._update_pictograph_size()

        # Update selection overlay if it exists
        if self._selection_overlay:
            self._selection_overlay.resize(self.size())
