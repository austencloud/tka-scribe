...
def demonstrate_positioning_difference():
    ...
    for row in range(rows):
        for col in range(cols):
            x = margin + col * (beat_size + margin)  # X positioning can keep margin for spacing
            y = additional_height_top + row * beat_size  # Y positioning should NOT add margin
            print(f"  Beat at row {row}, col {col}: x={x}, y={y}")
    ...
    print(f"Extra height in modern version: {modern_first_beat_y - legacy_first_beat_y} pixels")
...
def create_visual_comparison():
    ...
    for row in range(rows):
        for col in range(cols):
            x = margin + col * (beat_size + margin)
            y = additional_height_top + margin + row * (beat_size + margin)  # BUG: extra margin
