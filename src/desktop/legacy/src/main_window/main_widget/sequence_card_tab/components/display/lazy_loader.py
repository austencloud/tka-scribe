...
class LazyImageLoader(QObject):
    ...
    def __init__(self, scroll_area: QScrollArea, image_processor, buffer_pixels: int = 200):
        """
    ...
    def register_image(self, image_path: str, widget: QWidget, load_callback: Callable, **load_params) -> QPixmap:
        """
    ...
    def _is_widget_visible(self, widget: QWidget) -> bool:
        ...
        try:
            ...
            visible_rect.adjust(-self.buffer_pixels, -self.buffer_pixels, 
    ...
    def force_load_all(self) -> None:
        ...
        try:
            for image_path in list(self.pending_images.keys()):
                if image_path not in self.loaded_images and image_path not in self.loading_images:
                    self._load_image(image_path)
