...
function fixFile(filePath, errors) {
  const fullPath = path.join(
    "F:\\_THE KINETIC ALPHABET\\_TKA-STUDIO",
    "src",
    filePath
  );
  ...
  sortedErrors.forEach((err) => {
    ...
    if (line.includes("catch") && line.includes(varName)) {
      const newName =
        varName === "error" || varName === "err" || varName === "_e"
          ? "_error"
          : `_${varName}`;
      ...
      fileFixCount++;
      console.log(
        `  Fixed catch parameter '${varName}' -> '${newName}' at line ${err.line}`
      );
      return;
    }
    ...
    if (
      msg.includes("defined but never used") &&
      (line.includes("=>") ||
      ...
      fileFixCount++;
      console.log(
        `  Fixed parameter '${varName}' -> '${newName}' at line ${err.line}`
      );
      return;
    }
    ...
    if (msg.includes("assigned a value but never used")) {
      ...
      fileFixCount++;
      console.log(
        `  Fixed assigned variable '${varName}' -> '${newName}' at line ${err.line}`
      );
      return;
    }
    ...
    if (line.includes("{") && line.includes("}") && line.includes(varName)) {
      ...
      fileFixCount++;
      console.log(
        `  Fixed destructured variable '${varName}' -> '${newName}' at line ${err.line}`
      );
      return;
    }
    ...
    fileFixCount++;
    console.log(
      `  Fixed variable '${varName}' -> '${newName}' at line ${err.line}`
    );
  });
