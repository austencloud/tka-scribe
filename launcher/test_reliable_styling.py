...
def test_reliable_components():
    ...
    assert "background-color:" in glass_surface, "Glass surface should have background-color"
    assert "border:" in glass_surface, "Glass surface should have border"
    
    hover_surface = style_builder.glass_surface_hover("primary")
    assert "background-color:" in hover_surface, "Hover surface should have background-color"
    assert "border:" in hover_surface, "Hover surface should have border"
    
    accent_button = style_builder.accent_button()
    assert "background-color:" in accent_button, "Accent button should have background-color"
    assert "color:" in accent_button, "Accent button should have text color"
    
    secondary_button = style_builder.secondary_button()
    assert "background-color:" in secondary_button, "Secondary button should have background-color"
    ...
    try:
        ...
        assert hasattr(button, 'animation_manager'), "Button should have animation_manager"
        ...
        assert id(button) in shadow_manager.active_shadows, "Button should be in active shadows"
