...
class ApplicationData:
    ...
    def with_status(self, status: ApplicationStatus, process_id: Optional[int] = None) -> "ApplicationData":
    ...
    def mark_launched(self, timestamp: str, process_id: Optional[int] = None) -> "ApplicationData":
...
class LauncherState:
    ...
    def with_application_status(self, app_id: str, status: ApplicationStatus, 
                              process_id: Optional[int] = None) -> "LauncherState":
    ...
    def with_selected_category(self, category: Optional[ApplicationCategory]) -> "LauncherState":
    ...
    def get_applications_by_category(self, category: ApplicationCategory) -> List[ApplicationData]:
        """Get all applications in a category."""
        return [app for app in self.applications.values() if app.category == category and app.enabled]
    
    def get_running_applications(self) -> List[ApplicationData]:
        """Get all currently running applications."""
        return [app for app in self.applications.values() if app.status == ApplicationStatus.RUNNING]
    
    def get_favorite_applications(self) -> List[ApplicationData]:
        """Get all favorite applications."""
        return [app for app in self.applications.values() if app.is_favorite and app.enabled]
...
class LaunchResult:
    ...
    @classmethod
    def success_result(cls, request: LaunchRequest, process_id: int, 
                      execution_time_ms: int = 0) -> "LaunchResult":
    ...
    @classmethod
    def error_result(cls, request: LaunchRequest, error_message: str, 
                    execution_time_ms: int = 0) -> "LaunchResult":
