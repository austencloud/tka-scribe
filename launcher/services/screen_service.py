...
class ScreenService(IScreenService):
    ...
    def __init__(self):
        ...
        if not self._app:
            logger.warning("No QApplication instance found, screen detection may be limited")
    ...
    def get_current_screen(self, window_geometry: WindowGeometry) -> Optional[ScreenData]:
    ...
    def calculate_docked_geometry(self, screen: ScreenData, dock_width: int = 110) -> WindowGeometry:
    ...
    def calculate_centered_geometry(self, screen: ScreenData, width: int, height: int) -> WindowGeometry:
    ...
    def is_geometry_on_screen(self, geometry: WindowGeometry, screen: ScreenData) -> bool:
        ...
        return (geometry.x < screen_right and 
                window_right > screen.geometry.x and
                geometry.y < screen_bottom and 
                window_bottom > screen.geometry.y)
    ...
    def _point_in_screen(self, x: int, y: int, screen: ScreenData) -> bool:
        ...
        return (screen.geometry.x <= x < screen.geometry.x + screen.geometry.width and
                screen.geometry.y <= y < screen.geometry.y + screen.geometry.height)
