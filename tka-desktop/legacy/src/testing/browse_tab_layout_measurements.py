...
class BrowseTabLayoutMeasurements:
    ...
    def measure_layout_dimensions(self, event_name: str = "measurement") -> Dict:
        ...
        try:
            ...
            self.logger.info(f"\n📊 EXPECTED DIMENSIONS:")
            self.logger.info(f"Expected Left:  {expected_left_width:.0f}px (2/3 of {main_width}px)")
            self.logger.info(f"Expected Right: {expected_right_width:.0f}px (1/3 of {main_width}px)")
            ...
            left_diff_pct = abs(left_width - expected_left_width) / expected_left_width * 100 if expected_left_width > 0 else 0
            right_diff_pct = abs(right_width - expected_right_width) / expected_right_width * 100 if expected_right_width > 0 else 0
            ratio_diff_pct = abs(actual_ratio - expected_ratio) / expected_ratio * 100 if expected_ratio > 0 else 0
            
            self.logger.info(f"\n📈 ACTUAL vs EXPECTED:")
            self.logger.info(f"Left Panel:  {left_width}px vs {expected_left_width:.0f}px (diff: {left_diff_pct:.1f}%)")
            self.logger.info(f"Right Panel: {right_width}px vs {expected_right_width:.0f}px (diff: {right_diff_pct:.1f}%)")
            self.logger.info(f"Ratio:       {actual_ratio:.3f} vs {expected_ratio:.3f} (diff: {ratio_diff_pct:.1f}%)")
            ...
            self.logger.info(f"Left Panel:  {'✅ PASS' if left_pass else '❌ FAIL'} ({left_diff_pct:.1f}%)")
            self.logger.info(f"Right Panel: {'✅ PASS' if right_pass else '❌ FAIL'} ({right_diff_pct:.1f}%)")
            self.logger.info(f"Ratio:       {'✅ PASS' if ratio_pass else '❌ FAIL'} ({ratio_diff_pct:.1f}%)")
    ...
    def generate_measurement_report(self):
        ...
        passed_measurements = sum(1 for m in self.measurement_history if m.get('overall_pass', False))
        
        self.logger.info(f"Total measurements: {total_measurements}")
        self.logger.info(f"Passed measurements: {passed_measurements}")
        self.logger.info(f"Failed measurements: {total_measurements - passed_measurements}")
        self.logger.info(f"Success rate: {passed_measurements/total_measurements*100:.1f}%")
        
        # Show failed measurements
        failed_measurements = [m for m in self.measurement_history if not m.get('overall_pass', True)]
        if failed_measurements:
            self.logger.info(f"\n🚨 FAILED MEASUREMENTS:")
            for m in failed_measurements:
                self.logger.info(f"  - {m['event']}: Ratio {m['actual_ratio']:.3f} (expected 2.0)")
        
        # Show latest measurement
        if self.measurement_history:
            latest = self.measurement_history[-1]
            self.logger.info(f"\n📈 LATEST MEASUREMENT ({latest['event']}):")
            self.logger.info(f"  Left: {latest['left_stack_width']}px ({latest['left_diff_pct']:.1f}% diff)")
            self.logger.info(f"  Right: {latest['right_stack_width']}px ({latest['right_diff_pct']:.1f}% diff)")
            self.logger.info(f"  Ratio: {latest['actual_ratio']:.3f} ({latest['ratio_diff_pct']:.1f}% diff)")
            self.logger.info(f"  Status: {'✅ PASS' if latest['overall_pass'] else '❌ FAIL'}")
...
class BrowseTabLayoutTester(QObject):
    ...
    def _test_browse_tab_activation(self):
        ...
        try:
            ...
            if tab_manager:
                ...
                activation = self.measurements.measure_layout_dimensions("browse_tab_activated")
    ...
    def _test_thumbnail_selection(self):
        ...
        try:
            ...
            if thumbnail_boxes:
                ...
                before = self.measurements.measure_layout_dimensions("before_thumbnail_selection")
    ...
    def _complete_test(self, success: bool, message: str):
        ...
        latest_measurement = self.measurements.measurement_history[-1] if self.measurements.measurement_history else {}
