...
class ModernToggleTab(QWidget):
    ...
    def _position_tab(self):
        """Position the toggle tab at the bottom left of workbench"""
        # Find the correct parent workbench
        workbench_parent = self._find_workbench_parent()
        ...
        if workbench_parent.width() <= 10 or workbench_parent.height() <= 10:
            # Defer positioning until parent has valid size
            QTimer.singleShot(100, self._position_tab)
            return

        # Reparent to workbench if not already
        if self.parent() != workbench_parent:
            self.setParent(workbench_parent)

        # Force widget to calculate its size
        self.adjustSize()

        # Position at bottom left with safety margins
        x = 20  # 20px from left edge
        y = workbench_parent.height() - self.height() - 20  # 20px from bottom
        ...
        max_x = workbench_parent.width() - self.width()
        max_y = workbench_parent.height() - self.height()
        x = min(x, max(max_x, 0))
        y = min(y, max(max_y, 0))
        ...
        print(f"   Parent size: {workbench_parent.width()}x{workbench_parent.height()}")
        print(f"   Tab size: {self.width()}x{self.height()}")

    def _find_workbench_parent(self):
        """Find the workbench parent by traversing up the hierarchy"""
        current = self._graph_editor.parent()

        # Look for the workbench by checking for specific attributes
        while current:
            # Check if this is the workbench (has beat frame section)
            if hasattr(current, "_beat_frame_section"):
                return current
            # Also check class name as backup
            if "Workbench" in type(current).__name__:
                return current
    ...
    def update_position(self, animate=True):
        """Update position to stay at bottom left of workbench"""
        workbench_parent = self._find_workbench_parent()
        if not workbench_parent:
            return
        ...
        # Update button text based on graph editor state
    ...
    def eventFilter(self, obj, event):
        ...
        if (
            ...
            and workbench_parent
            and obj == workbench_parent
        ):
