...
export class TKAAPIService {
    ...
    async updateSequence(id: string, data: CreateSequenceRequest): Promise<SequenceData> {
        const response = await fetch(`${this.baseUrl}/api/sequences/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
...
import { tkaAPI, type SequenceData, type CreateSequenceRequest } from '../api/tka-api';
