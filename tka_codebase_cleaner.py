...
class TKACodebaseAnalyzer:
    ...
    def _find_event_bus_patterns(self, file_path: Path, content: str, visitor) -> None:
        ...
        for pattern in event_patterns:
            ...
            for match in matches:
                ...
                self.event_bus_usage.append((file_str, f"Line {line_num}: {match.group()}"))
        ...
        for pattern in signal_patterns:
            ...
            for match in matches:
                line_num = content[:match.start()].count('\n') + 1
                self.signal_usage.append((file_str, f"Line {line_num}: {match.group()}"))
    ...
    def _find_complexity_issues(self, file_path: Path, content: str, visitor) -> None:
        ...
        for i, line in enumerate(lines):
            ...
            if indent_level > 20:  # Very deep nesting
                self.opportunities.append(CleanupOpportunity(
                    category="complexity",
                    severity="medium",
    ...
    def _analyze_unused_imports(self) -> None:
        ...
        for file_path, imports in self.imports_by_file.items():
            ...
            for import_name in imports:
                ...
                is_used = (
                    import_name in function_calls or
                    import_name in content or
                    any(import_name in call for call in function_calls)
                )
    ...
    def _is_likely_dynamic_import(self, import_name: str) -> bool:
        ...
        dynamic_patterns = [
            'pytest', 'unittest', 'mock',  # Testing frameworks
            'logging', 'sys', 'os', 'Path',  # Common utilities
            'Qt', 'PyQt',  # Qt might be used in string form
            'typing',  # Type hints
        ]
    ...
    def _analyze_ai_generated_bloat(self) -> None:
        ...
        for file_path, classes in self.class_definitions.items():
            ...
            manager_classes = [cls for cls in classes if 'Manager' in cls or 'Service' in cls]
        ...
        for file_path, line_num, comment in self.todo_comments:
            ai_indicators = [
                'implement', 'placeholder', 'stub', 'todo', 'fixme',
                'add logic here', 'complete this', 'finish implementation'
            ]
    ...
    def generate_report(self, output_file: str = None) -> str:
        ...
        for opp in sorted_opportunities:
            ...
            file_short = Path(opp.file_path).name if opp.file_path != "(multiple)" else opp.file_path
        ...
        if self.event_bus_usage or self.signal_usage:
            ...
            if len(self.event_bus_usage) < len(self.signal_usage) / 4:
                report_lines.append("  💡 Recommendation: Consider removing event bus and standardizing on Qt signals")
        ...
        if self.large_functions:
            ...
            sorted_funcs = sorted(self.large_functions, key=lambda x: x[2], reverse=True)[:5]
            for file_path, func_name, line_count in sorted_funcs:
                file_short = Path(file_path).name
                report_lines.append(f"    📄 {file_short}: {func_name}() - {line_count} lines")
...
def main():
    ...
    if total_issues > 0:
        high_priority = sum(1 for opp in analyzer.opportunities if opp.severity == 'high')
